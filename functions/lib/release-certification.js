...
function evaluate(input,now=Date.now()){
  ...
  checks.push(check("production_health","Production health",health.status&&health.status!=="critical"?"passed":"blocked",health.status==="warning"?`Monitor has ${Number(health.counts&&health.counts.warning)||0} warning(s), but no release-blocking critical condition`:`Monitor status: ${String(health.status||"missing")}`));
