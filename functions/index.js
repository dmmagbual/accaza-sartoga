...
function advanceFundingAccount(row) {
  ...
  if (id === "revolving_fund") return {kind:"revolving_fund", account:"asset:petty_cash", id:"revolving_fund"};
...
exports.postFinancialCommand = onCall(
  ...
  async (request) => observeFinancialOperation(request, "postFinancialCommand", async () => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      if (invoiceDate > openingDate) throw new HttpsError("invalid-argument", "The original invoice date cannot be after the opening-balance date.");
      const originalAmount = amount(data.originalAmount), paidBeforeOpening = Financial.money(data.paidBeforeOpening || 0);
      if (!Number.isFinite(paidBeforeOpening) || paidBeforeOpening < 0) throw new HttpsError("invalid-argument", "Amount paid before the opening date cannot be negative.");
      if (paidBeforeOpening > originalAmount + 0.009) throw new HttpsError("invalid-argument", "Amount paid before the opening date cannot exceed the original invoice amount.");
      const value = Financial.money(originalAmount - paidBeforeOpening);
      if (!(value > 0)) throw new HttpsError("invalid-argument", "The invoice has no unpaid opening balance.");
      if (data.confirmedOutstanding !== true) throw new HttpsError("failed-precondition", "Confirm that this amount was genuinely unpaid at the opening date and is not a new purchase.");
      const normalizedReference = invoiceReference.toLowerCase().replace(/\s+/g, " "), referenceHash = crypto.createHash("sha256").update(`${supplier.id}|${normalizedReference}`).digest("hex").slice(0, 32), docId = `ap_open_${referenceHash}`, postingId = `supplier_opening_balance_${referenceHash}`;
      const existing = (await db.ref(`/payables/${docId}`).get()).val();
      if (existing) {
        if (financeText(existing.supplierId, 160) !== supplier.id || financeText(existing.ref, 120).trim().toLowerCase().replace(/\s+/g, " ") !== normalizedReference) throw new HttpsError("already-exists", "The opening-balance reference conflicts with another payable. Review it before posting.");
        return {movementId: financeText(existing.movementId, 160) || postingId, documentId: docId, amount: Financial.money(existing.amount), duplicate: true};
      }
      movementIdOverride = postingId;
      movement = Financial.movement("supplier_opening_balance", "payable", docId, [Financial.line("equity:opening_balance", value, 0, `Opening payable · ${supplier.name}`), Financial.line(`liability:payable:${docId}`, 0, value, `Opening payable · ${supplier.name}`)], {occurredAt: accountingTimestamp(openingDate, now), actorName: actor.role, reference: invoiceReference, supplierId: supplier.id, supplierName: supplier.name, originalInvoiceDate: invoiceDate, originalInvoiceAmount: originalAmount, paidBeforeOpening});
      writes[`payables/${docId}`] = {supplierId: supplier.id, party: supplier.name, type: "opening balance", amount: value, remainingAmount: value, originalInvoiceAmount: originalAmount, paidBeforeOpening, date: invoiceDate, openingDate, due, ref: invoiceReference, status: "open", openingBalance: true, movementId: postingId, liabilityAccount: `liability:payable:${docId}`, reversalOffsetAccount: "equity:opening_balance", ts: now, createdBy: actor.uid, schemaVersion: 3};
      writes[`openingPayableReferences/${referenceHash}`] = {supplierId: supplier.id, normalizedReference, documentId: docId, movementId: postingId, status: "posted", postedAt: now, postedBy: actor.uid, schemaVersion: 1};
      ...
      if (doc.status === "reversed") return {movementId: financeText(doc.reversalMovementId, 160), documentId: docId, amount: Financial.money(doc.amount), duplicate: true};
      if (doc.status !== "open" || Number(doc.paidAmount || 0) > 0 || doc.settlementMovementId) throw new HttpsError("failed-precondition", "This opening balance has been paid or otherwise settled. Reverse that settlement first.");
      const reason = financeText(data.reason, 300), reference = financeText(data.ref, 120), date = financeDate(data.date);
      if (!reason || !reference) throw new HttpsError("invalid-argument", "A reversal reason and correction reference are required.");
      const originalId = financeKey(doc.movementId, "Opening-balance movement ID"), original = (await db.ref(`/financialMovements/${originalId}`).get()).val();
      if (!original || original.type !== "supplier_opening_balance" || financeText(original.sourceId, 160) !== docId) throw new HttpsError("failed-precondition", "The original supplier opening-balance journal is missing or mismatched.");
      if (original.reversedByMovementId) throw new HttpsError("failed-precondition", "The original supplier opening-balance journal has already been reversed.");
      const reverseId = `supplier_opening_balance_reverse_${docId}`, reversal = Financial.reverseMovement(Object.assign({}, original, {id: originalId}), "supplier_opening_balance_reversed", "Reverse supplier opening balance");
      if (!BooksBridge.linesBalanced(reversal.lines)) throw new HttpsError("failed-precondition", "The calculated opening-balance reversal is unbalanced.");
      movementIdOverride = reverseId;
      movement = Object.assign(reversal, {sourceType: "payable", sourceId: docId, occurredAt: accountingTimestamp(date, now), actorName: actor.role, reference, reason, reversalOf: originalId, reversesMovementId: originalId});
      writes[`payables/${docId}/status`] = "reversed"; writes[`payables/${docId}/remainingAmount`] = 0; writes[`payables/${docId}/reversedAt`] = now; writes[`payables/${docId}/reversedBy`] = actor.uid; writes[`payables/${docId}/reversalReason`] = reason; writes[`payables/${docId}/reversalReference`] = reference; writes[`payables/${docId}/reversalMovementId`] = reverseId;
      writes[`financialMovements/${originalId}/reversedByMovementId`] = reverseId;
      const referenceHash = docId.replace(/^ap_open_/, ""); if (referenceHash) writes[`openingPayableReferences/${referenceHash}/status`] = "reversed";
      writes[`operationalAudit/${now}_supplier_opening_balance_reverse_${docId}`] = operationalAuditRecord("reverse_supplier_opening_balance", "payable", docId, actor, {movementId: reverseId, reversedMovementId: originalId, supplierId: financeText(doc.supplierId, 160), supplierName: financeText(doc.party, 120), invoiceReference: financeText(doc.ref, 120), amount: Financial.money(doc.amount), date, reference, reason, accounting: "Debit Supplier Accounts Payable and credit Owner's Capital. No cash, inventory, purchase, or current-period expense changed."});
      result = {documentId: docId, amount: Financial.money(doc.amount), supplierName: financeText(doc.party, 120)};
...
exports.settlePlatformPayout = onCall(
  ...
  async (request) => {
    ...
    const orderSnapshots = found.map((entry) => { const o=entry.order||{},gross=Financial.money(o.grossPlatform!=null?o.grossPlatform:(o.subtotal!=null?o.subtotal:o.total)),commission=Financial.money(o.commission),discount=Financial.money(o.platformDiscount),hasMappedDiscounts=o.platformMerchantPromo!=null||o.platformDeliveryFeeDiscount!=null,discountParts=(Array.isArray(o.platformDiscountLines)?o.platformDiscountLines:[]).reduce((out,row)=>{const value=Financial.money(row&&row.amount),category=financeText(row&&row.category,60).toLowerCase(),label=financeText(row&&row.type,60).toLowerCase();if(category==="delivery_fee_discount"||label.includes("delivery fee"))out.delivery=Financial.money(out.delivery+value);else out.promo=Financial.money(out.promo+value);return out;},{promo:0,delivery:0}),merchantPromo=hasMappedDiscounts?Financial.money(o.platformMerchantPromo):(discountParts.promo||(!discountParts.delivery?discount:0)),deliveryFeeDiscount=hasMappedDiscounts?Financial.money(o.platformDeliveryFeeDiscount):discountParts.delivery,wht=Financial.money(o.platformWht),vat=Financial.money(o.platformVat),adsMarketing=Financial.money(o.platformAdsMarketing),marketingFee=Financial.money(o.platformMarketingFee),net=Financial.money(o.netPlatform!=null?o.netPlatform:gross-commission-discount-wht-vat-adsMarketing-marketingFee); return {orderId:entry.id,sourceNode:entry.node,platformRef:financeText(o.platformRef||o.id||entry.id,60),orderDate:financeText(o.date,10),orderedAt:Number(o.timestamp||0),gross,merchantPromo,deliveryFeeDiscount,platformDiscount:discount,commission,wht,vat,adsMarketing,marketingFee,expectedNet:net,schemaVersion:1}; });
    const payoutRecord = {channel, periodStart: financeText(data.periodStart, 10), periodEnd: financeText(data.periodEnd, 10), payoutDate, accountingOccurredAt:accountingTimestamp(payoutDate,settledAt), platformStatementReference:platformStatementReference||null, depositReference:depositReference||null, owing: owingCreated || null, owingOutstanding: owingCreated || 0, owingApplied: owingApplied || null, owingRecoveredSources: (owingApplied > 0 ? owingSources : null), expectedNet: expected, actualPayout: actual, variance, allocations, allocationRefs, allocationMeta, orderIds: ids, orderSnapshots, orderSnapshotTotal:Financial.money(orderSnapshots.reduce((sum,row)=>sum+row.expectedNet,0)), by: actor.role, actorUid: actor.uid, approvedBy: approval.record.approvedEmail || approval.record.approvedRole, approvalId: approval.id, settledAt, movementId: `payout_${payoutId}`, schemaVersion: 2};
...
const ACCAZA_AI_KNOWLEDGE_SEED={general:{title:"Accaza Coffee House system guide",content:"Accaza is the operational source of truth for POS, inventory and Finance Books. Accaza AI explains procedures and reports supplied facts only. It never writes, approves, posts or changes records."},pos:{title:"POS and sales controls",content:"The POS prices server-side. Completed sales require valid payment control and inventory/Finance Books evidence. Use the controlled correction or refund process for errors; do not edit posted sales directly. Shift close is a verified operational control."},finance:{title:"Finance Books controls",content:"Finance Books is double-entry. Cash, retained float, receivables, supplier payables and profit are distinct. Posted history is corrected through reversible controlled workflows with an audit trail, not direct changes."},inventory:{title:"Inventory and recipe costing",content:"Inventory quantity and value come from receiving, recipes, completed sales, internal usage and approved adjustments. Recipe cost is calculated from current cost inputs, while posted sales retain sale-time evidence."},suppliers:{title:"Suppliers and payables",content:"Supplier invoices remain the source evidence for Accounts Payable. Partial settlements reduce only the outstanding amount and retain invoice, payment, source account and audit evidence."},staff:{title:"Staff information boundary",content:"Management may use Accaza AI for operational roster and process guidance. Payroll, credentials and private employee details are excluded from AI responses."},operations:{title:"Operational safeguards",content:"Managers must investigate discrepancies and close exceptions through the relevant controlled workflow. Inventory, cash and Finance Books records must reconcile to their source evidence."},menu:{title:"Menu and customer ordering",content:"Menu availability, prices, recipes, options and packaging are maintained in Admin. Customer ordering shows only published menu items; internal costing and configuration remain management controls."}};
function accazaAiKnowledgeCategories(question){const text=accazaAiSearchText(question),categories=["general"];if(/\b(pos|cashier|sale|order|refund|shift|checkout)\b/.test(text))categories.push("pos");if(/\b(finance|cash|profit|revenue|ledger|journal|payable|receivable)\b/.test(text))categories.push("finance");if(/\b(inventory|stock|ingredient|recipe|cogs|wastage)\b/.test(text))categories.push("inventory");if(/\b(supplier|vendor|invoice|bill)\b/.test(text))categories.push("suppliers");if(/\b(staff|employee|barista|manager|roster)\b/.test(text))categories.push("staff");if(/\b(menu|coffee|drink|price|customer|option)\b/.test(text))categories.push("menu");if(categories.length===1)categories.push("operations");return[...new Set(categories)].slice(0,3);}
...
exports.manageAccazaAiKnowledge=onCall({region:ORDER_REGION,enforceAppCheck:ENFORCE_APP_CHECK,timeoutSeconds:60,memory:"256MiB"},async request=>{const db=getDatabase(),actor=await requirePortalUser(db,request);if(!["owner","superadmin","admin","manager"].includes(actor.role))throw new HttpsError("permission-denied","Accaza Knowledge Base is available to management accounts only.");const data=request.data||{},action=accazaAiText(data.action,20),now=Date.now();if(action==="seed"){const writes={};await Promise.all(Object.entries(ACCAZA_AI_KNOWLEDGE_SEED).map(async([category,article])=>{const existing=(await db.ref(`/accazaAiKnowledge/${category}`).get()).val();if(!existing)writes[`accazaAiKnowledge/${category}`]={...article,status:"published",version:1,createdAt:now,updatedAt:now,updatedBy:actor.uid,seeded:true};}));if(Object.keys(writes).length)await db.ref().update(Object.assign(writes,{[`operationalAudit/${now}_accaza_ai_knowledge_seed`]:operationalAuditRecord("seed_accaza_ai_knowledge","accazaAiKnowledge","seed",actor,{categories:Object.keys(writes).filter(key=>key.startsWith("accazaAiKnowledge/"))})}));return{seeded:Object.keys(writes).filter(key=>key.startsWith("accazaAiKnowledge/")).length};}const category=accazaAiText(data.category,40).toLowerCase();if(!Object.prototype.hasOwnProperty.call(ACCAZA_AI_KNOWLEDGE_SEED,category))throw new HttpsError("invalid-argument","Choose a valid Accaza knowledge category.");if(action==="get")return{category,article:(await db.ref(`/accazaAiKnowledge/${category}`).get()).val()||null};if(action!=="save")throw new HttpsError("invalid-argument","Use seed, get or save.");const title=accazaAiText(data.title,160),content=accazaAiText(data.content,12000);if(!title||content.length<20)throw new HttpsError("invalid-argument","A title and at least 20 characters of knowledge are required.");const current=(await db.ref(`/accazaAiKnowledge/${category}`).get()).val()||{},record={title,content,status:data.status==="draft"?"draft":"published",version:Number(current.version||0)+1,createdAt:Number(current.createdAt||now),updatedAt:now,updatedBy:actor.uid};await db.ref().update({[`accazaAiKnowledge/${category}`]:record,[`operationalAudit/${now}_accaza_ai_knowledge_${category}`]:operationalAuditRecord("save_accaza_ai_knowledge","accazaAiKnowledge",category,actor,{version:record.version,status:record.status,contentHash:crypto.createHash("sha256").update(content).digest("hex")})});return{category,article:record};});
