...
exports.legacyOwnerCapitalReset = onCall({region: ORDER_REGION, enforceAppCheck: ENFORCE_APP_CHECK, timeoutSeconds: 60, memory: "256MiB"}, async (request) => {
  const db=getDatabase(),actor=await requirePortalPermission(db,request,["cashflow","discrepancy"]),data=request.data||{},cutoffDate=financeDate(data.cutoffDate||data.date||financeDateFromTimestamp(Date.now())),date=financeDate(data.date||cutoffDate),reason=financeText(data.reason,500),id=`legacy_owner_capital_reset_v3_${cutoffDate}`;
  ...
  if(data.preview===true)return{preview:true,date,cutoffDate,balances,affectedDiscrepancies:affected.length,protectedDiscrepancies:protectedDiscrepancies.length,protectedDate,protectedShortage};if(!reason)throw new HttpsError("invalid-argument","A legacy cutover reason is required.");if(!lines.length&&!affected.length)throw new HttpsError("failed-precondition","There are no remaining legacy balances or discrepancies to close.");
  const now=Date.now(),writes={};affected.forEach((key)=>{writes[`discrepancies/${key}/status`]="legacy_closed";writes[`discrepancies/${key}/financialStatus`]="legacy_cleanup_closed";writes[`discrepancies/${key}/legacyCutover`]={movementId:id,cutoffDate,closedAt:now,closedBy:actor.uid,reason,protectedException:`${protectedDate} shortage ${protectedShortage.toFixed(2)} retained`};});writes[`operationalAudit/${now}_${id}`]=operationalAuditRecord("legacy_owner_capital_reset","legacyCutover",id,actor,{date,cutoffDate,balances,affectedDiscrepancies:affected.length,protectedDiscrepancies:protectedDiscrepancies.length,protectedDate,protectedShortage,reason});
