...
exports.postFinancialCommand = onCall(
  ...
  async (request) => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      if (advanceId) {const shifts=(await db.ref("/shifts").get()).val()||{};let found=null;for(const shiftId of Object.keys(shifts)){const rows=Array.isArray(shifts[shiftId].payOuts)?shifts[shiftId].payOuts:[];const index=rows.findIndex((row)=>row&&row.id===advanceId&&row.type==="purchase_advance");if(index>=0){found={shiftId,index,row:rows[index]};break;}}if(!found)throw new HttpsError("not-found","Purchase cash advance was not found.");const allocations=found.row.allocations||{};if(allocations[invoiceId])throw new HttpsError("already-exists","This purchase is already allocated to the selected cash advance.");const allocated=Financial.money(Object.values(allocations).reduce((sum,row)=>sum+Number(row&&row.amount||0),0)),remaining=Financial.money(Number(found.row.amount||0)-allocated);if(value>remaining+0.009)throw new HttpsError("failed-precondition",`Purchase exceeds the remaining cash advance of ${remaining}.`);const next=Financial.money(remaining-value);cashAsset=`asset:purchase_cash_advance:${advanceId}`;writes[`shifts/${found.shiftId}/payOuts/${found.index}/allocations/${invoiceId}`]={purchaseInvoiceId:invoiceId,amount:value,supplier:financeText(invoice.supplier,120),ref:financeText(invoice.ref,120),allocatedAt:now,allocatedBy:actor.uid};writes[`shifts/${found.shiftId}/payOuts/${found.index}/allocatedAmount`]=Financial.money(allocated+value);writes[`shifts/${found.shiftId}/payOuts/${found.index}/remainingAmount`]=next;writes[`shifts/${found.shiftId}/payOuts/${found.index}/status`]=next>0?"partially_allocated":"fully_allocated";if(!(next>0))writes[`shifts/${found.shiftId}/payOuts/${found.index}/completedAt`]=now;writes[`purchaseInvoices/${invoiceId}/purchaseAdvanceId`]=advanceId;writes[`purchaseInvoices/${invoiceId}/advanceShiftId`]=found.shiftId;}
