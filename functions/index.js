  ...
  async (request) => {
    ...
    if(row.kind==="cash"){
      ...
      if(treatment==="cash_recovered_to_undeposited"){
        ...
        const custodyRef=db.ref(`/cashCustody/${shiftId}`);let custodyDuplicate=false;
        ...
        if(!custodyResult.committed)throw new HttpsError("failed-precondition","The original shift custody record was not found. Restore the shift custody record before resolving this shortage.");
        const recoveryWrites=Object.assign({},approval.usedWrites,{[`discrepancies/${id}/status`]:"reviewed",[`discrepancies/${id}/financialStatus`]:"cash_recovered_to_undeposited",[`discrepancies/${id}/treatment`]:treatment,[`discrepancies/${id}/correctionMovementId`]:correctionKey,[`discrepancies/${id}/reviewedAt`]:now,[`discrepancies/${id}/reviewedBy`]:reviewedBy,[`discrepancies/${id}/reviewedByUid`]:approval.record.approvedBy,[`discrepancies/${id}/reviewApprovalId`]:approval.id,[`discrepancies/${id}/resolutionMovementId`]:correctionKey,[`discrepancies/${id}/note`]:note,[`shifts/${shiftId}/varianceStatus`]:"cash_recovered_to_undeposited",[`shifts/${shiftId}/varianceResolution`]:{treatment,note,correctionMovementId:correctionKey,approvalId:approval.id,approvedBy:reviewedBy,resolutionMovementId:correctionKey,resolvedAt:now},[`operationalAudit/${now}_${id}_cash_recovered`]:operationalAuditRecord("recover_shift_shortage_to_undeposited","discrepancy",id,actor,{shiftId,amount:value,correctionMovementId:correctionKey,approvalId:approval.id,custodyAdjusted:!custodyDuplicate,newFinancialMovement:false,note})});
        await db.ref().update(recoveryWrites);
        return{discrepancyId:id,reviewedAt:now,treatment,movementId:correctionKey,duplicate:custodyDuplicate,custodyAdjusted:!custodyDuplicate,financialMovementPosted:false};
...
exports.postFinancialCommand = onCall(
  ...
  async (request) => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      movement=Financial.movement("manual_books_journal","booksManualJournal",commandId,lines,{occurredAt,actorName:actor.role,reference,memo,linkedPayableId:prepared.linkedPayableId});
