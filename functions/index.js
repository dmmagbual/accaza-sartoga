    ...
    if(row.kind==="cash"&&Number(data.caseVersion)===2){
      ...
      for(const allocation of allocations){const d=allocation.details||{},v=allocation.amount,label=`${short?"Shortage":"Overage"} · ${allocation.treatment}`,record={id:allocation.id,treatment:allocation.treatment,amount:v,details:{},approvedAt:now,approvedBy:reviewedBy,approvalId:approval.id};
        if(allocation.treatment==="cash_recovered"){
          const destination=text(d.destination||"undeposited",160),asset=cashAccount(destination),requested=text(d.correctionMovementId,160);let linked=requested&&movements[requested]?requested:"";
...
exports.legacyOwnerCapitalReset = onCall({region: ORDER_REGION, enforceAppCheck: ENFORCE_APP_CHECK, timeoutSeconds: 60, memory: "256MiB"}, async (request) => {
  const db=getDatabase(),actor=await requirePortalPermission(db,request,["cashflow","discrepancy"]),data=request.data||{},cutoffDate=financeDate(data.cutoffDate||data.date||financeDateFromTimestamp(Date.now())),date=financeDate(data.date||cutoffDate),reason=financeText(data.reason,500),id=`legacy_owner_capital_reset_${cutoffDate}`;
  ...
  const normal={"6110":"debit","1190":"debit","2100":"credit"},balances={"6110":0,"1190":0,"2100":0};
  Object.values(journalSnap.val()||{}).forEach((entry)=>{if(!entry||entry.reversedByMovementId)return;const entryDate=String(entry.date||financeDateFromTimestamp(entry.occurredAt||entry.postedAt||0)).slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(entryDate)||entryDate>cutoffDate)return;(entry.lines||[]).forEach((line)=>{const code=String(line.code||"");if(!Object.prototype.hasOwnProperty.call(balances,code))return;balances[code]=Financial.money(balances[code]+(normal[code]==="debit"?(Number(line.debit)||0)-(Number(line.credit)||0):(Number(line.credit)||0)-(Number(line.debit)||0)));});});
  ...
  const affected=Object.entries(discrepancySnap.val()||{}).filter(([,row])=>{if(!row||row.status==="legacy_closed")return false;const rowDate=String(row.date||financeDateFromTimestamp(row.closedAt||row.ts||row.createdAt||0)).slice(0,10);return /^\d{4}-\d{2}-\d{2}$/.test(rowDate)&&rowDate<=cutoffDate;}).map(([key])=>key);
  if(data.preview===true)return{preview:true,date,cutoffDate,balances,affectedDiscrepancies:affected.length};if(!reason)throw new HttpsError("invalid-argument","A legacy cutover reason is required.");if(!lines.length)throw new HttpsError("failed-precondition","There are no historical 2100, 1190, or 6110 balances left to close through the selected date.");
  const now=Date.now(),writes={};affected.forEach((key)=>{writes[`discrepancies/${key}/status`]="legacy_closed";writes[`discrepancies/${key}/financialStatus`]="owner_capital_cutover";writes[`discrepancies/${key}/legacyCutover`]={movementId:id,cutoffDate,closedAt:now,closedBy:actor.uid,reason};});writes[`operationalAudit/${now}_${id}`]=operationalAuditRecord("legacy_owner_capital_reset","legacyCutover",id,actor,{date,cutoffDate,balances,affectedDiscrepancies:affected.length,reason});
...
const BOOKS_CHART_SEED_ROWS = [
  ...
  ["4000","Sales - In-store","Income"],["4010","Sales - Online (own)","Income"],["4020","Sales - GrabFood","Income"],["4030","Sales - FoodPanda","Income"],["4900","Discounts & Comps","Income","Contra-income (debit balance)"],["4910","Sales Returns & Refunds","Income","Existing void and refund adjustments"],["4990","Other Income","Income"],["4995","Inventory Reconciliation Gain","Income","System-generated adjustment when verified stock value exceeds Finance Books"],
  ["5000","COGS - Coffee & Beans","COGS"],["5010","COGS - Milk & Dairy","COGS"],["5020","COGS - Syrups & Flavors","COGS"],["5030","COGS - Food & Pastries","COGS"],["5040","COGS - Cups & Packaging","COGS"],["5090","Unposted COGS Clearing","COGS","Costs awaiting complete item-level posting"],["5900","Wastage & Spoilage","COGS"],["5905","Inventory Reconciliation Loss","COGS","System-generated adjustment when Finance Books exceeds verified stock value"],
...
exports.postFinancialCommand = onCall(
  ...
  async (request) => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      const rows=reconciliation.rows.filter((row)=>row.code!=="1290"&&Math.abs(Number(row.difference)||0)>=0.005),fingerprint=rows.map((row)=>`${row.code}_${Number(row.difference)<0?"n":"p"}${Math.round(Math.abs(Number(row.difference)||0)*100)}`).join("_")||"balanced",expectedFingerprint=financeText(data.expectedFingerprint,160);
      ...
      if(data.preview===true)return Object.assign({preview:true,fingerprint,adjustmentRows:rows},reconciliation);
      if(!rows.length)return Object.assign({alreadyReconciled:true,movementId:"",adjustment:0,fingerprint},reconciliation);
      ...
      writes[`inventoryReconciliations/adjustments/${movementId}`]={movementId,date,fingerprint,stockValue:reconciliation.totalStock,booksValueBefore:reconciliation.totalBooks,adjustmentRows:rows,postedAt:now,postedBy:actor.uid,postedRole:actor.role,schemaVersion:1};
