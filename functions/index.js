...
exports.onPettyVoucherFinancial = onValueWritten(
  ...
  async (event) => {const before = event.data.before.val() || {}, after = event.data.after.val(); if (!after) return; const db = getDatabase(), id = event.params.voucherId, value = Financial.money(after.amount), actor = {uid: "server", role: "server"}, isAdvance=after.transactionType==="purchase_advance", posting=revolvingFundPosting(after); if (after.status === "approved" && before.status !== "approved" && value > 0) {const movement = Financial.movement(isAdvance?"revolving_fund_purchase_advance":posting.movementType, "pettyVoucher", id, [Financial.line(isAdvance?`asset:purchase_cash_advance:${id}`:posting.account, value, 0, isAdvance?(after.recipient||"Supplier payment pending allocation"):posting.label), Financial.line("asset:petty_cash", 0, value, "Revolving Fund")], {occurredAt: Number(after.approvedAt || Date.now()), actorName: after.approvedBy || "Manager"}); await commitFinancial(db, `petty_${id}`, movement, actor);} if (after.voided === true && before.voided !== true && after.status === "approved" && value > 0) {const movement = Financial.movement(isAdvance?"revolving_fund_purchase_advance_void":posting.movementType+"_void", "pettyVoucher", id, [Financial.line("asset:petty_cash", value, 0, "Revolving Fund restored"), Financial.line(isAdvance?`asset:purchase_cash_advance:${id}`:posting.account, 0, value, isAdvance?"Reverse supplier payment":"Reverse "+posting.label)], {occurredAt: Number(after.voidedAt || Date.now()), actorName: "Manager"}); await commitFinancial(db, `petty_void_${id}`, movement, actor);}},
);
...
async function backfillPettyVoucher(db, id, row) {const value = Financial.money(row && row.amount), actor = {uid: "server", role: "server"}, isAdvance=row&&row.transactionType==="purchase_advance", posting=revolvingFundPosting(row); if (!row || row.status !== "approved" || !(value > 0)) return; const expense = Financial.movement(isAdvance?"revolving_fund_purchase_advance":posting.movementType, "pettyVoucher", id, [Financial.line(isAdvance?`asset:purchase_cash_advance:${id}`:posting.account, value, 0, isAdvance?(row.recipient||"Supplier payment pending allocation"):posting.label), Financial.line("asset:petty_cash", 0, value, "Revolving Fund")], {occurredAt: Number(row.approvedAt || row.createdAt || Date.now()), actorName: row.approvedBy || "Manager"}); await commitFinancial(db, `petty_${id}`, expense, actor); if (row.voided === true) {const reversal = Financial.movement(isAdvance?"revolving_fund_purchase_advance_void":posting.movementType+"_void", "pettyVoucher", id, [Financial.line("asset:petty_cash", value, 0, "Revolving Fund restored"), Financial.line(isAdvance?`asset:purchase_cash_advance:${id}`:posting.account, 0, value, isAdvance?"Reverse supplier payment":"Reverse "+posting.label)], {occurredAt: Number(row.voidedAt || Date.now()), actorName: "Manager"}); await commitFinancial(db, `petty_void_${id}`, reversal, actor);}}
