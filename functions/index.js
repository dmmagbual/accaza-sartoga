...
exports.reviewDiscrepancy = onCall(
  ...
  async (request) => {
    ...
    if(row.kind==="cash"){
      const treatment=financeText(data.treatment,40),short=Number(row.variance)<0,value=Financial.money(Math.abs(Number(row.variance)||0)),shiftId=financeKey(row.shiftId,"Shift ID"),allowed=short?["supplier_payment_pending_allocation","documented_cash_correction","shortage_expense","staff_receivable","owner_draw"]:["documented_cash_correction","overage_other_income"];
      if(!allowed.includes(treatment))throw new HttpsError("invalid-argument","Select a valid cash-variance treatment.");
      const pending=short?"asset:cash_shortage_pending":"liability:cash_overage_pending",correctionMovementId=financeText(data.correctionMovementId,160),recipient=financeText(data.recipient,160),purpose=financeText(data.purpose,300),reference=financeText(data.reference,120),originalMovement=(await db.ref(`/financialMovements/shift_variance_${shiftId}`).get()).val()||{},legacyFinal=originalMovement.type==="shift_cash_variance";let lines,target="",advanceId="";
      ...
      approval=await claimManagerApproval(db,data,"review_discrepancy",id,null,`review_discrepancy_${id}`);reviewedBy=approval.record.approvedName||approval.record.approvedEmail||approval.record.approvedRole;const writes=Object.assign({},approval.usedWrites,{[`discrepancies/${id}/status`]:"reviewed",[`discrepancies/${id}/financialStatus`]:treatment==="supplier_payment_pending_allocation"?"awaiting_inventory_allocation":"resolved",[`discrepancies/${id}/treatment`]:treatment,[`discrepancies/${id}/correctionMovementId`]:correctionMovementId||null,[`discrepancies/${id}/purchaseAdvanceId`]:advanceId||null,[`discrepancies/${id}/reviewedAt`]:now,[`discrepancies/${id}/reviewedBy`]:reviewedBy,[`discrepancies/${id}/reviewedByUid`]:approval.record.approvedBy,[`discrepancies/${id}/reviewApprovalId`]:approval.id,[`discrepancies/${id}/resolutionMovementId`]:movementId,[`discrepancies/${id}/note`]:note,[`shifts/${shiftId}/varianceStatus`]:"resolved",[`shifts/${shiftId}/varianceResolution`]:{treatment,note,correctionMovementId:correctionMovementId||null,purchaseAdvanceId:advanceId||null,approvalId:approval.id,approvedBy:reviewedBy,resolutionMovementId:movementId,resolvedAt:now}});if(advanceId){const shift=(await db.ref(`/shifts/${shiftId}`).get()).val()||{},payOuts=Array.isArray(shift.payOuts)?shift.payOuts:[];writes[`shifts/${shiftId}/payOuts/${payOuts.length}`]={id:advanceId,type:"purchase_advance",status:"pending_details",amount:value,remainingAmount:value,recipient,purpose,reference,reason:`Emergency supplier payment — ${purpose}`,by:reviewedBy,approvalId:approval.id,ts:now,source:"closed_shift_variance"};}
