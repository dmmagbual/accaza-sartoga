...
exports.settlePlatformPayout = onCall(
  ...
  async (request) => {
    ...
    const writes = Object.assign({}, approval.usedWrites), settledAt = Date.now(); found.forEach((entry) => {writes[`${entry.node}/${entry.id}/settlementStatus`] = "settled"; writes[`${entry.node}/${entry.id}/payoutId`] = payoutId;}); writes[`platformPayouts/${payoutId}`] = {channel, periodStart: financeText(data.periodStart, 10), periodEnd: financeText(data.periodEnd, 10), payoutDate: /^\d{4}-\d{2}-\d{2}$/.test(financeText(data.payoutDate, 10)) ? financeText(data.payoutDate, 10) : null, expectedNet: expected, actualPayout: actual, variance, allocations, orderIds: ids, by: actor.role, actorUid: actor.uid, approvedBy: approval.record.approvedEmail || approval.record.approvedRole, approvalId: approval.id, settledAt, movementId: `payout_${payoutId}`, schemaVersion: 1};
