...
exports.legacyOwnerCapitalReset = onCall({region: ORDER_REGION, enforceAppCheck: ENFORCE_APP_CHECK, timeoutSeconds: 60, memory: "256MiB"}, async (request) => {
  const db=getDatabase(),actor=await requirePortalPermission(db,request,["cashflow","discrepancy"]),data=request.data||{},cutoffDate=financeDate(data.cutoffDate||data.date||financeDateFromTimestamp(Date.now())),date=financeDate(data.date||cutoffDate),reason=financeText(data.reason,500),id=`legacy_owner_capital_reset_v4_${cutoffDate}`;
  ...
  Object.entries(journal).forEach(([key,entry])=>{if(!entry||!entry.reversalOf)return;const original=journalById.get(String(entry.reversalOf)),entryDate=String(entry.date||financeDateFromTimestamp(entry.occurredAt||entry.postedAt||0)).slice(0,10),originalDate=String(original&&(original.date||financeDateFromTimestamp(original.occurredAt||original.postedAt||0))||"").slice(0,10);if(entryDate<augustStart||entryDate>cutoffDate||!originalDate||originalDate>=augustStart)return;let included=false;(entry.lines||[]).forEach((line)=>{const code=String(line.code||"");if(!Object.prototype.hasOwnProperty.call(crossPeriodReversals,code))return;crossPeriodReversals[code]=Financial.money(crossPeriodReversals[code]+(Number(line.debit)||0)-(Number(line.credit)||0));included=true;});if(included)crossPeriodSources.push({reversalId:key,originalId:String(entry.reversalOf),reversalDate:entryDate,originalDate});});
  ...
  Object.keys(crossPeriodReversals).forEach((code)=>{const net=Financial.money(crossPeriodReversals[code]);if(Math.abs(net)<.005)return;const value=Math.abs(net);lines.push(Financial.line(`coa:${code}`,net<0?value:0,net>0?value:0,"Reclassify pre-August reversal from August activity"));lines.push(Financial.line("equity:owner_capital",net>0?value:0,net<0?value:0,"Pre-August reversal reclassified to opening equity"));});
  ...
  if(data.preview===true)return{preview:true,date,cutoffDate,balances,crossPeriodReversals,crossPeriodSources,affectedDiscrepancies:affected.length,protectedDiscrepancies:protectedDiscrepancies.length,protectedDate,protectedShortage};if(!reason)throw new HttpsError("invalid-argument","A legacy cutover reason is required.");if(!lines.length&&!affected.length)throw new HttpsError("failed-precondition","There are no remaining legacy balances, cross-period reversals, or discrepancies to close.");
