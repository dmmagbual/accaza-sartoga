...
exports.postFinancialCommand = onCall(
  ...
  async (request) => observeFinancialOperation(request, "postFinancialCommand", async () => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      const supplier = await requireActiveSupplier(db, data.supplierId, data.party), invoiceReference = financeText(data.invoiceReference || data.ref, 120).trim(), statementReference = financeText(data.statementReference, 120).trim();
      if (!invoiceReference) throw new HttpsError("invalid-argument", "The supplier invoice reference is required.");
      if (!statementReference) throw new HttpsError("invalid-argument", "The supplier statement or opening-balance evidence reference is required.");
      ...
      movement = Financial.movement("supplier_opening_balance", "payable", docId, [Financial.line("equity:opening_balance", value, 0, `Opening payable · ${supplier.name}`), Financial.line(`liability:payable:${docId}`, 0, value, `Opening payable · ${supplier.name}`)], {occurredAt: accountingTimestamp(openingDate, now), actorName: actor.role, reference: invoiceReference, supplierId: supplier.id, supplierName: supplier.name, statementReference, originalInvoiceDate: invoiceDate, originalInvoiceAmount: originalAmount, paidBeforeOpening});
      writes[`payables/${docId}`] = {supplierId: supplier.id, party: supplier.name, type: "opening balance", amount: value, remainingAmount: value, originalInvoiceAmount: originalAmount, paidBeforeOpening, date: invoiceDate, openingDate, due, ref: invoiceReference, statementReference, status: "open", openingBalance: true, movementId: postingId, liabilityAccount: `liability:payable:${docId}`, reversalOffsetAccount: "equity:opening_balance", ts: now, createdBy: actor.uid, schemaVersion: 3};
