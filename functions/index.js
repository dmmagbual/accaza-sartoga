...
exports.reviewDiscrepancy = onCall(
  ...
  async (request) => {
    ...
    if(row.kind==="cash"&&Number(data.caseVersion)===2){
      ...
      const allowed=short?new Set(["cash_recovered","business_expense","supplier_purchase","staff_receivable","owner_draw","counting_error"]):new Set(["customer_refund","unrecorded_sale","capital_contribution","supplier_refund","unrecorded_cash_in","unexplained_overage","counting_error"]),allocations=[],seen=new Set();let batchTotal=0;
      ...
      for(const allocation of allocations){const d=allocation.details||{},v=allocation.amount,label=`${short?"Shortage":"Overage"} · ${allocation.treatment}`,record={id:allocation.id,treatment:allocation.treatment,amount:v,details:{},approvedAt:now,approvedBy:reviewedBy,approvalId:approval.id};
        ...
        allocationRecords[allocation.id]=record;writes[`cashDifferenceCases/${id}/allocations/${allocation.id}`]=record;
      }
...
async function ensureBooksChart(db){const seed=booksChartSeed();const snap=await db.ref("/booksChart").get();const current=snap.val()||{};const writes={};Object.keys(seed).forEach(function(code){if(!current[code])writes[`booksChart/${code}`]=Object.assign({},seed[code],{createdAt:Date.now(),schemaVersion:1});});if(current["2100"]&&current["2100"].system!==false&&String(current["2100"].name||"")==="Accrued Salaries"){writes["booksChart/2100/name"]="Cash Overage Under Review";writes["booksChart/2100/note"]="Pending manager reconciliation";current["2100"]=Object.assign({},current["2100"],{name:"Cash Overage Under Review",note:"Pending manager reconciliation"});}if(Object.keys(writes).length)await db.ref().update(writes);return Object.assign({},seed,current);}
