...
exports.postFinancialCommand = onCall(
  ...
  async (request) => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      const requestedAccount = financeText(data.accountId, 120), fromRegister = requestedAccount === "register", accountId = fromRegister ? "register" : accountIdFor(accounts, requestedAccount), value = amount(invoice.total), date = financeDate(data.date), split = await purchaseInventoryLines(db, invoice, false), cashAsset = fromRegister ? "asset:register_cash" : `asset:cash_account:${accountId}`;
