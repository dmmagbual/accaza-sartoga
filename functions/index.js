  ...
  async (request) => {
    ...
    const result = await vouchersRef.transaction((all) => {
      ...
      if (action === "approve") {
        ...
        if (value > available + 0.009) {failure = `Voucher exceeds the available Revolving Fund (${available.toFixed(2)}).`; return;}
...
exports.onPettyVoucherFinancial = onValueWritten(
  ...
  async (event) => {const before = event.data.before.val() || {}, after = event.data.after.val(); if (!after) return; const db = getDatabase(), id = event.params.voucherId, value = Financial.money(after.amount), actor = {uid: "server", role: "server"}, isAdvance=after.transactionType==="purchase_advance"; if (after.status === "approved" && before.status !== "approved" && value > 0) {const movement = Financial.movement(isAdvance?"revolving_fund_purchase_advance":"petty_cash_expense", "pettyVoucher", id, [Financial.line(isAdvance?`asset:purchase_cash_advance:${id}`:`expense:petty:${financeText(after.category || "other", 60)}`, value, 0, isAdvance?(after.recipient||"Purchase advance"):after.category), Financial.line("asset:petty_cash", 0, value, "Revolving Fund")], {occurredAt: Number(after.approvedAt || Date.now()), actorName: after.approvedBy || "Manager"}); await commitFinancial(db, `petty_${id}`, movement, actor);} if (after.voided === true && before.voided !== true && after.status === "approved" && value > 0) {const movement = Financial.movement(isAdvance?"revolving_fund_purchase_advance_void":"petty_cash_void", "pettyVoucher", id, [Financial.line("asset:petty_cash", value, 0, "Revolving Fund restored"), Financial.line(isAdvance?`asset:purchase_cash_advance:${id}`:`expense:petty:${financeText(after.category || "other", 60)}`, 0, value, isAdvance?"Reverse purchase advance":"Reverse Revolving Fund expense")], {occurredAt: Number(after.voidedAt || Date.now()), actorName: "Manager"}); await commitFinancial(db, `petty_void_${id}`, movement, actor);}},
);
...
async function backfillPettyVoucher(db, id, row) {const value = Financial.money(row && row.amount), actor = {uid: "server", role: "server"}, isAdvance=row&&row.transactionType==="purchase_advance"; if (!row || row.status !== "approved" || !(value > 0)) return; const expense = Financial.movement(isAdvance?"revolving_fund_purchase_advance":"petty_cash_expense", "pettyVoucher", id, [Financial.line(isAdvance?`asset:purchase_cash_advance:${id}`:`expense:petty:${financeText(row.category || "other", 60)}`, value, 0, isAdvance?(row.recipient||"Purchase advance"):row.category), Financial.line("asset:petty_cash", 0, value, "Revolving Fund")], {occurredAt: Number(row.approvedAt || row.createdAt || Date.now()), actorName: row.approvedBy || "Manager"}); await commitFinancial(db, `petty_${id}`, expense, actor); if (row.voided === true) {const reversal = Financial.movement(isAdvance?"revolving_fund_purchase_advance_void":"petty_cash_void", "pettyVoucher", id, [Financial.line("asset:petty_cash", value, 0, "Revolving Fund restored"), Financial.line(isAdvance?`asset:purchase_cash_advance:${id}`:`expense:petty:${financeText(row.category || "other", 60)}`, 0, value, isAdvance?"Reverse purchase advance":"Reverse Revolving Fund expense")], {occurredAt: Number(row.voidedAt || Date.now()), actorName: "Manager"}); await commitFinancial(db, `petty_void_${id}`, reversal, actor);}}
...
exports.postFinancialCommand = onCall(
  ...
  async (request) => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      const requestedAccount = financeText(data.accountId, 120), advanceId = data.advanceId ? financeKey(data.advanceId, "Purchase advance ID") : "", fromRegister = requestedAccount === "register" || !!advanceId, accountId = advanceId ? "" : (fromRegister ? "register" : accountIdFor(accounts, requestedAccount)), value = amount(invoice.total), date = financeDate(data.date), split = await purchaseInventoryLines(db, invoice, false); let cashAsset = fromRegister ? "asset:register_cash" : `asset:cash_account:${accountId}`;
      ...
      movement = Financial.movement("purchase_cash", "purchaseInvoice", invoiceId, split.concat([Financial.line(cashAsset, 0, value, invoice.supplier || "Inventory purchase")]), {occurredAt: Date.parse(`${date}T00:00:00+08:00`) || now, actorName: actor.role});
      if (!fromRegister) addCash(`fm_${commandId}`, {date, accountId, dir:"out", category:"Inventory purchase", amount:value, party:invoice.supplier, ref:invoice.ref, auto:true});
...
exports.settlePlatformPayout = onCall(
  ...
  async (request) => {
    ...
    const actual = Financial.money(data.actualPayout); const approval = await claimManagerApproval(db, data, "settle_platform_payout", payoutId, actual, `payout_${payoutId}`), variance = Financial.money(actual - expected), defs = (await db.ref("/platformVarAccounts").get()).val() || {}, allocations = data.allocations || {}; const _allPo = (await db.ref("/platformPayouts").get()).val() || {}; let outstandingOwing = 0; const owingSources = []; Object.keys(_allPo).forEach((k) => { const po = _allPo[k] || {}; if (po.channel === channel && !po.reversed && Financial.money(po.owingOutstanding) > 0.009) { outstandingOwing = Financial.money(outstandingOwing + Financial.money(po.owingOutstanding)); owingSources.push(k); } });
