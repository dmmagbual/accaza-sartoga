...
exports.legacyOwnerCapitalReset = onCall({region: ORDER_REGION, enforceAppCheck: ENFORCE_APP_CHECK, timeoutSeconds: 60, memory: "256MiB"}, async (request) => {
  const db=getDatabase(),actor=await requirePortalPermission(db,request,["cashflow","discrepancy"]),data=request.data||{},cutoffDate=financeDate(data.cutoffDate||data.date||financeDateFromTimestamp(Date.now())),date=financeDate(data.date||cutoffDate),reason=financeText(data.reason,500),id=`legacy_owner_capital_reset_v2_${cutoffDate}`;
  ...
  if(data.preview===true)return{preview:true,date,cutoffDate,balances,affectedDiscrepancies:affected.length};if(!reason)throw new HttpsError("invalid-argument","A legacy cutover reason is required.");if(!lines.length)throw new HttpsError("failed-precondition","There are no historical 4990, 2100, 1190, or 6110 balances left to close through the selected date.");
