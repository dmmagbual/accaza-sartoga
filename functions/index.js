...
exports.saveSharedChoiceIngredients = onCall(
  ...
  async (request) => {
    ...
    const [inventorySnap,groupsSnap]=await Promise.all([db.ref("/inventory").get(),db.ref("/optionGroups").get()]),inventory=inventorySnap.val()||{},groups=groupsSnap.val()||{},temperatureId=Object.keys(groups).find((id)=>String(groups[id]&&groups[id].name||"").toLowerCase()==="temperature"),temperatureLabels=temperatureId?(groups[temperatureId].choices||[]).map((choice)=>String(choice&&choice.label||"")):[];let rows=0;
    for (const groupId of Object.keys(optionCosts)) {const group=groups[groupId],choices=optionCosts[groupId];if(!group||!choices||typeof choices!=="object"||Array.isArray(choices))throw new HttpsError("invalid-argument",`Choice group ${groupId} is invalid.`);for(const key of Object.keys(choices)){const entry=choices[key],lines=entry&&entry.ings,coverage={};if(!entry||!Array.isArray(lines)||!lines.length||lines.length>20)throw new HttpsError("invalid-argument",`Shared choice ${key} has invalid ingredient rows.`);for(const line of lines){rows++;const ing=String(line&&line.ing||""),when=line&&line.when||{},scopeIds=Object.keys(when);if(!inventory[ing])throw new HttpsError("invalid-argument",`Shared choice ingredient ${ing||"(blank)"} is missing from Inventory.`);if(scopeIds.some((id)=>id!==temperatureId)||scopeIds.some((id)=>!temperatureLabels.includes(String(when[id]))))throw new HttpsError("invalid-argument",`Shared choice ${key} has an invalid temperature assignment.`);for(const label of when[temperatureId]?[String(when[temperatureId])]:temperatureLabels){coverage[ing]=coverage[ing]||{};if(coverage[ing][label])throw new HttpsError("invalid-argument",`Shared choice ${key} assigns ${ing} to ${label} more than once.`);coverage[ing][label]=true;}for(const size of ["S","M","L"]){const qty=Number(line[`qty${size}`]);if(!Number.isFinite(qty)||qty<0||qty>1000000)throw new HttpsError("invalid-argument",`Shared choice ${key} has an invalid ${size} quantity.`);}}}}
