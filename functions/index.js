...
exports.repairOrderInventoryMarker = onCall(
  ...
  async (request) => {
    ...
    const usage=costing.usage||{},expectedIds=Object.keys(usage).sort(),movements=movementSnap.val()||{},actualIds=Object.keys(movements).sort();if(!expectedIds.length||expectedIds.length!==actualIds.length)throw new HttpsError("failed-precondition","Order inventory movements are incomplete; no marker was restored.");
...
exports.reviewDiscrepancy = onCall(
  ...
  async (request) => {
    ...
    if(row.kind==="cash"){
      ...
      if(treatment==="supplier_payment_pending_allocation"){
