...
exports.postFinancialCommand = onCall(
  ...
  async (request) => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      const value = amount(invoice.total), date = financeDate(data.date || invoice.date), split = await purchaseInventoryLines(db, invoice, false);
      movement = Financial.movement("purchase_owner_funded", "purchaseInvoice", invoiceId, split.concat([Financial.line("equity:capital_in", 0, value, "Paid personally by owner/partner")]), {occurredAt: Date.parse(`${date}T00:00:00+08:00`) || now, actorName: actor.role});
...
exports.managePurchaseCorrection = onCall(
  ...
  async (request) => {
    ...
    if (invoice.payMode === "owner_funded") {
      const ownerReversalLines=await purchaseInventoryLines(db,invoice,true),financialId=`purchase_owner_reversal_${invoiceId}`;
      const ownerReversal=Financial.movement("purchase_owner_funded_reversed","purchaseInvoice",invoiceId,[Financial.line("equity:capital_in",safeInvoice.total,0,"Reverse owner/partner-funded purchase")].concat(ownerReversalLines),{occurredAt:now,actorName:actor.role});
