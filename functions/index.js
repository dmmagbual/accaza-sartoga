...
exports.postFinancialCommand = onCall(
  ...
  async (request) => observeFinancialOperation(request, "postFinancialCommand", async () => {
    ...
    if (action === "inventory_opening_balance") {
      ...
      if(advanceId){const supplierAdvance=(await db.ref(`/pettyCashVouchers/${advanceId}`).get()).val();if(supplierAdvance){const advanceSupplier=financeText(supplierAdvance.supplierName||supplierAdvance.recipient,120).trim().toLowerCase(),invoiceSupplier=financeText(invoice.supplier,120).trim().toLowerCase();if(!advanceSupplier||advanceSupplier!==invoiceSupplier)throw new HttpsError("failed-precondition","The selected advance payment belongs to a different supplier. Select an advance issued to this purchase supplier.");}}
