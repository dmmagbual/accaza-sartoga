...
const context={
  window:{
    ...
    __financialMovements:{
      opening:{id:'opening',type:'opening_balance',sourceType:'cashAccount',sourceId:'security458',occurredAt:stamp('2026-09-01'),lines:[line('asset:cash_account:security458',13050,0),line('equity:opening_balance',0,13050)]},
...
assert.equal(result.begin.security458||0,0,'cash introduced on the From date must not be reported as prior-day opening cash');
assert.equal(result.add['Opening funds introduced'],13050,'same-day opening funds must be disclosed as a period inflow');
...
for(const marker of ['Opening cash · before','Cash received from outside the business','Cash paid outside the business','Banking actions · excluded from cash flow','internal transfer—not income'])assert.ok(page.includes(marker),`missing cash-flow presentation marker: ${marker}`);
