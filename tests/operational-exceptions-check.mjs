...
const result=buildOperationalExceptions({
  ...
  orders:{bad:{status:'Completed',timestamp:now-hour,paymentStatus:'confirmed'},good:{status:'Received',timestamp:now-hour,paymentStatus:'confirmed',inventoryDeducted:true,inventoryLedgerVersion:1}},
...
for(const expected of ['stuck_order','offline_sync','inventory_gap','financial_gap','cash_custody','payment_proof','client_error'])if(!cats.includes(expected))throw new Error('missing exception '+expected);
if(result.counts.critical!==5||result.counts.warning!==2)throw new Error('severity counts are incorrect');
