...
assert.equal(B.mapAccount('asset:cash_awaiting_deposit').code,'1001','the renamed Undeposited Collection account must retain its internal ledger identity');
assert.equal(B.mapAccount('asset:register_cash').code,'1000','the cash-register side of the Admin transfer remains on its existing internal account');
