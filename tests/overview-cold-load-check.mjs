...
if(!overview.includes('api.getOperationalExceptions(force)')||!core.includes('getOperationalExceptions:function(force)')||!core.includes('{force:force===true,cached:true}'))throw new Error('Manual System Health must be the only client path that requests a forced server scan.');
...
if(!core.includes('readHistoricalSalesRollup')||core.includes('subscriptionHub.readHistoricalPeriod('))throw new Error('Admin Overview must use compact historical summaries and must not page archived orders for a dashboard view.');
