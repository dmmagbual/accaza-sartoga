...
for(const marker of ["button.textContent='Saving recipe…'","button.setAttribute('aria-busy','true')","return a.validateRecipeDefinition(raw)","button.removeAttribute('aria-busy')","button.textContent='Saving choices…'","a.saveSharedChoiceIngredients(clean)","Save not confirmed","},15000)","saved after the connection recovered","window.__posSettings.optionCosts=clean"]){
