...
ok(health.includes("accaza_pos_device_id")&&health.includes('oldestUnsyncedAt')&&health.includes('deviceId:deviceId')&&health.includes('},60000)')&&client.includes('AccazaPosSyncHealth.report'),'Cashier heartbeat does not retain device identity, queue age, or bounded cadence');
ok(source.includes('posDeviceHealth/${shiftId}/${deviceId}')&&source.includes('outstandingDevices')&&source.includes('health.reportedBy!==actor.uid'),'Multi-device health must not let one clean browser overwrite or impersonate another device');
ok((close.match(/await continuityReadyForClose\(\)/g)||[]).length===2&&close.includes('deviceId:health.deviceId()'),'Shift close must verify the calling device plus every reported device before counting and before final save');
