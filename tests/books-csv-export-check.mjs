...
for(const marker of ["App.printLedger('","App.exportAccountLedgerCsv('",'accountLedgerDetail(code)','printFinancePage(title)','viewEntry(id)','App.viewEntry(\'${esc(e.id)}\')','Balanced double-entry journal.'])if(!shell.includes(marker))throw new Error(`Ledger drill-down action missing: ${marker}`);
if(shell.includes('${entryLink}${esc(e.reference)}'))throw new Error('Subsidiary ledger reference must stay plain text; only the date opens the original journal');
const booksCss=fs.readFileSync('assets/css/books.css','utf8');for(const marker of ['.ledger-entry-link{background:none;border:0;padding:0','white-space:nowrap','.journal-entry-lines th:nth-child(1){width:auto}'])if(!booksCss.includes(marker))throw new Error(`Ledger date-link styling safeguard missing: ${marker}`);
for(const marker of ['<th>Date</th><th>Reference</th><th>Description</th><th class="num">Debit</th><th class="num">Credit</th><th class="num">Balance</th>','AccazaReportPagination.apply(m,\'ledger-\'+code,true)','Latest balance'])if(!shell.includes(marker))throw new Error(`Subsidiary ledger presentation safeguard missing: ${marker}`);
if(!pages.includes('<th>Date</th><th>Reference</th><th>Description</th><th class="num">Debit</th><th class="num">Credit</th><th class="num">Balance</th>'))throw new Error('Journal must expose all six accounting columns without a horizontal-scroll-only balance');
...
const entries=[{id:'before',date:'2026-07-31',lines:[{code:'1000',debit:100},{code:'4000',credit:100}]},{id:'sale',date:'2026-08-01',ref:'=SUM(1,2)',lines:[{code:'1000',debit:260},{code:'4000',credit:260}]},{id:'compound',date:'2026-08-02',ref:'C-2',memo:'Compound correction',lines:[{code:'1000',debit:40},{code:'1000',credit:10},{code:'4000',credit:30}]}];
...
ctx.exportLedgerCsv();
assert.equal(result.rows.find(r=>r[1]==='1000'&&r[10]==='Closing')[9],390);
assert.equal(result.rows.find(r=>r[1]==='4000'&&r[10]==='Closing')[9],290);
ctx.exportJournalCsv();
assert.equal(result.rows.length,5);
assert.equal(result.rows[0][1],'compound');
assert.equal(result.rows[0][8],40);
...
assert.equal(result.rows.at(-1)[9],390);
assert.equal(result.rows[1][4],'compound');
assert.equal(result.rows[1][7],40);
