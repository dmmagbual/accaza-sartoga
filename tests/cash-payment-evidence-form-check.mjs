...
function assertEvidenceFieldsOutsideSupplierWrapper(file){
  ...
  if(!source.includes('purpose:purpose')||!source.includes('hasReceipt:!!img')||!source.includes("if(img)writes['pettyCashReceipts/'+id]={meta:{createdAt:createdAt,bytes:img.length},image:img};"))fail(file+': voucher evidence is not retained with the operational record');
}
