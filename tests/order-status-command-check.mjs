...
assert.equal(db.data.orders.two.status,'Confirmed');assert.equal(db.data.orderStatusCommands.req_two.status,'failed');
const recovered=await run(db,{orderId:'two',status:'Confirmed',expectedStatus:'Pending',requestId:'req_two'});
assert.equal(recovered.duplicate,true);assert.equal(db.data.orderStatusCommands.req_two.status,'applied');
...
console.log('PASS: order-status command validates transitions and stale state, records one audit, resumes partial failure, and is idempotent.');
