...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=323')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=323"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ['va_refund_recovery','allocationRefs','Grab refund deductions and recoveries require the original order or statement reference'])if(!functionsSource.includes(marker)&&!analyticsSource.includes(marker))fail(`Grab refund recovery control missing: ${marker}`);
  ...
  for(const marker of ["saleshistory:['orders','archivedOrders','financialMovements']","cfAccounts:['purchases','cashflow','receivables','payables','undeposited']","'books/journal':['stockvalue']","financialMovements:['purchases','cashflow','receivables','payables','payouts','saleshistory','undeposited']"])if(!realtimeHubSource.includes(marker))fail(`Finance subscription scope missing: ${marker}`);
  for(const marker of ["function currentCashBalance(id)","id:'register',name:'Register cash'","a.get(a.ref(a.db,'financialMovements'))","accaza:cash-balances-updated"])if(!financeSource.includes(marker)&&!adminSource.includes(marker))fail(`Purchase current-cash balance marker missing: ${marker}`);
  for(const marker of ['requestedAccount === "register"','cashAsset = fromRegister ? "asset:register_cash"','if (!fromRegister) addCash'])if(!functionsSource.includes(marker))fail(`Purchase register-cash posting marker missing: ${marker}`);
  ...
  if(!salesHistorySource.includes("['expense:platform_discount','expense:customer_discount']")||salesHistorySource.includes("['expense:platform_discount','expense:platform_merchant_funded_promo','expense:platform_delivery_fee_discount','expense:customer_discount']"))fail('Sales reconciliation must not treat merchant promos or delivery-fee discounts as net-sales deductions');
  for(const marker of ['a.unpostedNet+=v.net','difference=Math.round((orphanNet-tot.unpostedNet)*100)/100','tot.unposted===0&&Math.abs(orphanNet)<.005'])if(!salesHistorySource.includes(marker))fail(`Sales reconciliation must compare source-linked missing and orphan postings: ${marker}`);
  ...
  for(const marker of ['sourceRef.replace(/[.#$\\/\\[\\]]/g, "_")','sourceRef, sources'])if(!functionsSource.includes(marker))fail(`Register cash-float Firebase-safe source key missing: ${marker}`);
  ...
  for(const marker of ['Browser entries are retained only as a recovery backup','function ENTRIES(){ return (window.__posEntries||[]).slice(); }','action:"manual_journal"','action:"reverse_manual_journal"'])if(!booksHtml.includes(marker))fail(`Shared server-authoritative manual Books journal missing: ${marker}`);
  ...
  for(const marker of ["pettyCashVouchers:['petty','purchases','undeposited']","cfAccounts:['purchases','cashflow','receivables','payables','undeposited']","financialMovements:['purchases','cashflow','receivables','payables','payouts','saleshistory','undeposited']","cashCustody:['cashflow','undeposited']"])if(!realtimeHubSource.includes(marker))fail(`Undeposited realtime scope missing: ${marker}`);
