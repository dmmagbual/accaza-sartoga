...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=380')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=380"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ['function entriesThroughPeriodEnd()','accountNet(a.code, entriesThroughPeriodEnd())','const ents = entriesThroughPeriodEnd()','opening balance ${peso(normalBalanceFor'])if(!booksHtml.includes(marker))fail(`Cumulative balance-report safeguard missing: ${marker}`);
