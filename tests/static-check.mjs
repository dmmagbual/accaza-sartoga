...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=373')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=373"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ['Amount mismatches: ','tot.mismatched===0','financeNet-tot.net','Admin '+"'+money(v.net)+'"+' · Finance '])if(!salesHistorySource.includes(marker))fail(`Sales amount-level reconciliation safeguard missing: ${marker}`);
