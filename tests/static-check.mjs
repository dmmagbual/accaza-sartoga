...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  for(const marker of ['historyComplete:false','entry.historyComplete?false','if(entry.historyComplete)return','entry.historyComplete=!hasOlder'])if(!realtimeHubSource.includes(marker))fail(`Complete Sales History session cache marker missing: ${marker}`);
  for(const marker of ['id="shReconcile"','Reconcile now','ensureBooksJournal','resetHistory'])if(!salesHistorySource.includes(marker)&&!coreSource.includes(marker)&&!realtimeHubSource.includes(marker))fail(`Sales History corrective action marker missing: ${marker}`);
  ...
  for(const marker of ['ledger[id]||ledger[k]','function knownOrderIds()','orderIdentityIds.has(sourceId)','orphan_order_restoration','orphanRestored'])if(!salesHistorySource.includes(marker)&&!functionsSource.includes(marker))fail(`Sales reconciliation identity marker missing: ${marker}`);
  ...
  for(const marker of ['const salesCodes = new Set(["4000","4010","4020","4030","4900","4910"])','Completed sales less refunds and voids','ensureFinancialLedger','net sales: '])if(!booksHtml.includes(marker))fail(`Books sales reconciliation marker missing: ${marker}`);
