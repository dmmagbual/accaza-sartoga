...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!customerSource.includes("button.textContent=open?'Place Order':'Online Orders Closed'")||!customerHtml.includes('disabled aria-disabled="true">Checking order availability'))fail('Place Order is not safely disabled while online ordering is closed or unknown');
  ...
  if(!posSource.includes("function verifyOnlinePayment(oid,button)")||!posSource.includes("a.managerApproval('confirm_payment'")||!posSource.includes("verifyOnlinePayment(b.getAttribute('data-online-verify'),b)"))fail('POS payment verification is not self-contained');
