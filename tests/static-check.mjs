...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=318')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=318"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ['PAID PERSONALLY BY OWNER/PARTNER','value="owner_funded"','action:\'purchase_owner_funded\'','Paid personally by owner/partner · capital contributed'])if(!adminSource.includes(marker))fail(`Owner/partner-funded purchase UI marker missing: ${marker}`);
  for(const marker of ['action === "purchase_owner_funded"','invoice.payMode !== "owner_funded"','Financial.line("equity:capital_in", 0, value','purchase_owner_funded_reversed','Financial.line("equity:capital_in",safeInvoice.total,0','fundingReversalMovementId'])if(!functionsSource.includes(marker))fail(`Owner/partner-funded purchase Finance Books marker missing: ${marker}`);
