...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  for(const marker of ["value=\"expense\"","expenseDescription","expenseAccount:'6075'","lineType:'expense'","no inventory created","one-time expense lines posted only to Finance Books"])if(!posSource.includes(marker))fail(`One-time purchase expense control missing: ${marker}`);
  ...
  for(const marker of ["function currentCashBalance(id)","function refreshCashBalances(forceAfterCurrent)","if(tab==='purchases')refreshCashBalances()","id:'cash_on_hand',name:'Cash on Hand'","id:'undeposited',name:'Undeposited Collection'","id:'cash_float',name:'Register Cash Float'","disabled:true","a.get(a.ref(a.db,'financialMovements'))","accaza:cash-balances-updated"])if(!financeSource.includes(marker)&&!adminSource.includes(marker))fail(`Purchase Balance Sheet cash-account marker missing: ${marker}`);
  for(const marker of ["requestedAccount === \"register\" || requestedAccount === \"cash_float\"","savedAccount !== requestedAccount","paymentMovementId","paymentAccountId","fromUndeposited","poolCustodyOutflow(db, value)","custodyAllocations","Purchase exceeds available Undeposited Collection","lastPaymentReversalMovementId"])if(!functionsSource.includes(marker))fail(`Purchase cash disbursement safeguard missing: ${marker}`);
  for(const marker of ["Balances match the Finance Books Balance Sheet","Undeposited Collection posts a disbursement","Cash Float is view-only","x.disabled?' disabled'","availableAccounts","purchasePaymentAccountLabel","Paid · "])if(!posSource.includes(marker))fail(`Purchase cash-account selector marker missing: ${marker}`);
