...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminSource.includes("value:'cash_recovered_to_undeposited'")||!adminSource.includes('without posting a second journal'))fail('Recovered-cash discrepancy workflow is missing from Admin');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=376')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=376"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ["saleshistory:['orders','archivedOrders','financialMovements']","cfAccounts:['purchases','cashflow','receivables','payables','undeposited']","'books/journal':['stockvalue']","financialMovements:['purchases','cashflow','receivables','payables','payouts','saleshistory','undeposited','discrepancy']"])if(!realtimeHubSource.includes(marker))fail(`Finance subscription scope missing: ${marker}`);
  ...
  for(const marker of ['Admin is the operational authority','Finance Books must be generated from Admin sales','Completed paid sales for the selected period',"A().subscribe('financialMovements'"])if(!salesHistorySource.includes(marker))fail(`Sales reconciliation procedure missing: ${marker}`);
  ...
  for(const marker of ['{id:"settings",label:"Settings"}','PAGES.settings','App.changeAccountingPeriod','App.runLegacyCutover','legacyOwnerCapitalReset','August 30 and later discrepancies remain visible'])if(!booksHtml.includes(marker))fail(`Finance Settings and legacy-cutover control missing: ${marker}`);
  for(const marker of ['cutoffDate','legacy_owner_capital_reset_v2_${cutoffDate}','rowDate<=cutoffDate','"4990":"credit","6110":"debit","1190":"debit","2100":"credit"'])if(!functionsSource.includes(marker))fail(`Date-bounded legacy close safeguard missing: ${marker}`);
  ...
  for(const marker of ['action:this._edit?"correct_manual_journal":"manual_journal"','void_manual_journal','Linked bill / payable','linkedPayableId','Correct</button>','View posting history','Void</button>'])if(!booksHtml.includes(marker))fail(`Manual journal correction/void/payable-link interface missing: ${marker}`);
