...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=322')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=322"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ['function migratedLocalEntry(e)','books_manual_draw25_','window.__financialMovements'])if(!booksHtml.includes(marker))fail(`Migrated local Books journal de-duplication missing: ${marker}`);
