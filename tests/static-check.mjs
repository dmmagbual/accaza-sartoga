...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  for(const marker of ['exports.repairOrderInventoryMarker = onCall','Inventory evidence does not match the expected order usage','no stock or Finance movement posted'])if(!functionsSource.includes(marker))fail(`Controlled inventory confirmation repair missing: ${marker}`);
