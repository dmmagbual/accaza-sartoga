...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=321')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=321"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ['function registerFloatControlEntry(amount, at)','books/journal/register_float_control','sourceId: "fixedFloat"','register_float_changed','total cash unchanged'])if(!functionsSource.includes(marker))fail(`Register cash-float Finance control missing: ${marker}`);
