...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  for(const marker of ["saleshistory:['orders','archivedOrders','financialMovements']","cfAccounts:['purchases','cashflow','receivables','payables']","'books/journal':['stockvalue']","financialMovements:['purchases','cashflow','receivables','payables','payouts','saleshistory']"])if(!realtimeHubSource.includes(marker))fail(`Finance subscription scope missing: ${marker}`);
  ...
  for(const marker of ["'setUndepositedOpeningBalance'","'repairPettyVoucherFinancial'",'exports.repairPettyVoucherFinancial = onCall','missing from the ledger','Repair missing payments','financialRepairedAt'])if(!functionsSource.includes(marker)&&!undepositedSource.includes(marker)&&!adminSource.includes(marker))fail(`Undeposited callable or missing-payment repair marker missing: ${marker}`);
