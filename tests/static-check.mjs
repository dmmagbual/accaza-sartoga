...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  for(const marker of ['cutoffDate','legacy_owner_capital_reset_v4_${cutoffDate}','protectedDate="2026-08-30"','protectedShortage=120','"4990":"credit","6110":"debit","1190":"debit","2100":"credit"','crossPeriodReversals={"6100":0,"6110":0}','entry.reversalOf','originalDate>=augustStart'])if(!functionsSource.includes(marker))fail(`Legacy close and protected discrepancy safeguard missing: ${marker}`);
