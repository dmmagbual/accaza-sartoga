...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  for(const marker of ['caseVersion:2','cashDifferenceCase','business_expense','supplier_purchase','staff_receivable','customer_refund','unrecorded_sale','capital_contribution','unexplained_overage'])if(!adminSource.includes(marker))fail(`Unified Admin cash-difference treatment missing: ${marker}`);
  for(const marker of ['cash_difference_case_resolution','cashDifferenceCases','partially_resolved','cashDifferencePurchaseAdvances','purchasePayouts.push','financialControlLinks/correctionMovements','This variance correction has already been deposited or settled'])if(!functionsSource.includes(marker))fail(`End-to-end cash-difference server safeguard missing: ${marker}`);
