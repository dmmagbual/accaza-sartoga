...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  for(const marker of ['Financial control exceptions','orphanedPayoutDeposits','App.repairPayoutDeposit','window.__booksRepairPayout','repairReversedPayoutDeposit'])if(!fs.readFileSync(path.join(root,'books.html'),'utf8').includes(marker))fail(`Books payout repair interface missing: ${marker}`);
