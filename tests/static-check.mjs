...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=312')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=312"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ["saleshistory:['orders','archivedOrders','financialMovements']","cfAccounts:['purchases','cashflow','receivables','payables']","'books/journal':['stockvalue']","financialMovements:['purchases','cashflow','receivables','payables','payouts','saleshistory','undeposited']"])if(!realtimeHubSource.includes(marker))fail(`Finance subscription scope missing: ${marker}`);
  ...
  for(const marker of ["pettyCashVouchers:['petty','purchases','undeposited']","financialMovements:['purchases','cashflow','receivables','payables','payouts','saleshistory','undeposited']","cashCustody:['cashflow','undeposited']"])if(!realtimeHubSource.includes(marker))fail(`Undeposited realtime scope missing: ${marker}`);
