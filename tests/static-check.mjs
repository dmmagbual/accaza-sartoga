...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  for(const marker of ['posPaymentVerification=null','paymentVerificationSignature(payments,total)',"button.textContent='Verify Payment'",'posPaymentVerification.signature===signature','Payment verified · complete the sale when ready'])if(!posSource.includes(marker))fail(`Two-stage in-store cashier verification marker missing: ${marker}`);
  ...
  for(const marker of ['Invoice pending — provisional obligation','purchaseHistoryHtml','data-purchase-details','data-purchase-finalize','data-purchase-legacy'])if(!posSource.includes(marker))fail(`Purchase review/GRNI UI marker missing: ${marker}`);
