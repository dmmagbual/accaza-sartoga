...
if(!functionsSource.includes('process.env.ENFORCE_APP_CHECK'))fail('App Check enforcement environment Boolean guard missing');
  ...
  if(!adminHtml.includes('assets/js/admin/core.mjs?v=378')||!adminCoreItem.source.includes('from"./overview-insights.mjs?v=378"'))fail('Admin Overview module graph is not tied to the visible Admin build');
  ...
  for(const marker of ['cutoffDate','legacy_owner_capital_reset_v3_${cutoffDate}','protectedDate="2026-08-30"','protectedShortage=120','"4990":"credit","6110":"debit","1190":"debit","2100":"credit"'])if(!functionsSource.includes(marker))fail(`Legacy close and protected discrepancy safeguard missing: ${marker}`);
