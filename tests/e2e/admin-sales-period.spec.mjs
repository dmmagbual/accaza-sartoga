...
test('full ranking reads its own dates and permits PDF only after its data loads',async({page})=>{
  await page.evaluate(async()=>{
    ...
    const overview=createOverviewInsights({esc:String,readRanking:range=>{window.rankingReads.push(range);return new Promise(resolve=>{window.finishRanking=()=>resolve([{id:'august',status:'Completed',paymentStatus:'confirmed',timestamp:Date.parse('2026-08-03T12:00:00+08:00'),total:300,subtotal:300,lineItems:[{name:'Latte',qty:3,unitTotal:100}]}]);});}});
    overview.render({historyComplete:true,sales:[],outcomes:[],active:[]});
  });
  await page.locator('#openDrinkRankingBtn').click();
  await expect(page.locator('#drinkRankingFrom')).toHaveValue('2026-08-01');
  await expect(page.locator('#drinkRankingTo')).toHaveValue('2026-08-31');
  ...
  expect(await page.evaluate(()=>window.rankingReads[0].from)).toBe('2026-08-01');
  expect(await page.evaluate(()=>window.rankingReads[0].to)).toBe('2026-08-31');
