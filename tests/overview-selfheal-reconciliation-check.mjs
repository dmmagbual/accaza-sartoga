// Overview self-heal + Sales History reconciliation regression.
// 1) The Overview dashboard must page its own complete history to totals WITHOUT
//    the operator first opening the Sales History tab (the reported v301 defect).
...
globalThis.window={dispatchEvent(){},AccazaSales:{
...
// ---- 1) Self-heal without opening Sales History ----
// financialMovements signals ready LAST. The old code returned out of the whole
// routine on the first not-ready feed and relied on another tab to drive completion.
...
for(var i=0;i<60&&calls.length<3;i++)await new Promise((r)=>setTimeout(r,0));
if(calls.join(',')!=='orders,archivedOrders,financialMovements')throw new Error('Overview did not self-complete all three feeds without opening Sales History.');
if(elements.overviewDataNote.textContent!=='Complete sales history verified.')throw new Error('Overview reported complete before finishing its own pagination.');
console.log('PASS: Overview finishes its own complete-history load without opening Sales History.');
