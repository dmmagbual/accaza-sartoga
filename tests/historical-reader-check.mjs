...
counters={};rollupState={complete:true,schemaVersion:7,runId:'v7-complete'};rollupDocs=[{id:'2026-09',data:()=>({schemaVersion:3,analyticsSchemaVersion:2})}];
const incompleteCoverage=await budgetContext.exports.readHistoricalSalesRollup({data:{from:'2026-01',to:'2026-09'}});
assert.equal(incompleteCoverage.coverageComplete,false,'a returned September record cannot prove a January-to-September YTD');
assert.equal(incompleteCoverage.analyticsReady,false,'comparison tables must stay hidden until every requested calendar month is present');
