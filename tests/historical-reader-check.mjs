...
const fakeDb={ref(path){return {async get(){return {val:()=>path.endsWith("salesRollupBackfill")?{complete:true,schemaVersion:5}:true};},async transaction(update){const next=update(counters[path]);if(next===undefined)return {committed:false};counters[path]=next;return {committed:true};}};}};
