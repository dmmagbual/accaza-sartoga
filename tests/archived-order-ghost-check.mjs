...
for(const marker of ['await liveRef.get()','await archivedRef.get()','liveRef.transaction((current) => current ? Object.assign({}, current, metadata) : undefined, undefined, false)','archivedRef.transaction((current) => current ? Object.assign({}, current, metadata) : undefined, undefined, false)','has no authoritative live or archived record'])must(orderRecords,marker,'Shared authoritative-order update guard missing');
...
const state={orders:{POS1:{id:'POS1',status:'Completed'}},archivedOrders:{GF1:{id:'GF1',status:'Archived',payoutId:'po1',settlementStatus:'settled'}}},warmed=new Set();
const db={ref(path){const parts=path.split('/').filter(Boolean),read=()=>{let current=state;for(const p of parts)current=current&&current[p];return current??null;};return{async get(){warmed.add(path);return{exists:()=>read()!=null,val:read};},async transaction(fn){let parent=state;for(const p of parts.slice(0,-1))parent=parent[p]||(parent[p]={});const key=parts.at(-1),current=warmed.has(path)?parent[key]??null:null,next=fn(current);if(next===undefined)return{committed:false,snapshot:{val:()=>parent[key]??null}};parent[key]=next;return{committed:true,snapshot:{val:()=>next}};}};}};
...
console.log('PASS: cold-cache metadata updates find live and archived orders without recreating deleted records or changing sales, payouts, stock movements, or journals.');
