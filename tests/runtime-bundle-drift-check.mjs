...
if(!/exports\.askAccazaAI=onCall\(\{[^}]*timeoutSeconds:120[^}]*ASHNA_API_KEY,CEREBRAS_API_KEY,GROQ_API_KEY\]\}/.test(functionsSource)||!/exports\.askAccazaAIStandalone=onCall\(\{[^}]*timeoutSeconds:120[^}]*ASHNA_API_KEY,CEREBRAS_API_KEY,GROQ_API_KEY\]\}/.test(functionsSource))throw new Error('Both Accaza AI callables need the 120 s limit and the Ashna, Cerebras and Groq secrets.');
...
// Groq is General chat only: its free tier (8,000 tokens/min) cannot carry a fact pack or record reading.
for(const [label,list,expected,text] of [['analysis',analysis,'gemini>cerebras>deepseek>ollama>ashna','Gemini > Cerebras > DeepSeek > Qwen > Ashna'],['general chat',general,'gemini>groq>cerebras>deepseek>ollama>ashna','Gemini > Groq > Cerebras > DeepSeek > Qwen > Ashna']]){const order=[...list.matchAll(/\{name:"([a-z]+)"/g)].map(m=>m[1]).join('>');if(order!==expected)throw new Error(`Accaza AI ${label} provider order must be ${text} (got ${order}).`);}
if(/GROQ/.test(functionsSource.slice(functionsSource.indexOf('function accazaAiAnalysisProviders('),functionsSource.indexOf('function accazaAiGeneralChatProviders(')))||/GROQ|askGroq/.test(fs.readFileSync(path.join(root,'src/functions/62a-accaza-ai-records.js'),'utf8')))throw new Error('Groq must stay out of Accaza analysis and record reading until a paid Groq tier is active.');
{const agentList=functionsSource.slice(functionsSource.indexOf('function accazaAiAgentProviders('));const agentOrder=[...agentList.slice(0,agentList.indexOf('];}')).matchAll(/\{name:"([a-z]+)"/g)].map(m=>m[1]).join('>');if(agentOrder!=='gemini>cerebras>deepseek>ollama>ashna')throw new Error(`Accaza AI record-reading provider order must be Gemini > Cerebras > DeepSeek > Qwen > Ashna (got ${agentOrder}).`);}
