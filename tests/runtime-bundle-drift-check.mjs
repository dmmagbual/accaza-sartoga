...
if(functionsSource.includes('tools:[{google_search:{}}]')||!functionsSource.includes('general chat mode')||!functionsSource.includes('accazaAiWebQuestionBlocked(question)')||functionsSource.includes('accazaAiFactPack')||functionsSource.includes('ACCAZA_AI_HOURLY_LIMIT'))throw new Error('Accaza AI is general-chat-only (2026-09-22): it must avoid billable search grounding, keep the business-question guard wired, and never regain a business-data fact pack or a rate-limit cap.');
