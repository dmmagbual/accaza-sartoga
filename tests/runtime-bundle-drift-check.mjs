...
if(!/exports\.askAccazaAI=onCall\(\{[^}]*timeoutSeconds:120[^}]*ASHNA_API_KEY,CEREBRAS_API_KEY\]\}/.test(functionsSource)||!/exports\.askAccazaAIStandalone=onCall\(\{[^}]*timeoutSeconds:120[^}]*ASHNA_API_KEY,CEREBRAS_API_KEY\]\}/.test(functionsSource))throw new Error('Both Accaza AI callables need the 120 s limit and the Ashna secret.');
...
for(const [label,list] of [['analysis',analysis],['general chat',general]]){const order=[...list.matchAll(/\{name:"([a-z]+)"/g)].map(m=>m[1]).join('>');if(order!=='gemini>cerebras>deepseek>ollama>ashna')throw new Error(`Accaza AI ${label} provider order must be Gemini > Cerebras > DeepSeek > Qwen > Ashna (got ${order}).`);}
