...
if(functionsSource.includes('tools:[{google_search:{}}]')||!functionsSource.includes('general chat mode')||!functionsSource.includes('Web chat never receives Accaza data')||!functionsSource.includes('accazaAiWebQuestionBlocked(question)'))throw new Error('Accaza AI general chat must avoid billable search grounding and keep Accaza business questions out of public requests.');
if(!functionsSource.includes('const ACCAZA_AI_RELEASE_VERSION = "1.2"')||!functionsSource.includes('releaseVersion:ACCAZA_AI_RELEASE_VERSION')||!functionsSource.includes('DEEPSEEK_API_KEY')||!functionsSource.includes('accazaAiWithFallback'))throw new Error('Accaza AI must expose its release and keep a server-side DeepSeek fallback provider.');
