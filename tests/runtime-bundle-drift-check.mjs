...
if(!functionsSource.includes('tools:[{google_search:{}}]')||!functionsSource.includes('Web chat never receives Accaza data')||!functionsSource.includes('accazaAiWebQuestionBlocked(question)'))throw new Error('Accaza AI web chat must use Google Search grounding and keep Accaza business questions out of public web requests.');
if(!functionsSource.includes('const ACCAZA_AI_RELEASE_VERSION = "1.0"')||!functionsSource.includes('releaseVersion:ACCAZA_AI_RELEASE_VERSION'))throw new Error('Accaza AI must return its explicit release version for deployment verification.');
