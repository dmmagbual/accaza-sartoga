...
for(const marker of ['function compactDrinkRanking(summary,data)','deps.readRankingSummary','open.hidden=false','open.textContent=\'View full ranking\''])if(!source.includes(marker))throw new Error('Compact Overview summary regressed the drink-only full-ranking path: '+marker);
