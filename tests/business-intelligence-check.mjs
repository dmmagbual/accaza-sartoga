...
if(!/accaza-books-build" content="93"/.test(books)||!/build v93/.test(books))throw new Error('Books build 93 markers are not synchronized');
if(manifest.builds.admin!==410||manifest.builds.books!==93||manifest.builds.serviceWorkerCache!==371)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v371'/.test(sw))throw new Error('Service worker cache 371 is not synchronized');
