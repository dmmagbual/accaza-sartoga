...
if(!/accaza-books-build" content="110"/.test(books)||!/build v110/.test(books))throw new Error('Books build 110 markers are not synchronized');
if(manifest.builds.admin!==500||manifest.builds.books!==110||manifest.builds.serviceWorkerCache!==475)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v475'/.test(sw))throw new Error('Service worker cache 475 is not synchronized');
