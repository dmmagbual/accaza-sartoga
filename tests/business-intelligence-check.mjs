...
if(!/accaza-books-build" content="109"/.test(books)||!/build v109/.test(books))throw new Error('Books build 109 markers are not synchronized');
if(manifest.builds.admin!==493||manifest.builds.books!==109||manifest.builds.serviceWorkerCache!==468)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v468'/.test(sw))throw new Error('Service worker cache 468 is not synchronized');
