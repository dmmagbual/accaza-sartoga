...
if(!/accaza-books-build" content="89"/.test(books)||!/build v89/.test(books))throw new Error('Books build 89 markers are not synchronized');
if(manifest.builds.admin!==409||manifest.builds.books!==89||manifest.builds.serviceWorkerCache!==367)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v367'/.test(sw))throw new Error('Service worker cache 367 is not synchronized');
