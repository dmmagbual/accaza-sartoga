...
if(!/accaza-books-build" content="120"/.test(books)||!/build v120/.test(books))throw new Error('Books build 120 markers are not synchronized');
if(manifest.builds.admin!==537||manifest.builds.books!==120||manifest.builds.serviceWorkerCache!==515)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v515'/.test(sw))throw new Error('Service worker cache 515 is not synchronized');
