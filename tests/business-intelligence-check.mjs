...
if(!/accaza-books-build" content="85"/.test(books)||!/build v85/.test(books))throw new Error('Books build 85 markers are not synchronized');
if(manifest.builds.admin!==405||manifest.builds.books!==85||manifest.builds.serviceWorkerCache!==360)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v360'/.test(sw))throw new Error('Service worker cache 360 is not synchronized');
