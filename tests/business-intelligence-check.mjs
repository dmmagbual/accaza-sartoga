...
if(!/accaza-books-build" content="126"/.test(books)||!/build v126/.test(books))throw new Error('Books build markers are not synchronized');
if(manifest.builds.admin!==549||manifest.builds.books!==126||manifest.builds.serviceWorkerCache!==529)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v529'/.test(sw))throw new Error('Service worker cache is not synchronized');
