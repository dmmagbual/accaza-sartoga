...
if(!/accaza-books-build" content="101"/.test(books)||!/build v101/.test(books))throw new Error('Books build 101 markers are not synchronized');
if(manifest.builds.admin!==449||manifest.builds.books!==101||manifest.builds.serviceWorkerCache!==417)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v417'/.test(sw))throw new Error('Service worker cache 417 is not synchronized');
