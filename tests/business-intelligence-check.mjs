...
if(!/accaza-books-build" content="98"/.test(books)||!/build v98/.test(books))throw new Error('Books build 98 markers are not synchronized');
if(manifest.builds.admin!==447||manifest.builds.books!==98||manifest.builds.serviceWorkerCache!==412)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v412'/.test(sw))throw new Error('Service worker cache 412 is not synchronized');
