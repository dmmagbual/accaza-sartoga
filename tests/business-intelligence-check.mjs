...
if(!/accaza-books-build" content="118"/.test(books)||!/build v118/.test(books))throw new Error('Books build 116 markers are not synchronized');
if(manifest.builds.admin!==536||manifest.builds.books!==118||manifest.builds.serviceWorkerCache!==513)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v513'/.test(sw))throw new Error('Service worker cache 511 is not synchronized');
