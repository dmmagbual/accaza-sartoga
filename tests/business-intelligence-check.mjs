...
if(!/accaza-books-build" content="92"/.test(books)||!/build v92/.test(books))throw new Error('Books build 92 markers are not synchronized');
if(manifest.builds.admin!==409||manifest.builds.books!==92||manifest.builds.serviceWorkerCache!==370)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v370'/.test(sw))throw new Error('Service worker cache 370 is not synchronized');
