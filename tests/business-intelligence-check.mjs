...
if(!/accaza-books-build" content="87"/.test(books)||!/build v87/.test(books))throw new Error('Books build 87 markers are not synchronized');
if(manifest.builds.admin!==408||manifest.builds.books!==87||manifest.builds.serviceWorkerCache!==365)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v365'/.test(sw))throw new Error('Service worker cache 365 is not synchronized');
