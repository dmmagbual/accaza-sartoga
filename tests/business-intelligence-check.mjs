...
if(!/accaza-books-build" content="82"/.test(books)||!/build v82/.test(books))throw new Error('Books build 82 markers are not synchronized');
if(manifest.builds.admin!==403||manifest.builds.books!==82||manifest.builds.serviceWorkerCache!==355)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v355'/.test(sw))throw new Error('Service worker cache 355 is not synchronized');
