...
if(!/accaza-books-build" content="102"/.test(books)||!/build v102/.test(books))throw new Error('Books build 102 markers are not synchronized');
if(manifest.builds.admin!==449||manifest.builds.books!==102||manifest.builds.serviceWorkerCache!==418)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v418'/.test(sw))throw new Error('Service worker cache 418 is not synchronized');
