...
if(!/accaza-books-build" content="95"/.test(books)||!/build v95/.test(books))throw new Error('Books build 95 markers are not synchronized');
if(manifest.builds.admin!==423||manifest.builds.books!==95||manifest.builds.serviceWorkerCache!==384)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v384'/.test(sw))throw new Error('Service worker cache 384 is not synchronized');
