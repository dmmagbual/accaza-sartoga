...
if(manifest.builds.admin!==523||manifest.builds.books!==115||manifest.builds.serviceWorkerCache!==500)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v500'/.test(sw))throw new Error('Service worker cache 500 is not synchronized');
