...
if(manifest.builds.admin!==451||manifest.builds.books!==105||manifest.builds.serviceWorkerCache!==422)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v422'/.test(sw))throw new Error('Service worker cache 422 is not synchronized');
