...
if(!/accaza-books-build" content="94"/.test(books)||!/build v94/.test(books))throw new Error('Books build 94 markers are not synchronized');
if(manifest.builds.admin!==415||manifest.builds.books!==94||manifest.builds.serviceWorkerCache!==376)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v376'/.test(sw))throw new Error('Service worker cache 376 is not synchronized');
