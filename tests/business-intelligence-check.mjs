...
if(!/accaza-books-build" content="88"/.test(books)||!/build v88/.test(books))throw new Error('Books build 88 markers are not synchronized');
if(manifest.builds.admin!==408||manifest.builds.books!==88||manifest.builds.serviceWorkerCache!==366)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v366'/.test(sw))throw new Error('Service worker cache 366 is not synchronized');
