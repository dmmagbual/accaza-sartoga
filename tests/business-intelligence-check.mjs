...
if(!/accaza-books-build" content="81"/.test(books)||!/build v81/.test(books))throw new Error('Books build 81 markers are not synchronized');
if(manifest.builds.admin!==402||manifest.builds.books!==81||manifest.builds.serviceWorkerCache!==354)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v354'/.test(sw))throw new Error('Service worker cache 354 is not synchronized');
