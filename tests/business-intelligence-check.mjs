...
if(!/accaza-books-build" content="100"/.test(books)||!/build v100/.test(books))throw new Error('Books build 100 markers are not synchronized');
if(manifest.builds.admin!==449||manifest.builds.books!==100||manifest.builds.serviceWorkerCache!==416)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v416'/.test(sw))throw new Error('Service worker cache 416 is not synchronized');
