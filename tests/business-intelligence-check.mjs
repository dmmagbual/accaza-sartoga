...
if(!/accaza-books-build" content="113"/.test(books)||!/build v113/.test(books))throw new Error('Books build 113 markers are not synchronized');
if(manifest.builds.admin!==503||manifest.builds.books!==113||manifest.builds.serviceWorkerCache!==478)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v478'/.test(sw))throw new Error('Service worker cache 478 is not synchronized');
