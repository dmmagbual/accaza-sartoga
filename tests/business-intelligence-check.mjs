...
if(!/accaza-books-build" content="129"/.test(books)||!/build v129/.test(books))throw new Error('Books build markers are not synchronized');
if(manifest.builds.admin!==589||manifest.builds.books!==129||manifest.builds.serviceWorkerCache!==570)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v570'/.test(sw))throw new Error('Service worker cache is not synchronized');
