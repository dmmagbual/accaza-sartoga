...
if(!/accaza-books-build" content="107"/.test(books)||!/build v107/.test(books))throw new Error('Books build 107 markers are not synchronized');
if(manifest.builds.admin!==484||manifest.builds.books!==107||manifest.builds.serviceWorkerCache!==456)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v456'/.test(sw))throw new Error('Service worker cache 456 is not synchronized');
