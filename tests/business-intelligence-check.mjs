...
if(!/accaza-books-build" content="91"/.test(books)||!/build v91/.test(books))throw new Error('Books build 91 markers are not synchronized');
if(manifest.builds.admin!==409||manifest.builds.books!==91||manifest.builds.serviceWorkerCache!==369)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v369'/.test(sw))throw new Error('Service worker cache 369 is not synchronized');
