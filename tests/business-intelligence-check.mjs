...
if(!/accaza-books-build" content="119"/.test(books)||!/build v119/.test(books))throw new Error('Books build 119 markers are not synchronized');
if(manifest.builds.admin!==536||manifest.builds.books!==119||manifest.builds.serviceWorkerCache!==514)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v514'/.test(sw))throw new Error('Service worker cache 514 is not synchronized');
