...
if(!/accaza-books-build" content="112"/.test(books)||!/build v112/.test(books))throw new Error('Books build 112 markers are not synchronized');
if(manifest.builds.admin!==502||manifest.builds.books!==112||manifest.builds.serviceWorkerCache!==477)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v477'/.test(sw))throw new Error('Service worker cache 477 is not synchronized');
