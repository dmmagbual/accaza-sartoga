...
if(!/accaza-books-build" content="96"/.test(books)||!/build v96/.test(books))throw new Error('Books build 96 markers are not synchronized');
if(manifest.builds.admin!==427||manifest.builds.books!==96||manifest.builds.serviceWorkerCache!==389)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v389'/.test(sw))throw new Error('Service worker cache 389 is not synchronized');
