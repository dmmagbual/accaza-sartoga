...
if(!/accaza-books-build" content="125"/.test(books)||!/build v125/.test(books))throw new Error('Books build markers are not synchronized');
if(manifest.builds.admin!==548||manifest.builds.books!==125||manifest.builds.serviceWorkerCache!==528)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v528'/.test(sw))throw new Error('Service worker cache is not synchronized');
