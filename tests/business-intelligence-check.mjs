...
if(!/accaza-books-build" content="108"/.test(books)||!/build v108/.test(books))throw new Error('Books build 108 markers are not synchronized');
if(manifest.builds.admin!==487||manifest.builds.books!==108||manifest.builds.serviceWorkerCache!==462)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v462'/.test(sw))throw new Error('Service worker cache 462 is not synchronized');
