...
if(!/accaza-books-build" content="117"/.test(books)||!/build v117/.test(books))throw new Error('Books build 116 markers are not synchronized');
if(manifest.builds.admin!==535||manifest.builds.books!==117||manifest.builds.serviceWorkerCache!==512)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v512'/.test(sw))throw new Error('Service worker cache 511 is not synchronized');
