...
if(!/accaza-books-build" content="116"/.test(books)||!/build v116/.test(books))throw new Error('Books build 116 markers are not synchronized');
if(manifest.builds.admin!==533||manifest.builds.books!==116||manifest.builds.serviceWorkerCache!==510)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v510'/.test(sw))throw new Error('Service worker cache 510 is not synchronized');
