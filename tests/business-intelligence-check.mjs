...
if(manifest.builds.admin!==527||manifest.builds.books!==115||manifest.builds.serviceWorkerCache!==504)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v504'/.test(sw))throw new Error('Service worker cache 504 is not synchronized');
