...
if(!/accaza-books-build" content="121"/.test(books)||!/build v121/.test(books))throw new Error('Books build 120 markers are not synchronized');
if(manifest.builds.admin!==538||manifest.builds.books!==121||manifest.builds.serviceWorkerCache!==516)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v516'/.test(sw))throw new Error('Service worker cache 515 is not synchronized');
