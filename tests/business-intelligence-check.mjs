...
if(!/accaza-books-build" content="104"/.test(books)||!/build v104/.test(books))throw new Error('Books build 104 markers are not synchronized');
if(manifest.builds.admin!==451||manifest.builds.books!==104||manifest.builds.serviceWorkerCache!==421)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v421'/.test(sw))throw new Error('Service worker cache 421 is not synchronized');
