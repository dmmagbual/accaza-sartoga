...
if(!/accaza-books-build" content="124"/.test(books)||!/build v124/.test(books))throw new Error('Books build markers are not synchronized');
if(manifest.builds.admin!==546||manifest.builds.books!==124||manifest.builds.serviceWorkerCache!==525)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v525'/.test(sw))throw new Error('Service worker cache is not synchronized');
