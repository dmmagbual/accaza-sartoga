...
if(manifest.builds.admin!==458||manifest.builds.books!==105||manifest.builds.serviceWorkerCache!==429)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v429'/.test(sw))throw new Error('Service worker cache 429 is not synchronized');
