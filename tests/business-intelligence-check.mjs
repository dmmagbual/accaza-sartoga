...
if(!/accaza-books-build" content="99"/.test(books)||!/build v99/.test(books))throw new Error('Books build 99 markers are not synchronized');
if(manifest.builds.admin!==447||manifest.builds.books!==99||manifest.builds.serviceWorkerCache!==413)throw new Error('Release manifest build markers are not synchronized');
if(!/const CACHE='accaza-v413'/.test(sw))throw new Error('Service worker cache 413 is not synchronized');
