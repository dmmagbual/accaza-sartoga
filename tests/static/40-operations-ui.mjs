...
if(!operationsSource.includes('Production monitor:')||!operationsSource.includes('systemHealth/productionMonitor/current')||!operationsSource.includes('Management alert:')||!functionsSource.includes('exports.evaluateProductionHealth = onSchedule')||!functionsSource.includes('ProductionHealth.evaluate')||!functionsSource.includes('AlertEscalation.decide'))fail('Phase 18 production early-warning and escalation monitor is incomplete');
if(!operationsSource.includes('Release certification:')||!operationsSource.includes('getProductionCertification')||!functionsSource.includes('exports.getProductionCertification = onCall')||!functionsSource.includes('ReleaseCertification.evaluate'))fail('Phase 16 read-only production certification surface is incomplete');
if(!operationsSource.includes('Production validation:')||!operationsSource.includes('getProductionValidation')||!functionsSource.includes('exports.getProductionValidation = onCall')||!functionsSource.includes('ProductionValidation.evaluate'))fail('Phase 17 read-only production validation surface is incomplete');
...
for(const marker of ['update_platform_payout_metadata','update_platform_payout_date','edit_payout_reference','linked_dates_updated','platformPayoutDateRevisions'])if(!functionsSource.includes(marker))fail(`Platform payout metadata safeguard missing: ${marker}`);
