...
if(!functionsSource.includes('cogsDetailSource: {')||!functionsSource.includes('path: `orderInventoryPlans/${orderId}`')||!/lines:costing\.lines/.test(functionsSource))fail('Release 3B traceable COGS snapshot missing: the order must point at its immutable plan, which keeps every costed line');
