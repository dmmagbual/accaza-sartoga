(function(root,factory){
  ...
  function serveStyleFor(item,optLabels,ctx){
    ...
    var allAssignments=ctx.packagingAssignments||{},assignment=(allAssignments[String((item&&item.cat)||'')])||{};
    ...
    var itemStyle=(assignment.items||{})[String((item&&item.key)||'')]||(allAssignments._items||{})[String((item&&item.key)||'')];
  ...
  return {VERSION:VERSION,SIZES:SIZES,isSingleServing:isSingleServing,effectiveSize:effectiveSize,normalizeUnit:unit,unitInfo:unitInfo,compatible:compatible,convert:convert,normalizeRecipe:normalizeRecipe,costOrder:costOrder,costRecipe:costRecipe,optKey:optKey,serveStyleFor:serveStyleFor,packagingRows:packagingRows};
});
