(function(global){
  ...
  // Every new Finance Books session begins with the live current month. A user may
  // choose another range for the active session, but yesterday's saved filter must
  // never make today's books silently reopen on an older reporting period.
  var state=normalize({mode:'current'});
