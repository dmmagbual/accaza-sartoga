...
function renderUndeposited(){
  ...
  var rc=document.getElementById('ucReconcileCustody');if(rc)rc.onclick=function(){reconcileCustodyGap(custodyGap);};
...
function reconcileCustodyGap(gap){
  var a=A(),form=window.AccazaFormDialog;if(!a.reconcileUndepositedCustody||!a.managerApproval||!form){alert('Custody reconciliation service is unavailable. Refresh the portal.');return;}
  var options=poolRows().filter(function(r){return !r.movement.reversalOf&&!r.movement.reversedByMovementId&&Math.abs((r.inAmt-r.outAmt)-gap)<.009;}).map(function(r){return{value:r.id,label:fmtDate(r.ts)+' · '+(r.movement.memo||r.movement.reference||r.ref||r.id)+' · '+r.id};});
  if(!options.length){alert('No active Finance journal increasing Undeposited Collection by exactly '+peso(gap)+' was found. Review the journal amount and account first.');return;}
  form.run({title:'Link existing Finance journal to cash custody',subtitle:'The selected journal remains unchanged. Only its missing Admin custody and audit link will be created.',submitLabel:'Review & link',busyLabel:'Reconciling…',fields:[{name:'movementId',label:'Existing Finance journal',type:'select',required:true,options:options},{name:'reason',label:'Why this journal represents physical cash awaiting deposit',type:'textarea',required:true,maxLength:500}]},function(v){return a.reconcileUndepositedCustody({movementId:v.movementId,preview:true}).then(function(r){var d=(r&&r.data)||r||{};if(d.duplicate)return d;if(!confirm('Confirm that '+peso(d.amount)+' from this existing journal is physical cash awaiting deposit?\n\nNo new journal will be posted.'))throw new Error('cancelled');return a.managerApproval('reconcile_undeposited_custody',v.movementId,d.amount,v.reason).then(function(ap){return a.reconcileUndepositedCustody({movementId:v.movementId,reason:v.reason,approvalId:ap.approvalId});});});}).then(function(r){var d=(r&&r.data)||r||{};if(!d||d.duplicate)return;alert('Reconciled '+peso(d.amount)+'. Finance Books was unchanged; Admin custody now matches the ledger.');}).catch(function(e){if(String((e&&e.message)||e).indexOf('cancelled')<0)alert('Could not reconcile custody: '+((e&&e.message)||e));});
}
