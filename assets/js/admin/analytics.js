...
if(comparisonReady){var cashiers={},mtdCashiers=current.cashiers||{},ytdCashiers=ytd.cashiers||{};Object.keys(mtdCashiers).concat(Object.keys(ytdCashiers)).forEach(function(key){var row=ytdCashiers[key]||mtdCashiers[key]||{},mtd=mtdCashiers[key]||{},total=ytdCashiers[key]||{};cashiers[key]={label:row.name||key,mtd:Number(mtd.net)||0,ytd:Number(total.net)||0};});var cashierRows=Object.keys(cashiers).map(function(key){return cashiers[key];}).sort(function(a,b){return b.ytd-a.ytd||a.label.localeCompare(b.label);});cashierHtml=cashierRows.length?comparisonTable('Cashier',cashierRows):'<p class="az-note">No completed cashier sales in this period.</p>';var peakRows=hourKeys.map(function(hour){var h=Number(hour);return{label:(h%12||12)+(h<12?'am':'pm'),mtd:Number(hours[hour].net)||0,ytd:Number(ytd.hours&&ytd.hours[hour]&&ytd.hours[hour].net)||0};});peakHtml=peakRows.length?comparisonTable('Hour',peakRows):'<p class="az-note">No completed peak-hour sales in this period.</p>';var weekdayRows=[0,1,2,3,4,5,6].map(function(day){return{label:dowN[day],mtd:Number(weekdays[day]&&weekdays[day].net)||0,ytd:Number(ytd.weekdays&&ytd.weekdays[day]&&ytd.weekdays[day].net)||0,day:day};});weekdayHtml=comparisonTable('Day',weekdayRows,'weekday');var drinks={},mtdDrinks=compactDrinkItems(current),ytdDrinks=compactDrinkItems(ytd);mtdDrinks.concat(ytdDrinks).forEach(function(row){var key=row.itemKey||row.name,entry=drinks[key]||(drinks[key]={label:row.name,mtd:0,ytd:0,units:0});entry.label=row.name;});mtdDrinks.forEach(function(row){var entry=drinks[row.itemKey||row.name];entry.mtd=Number(row.net)||0;entry.units=Number(row.units)||0;});ytdDrinks.forEach(function(row){drinks[row.itemKey||row.name].ytd=Number(row.net)||0;});var drinkRows=Object.keys(drinks).map(function(key){return drinks[key];}).sort(function(a,b){return b.ytd-a.ytd||a.label.localeCompare(b.label);}).slice(0,5);drinksHtml=drinkRows.length?comparisonTable('Drink',drinkRows,'drink',true):'<p class="az-note">No completed drink sales in this period.</p>';}
...
root.innerHTML='<div class="pz-h">📊 Sales Analytics</div><div class="az-note">Quota-safe historical summaries for the selected period.</div><div class="az-kpis">'+kpi('Net sales',peso(current.net),trend)+kpi('Transactions',String(current.orders))+kpi('Average sale',peso(aov))+'</div><div class="az-sec">Daily net sales</div><div class="pz-card">'+(days.length?days.map(function(day){return bar(fmtD(day),Number(current.days[day].net)||0,maxDay,peso0(current.days[day].net));}).join(''):'<p class="az-note">No completed sales in this period.</p>')+'</div><div class="az-sec">Sales by cashier</div><div class="pz-card az-comparison-card">'+cashierHtml+'</div><div class="az-analytics-split"><div><div class="az-sec">Weekly sales</div><div class="pz-card az-week-card">'+weeklyHtml+'</div><div class="az-sec">Top drinks</div><div class="pz-card az-drinks-card">'+drinksHtml+'</div></div><div><div class="az-sec">Peak hours</div><div class="pz-card az-comparison-card">'+peakHtml+'</div></div></div><div class="az-sec">Sales by day of week</div><div class="pz-card az-comparison-card">'+weekdayHtml+'</div>';}
...
function comparisonTable(label,rows,kind,showUnits){var max=Math.max.apply(null,rows.map(function(row){return Math.max(Number(row.mtd)||0,Number(row.ytd)||0);}).concat([1]));return '<div style="overflow-x:auto;"><table class="pz-tbl az-comparison-table az-'+(kind||'standard')+'-table"><thead><tr><th>'+esc(label)+'</th>'+(showUnits?'<th class="r">Cups</th>':'')+'<th class="r">MTD</th><th class="r">YTD</th></tr></thead><tbody>'+rows.map(function(row){var mtd=Math.max(0,Math.round((Number(row.mtd)||0)/max*100)),ytd=Math.max(0,Math.round((Number(row.ytd)||0)/max*100)),rowClass=kind==='weekday'?' az-weekday-'+row.day:'';return '<tr class="'+rowClass+'"><td>'+esc(row.label)+'</td>'+(showUnits?'<td class="r az-units">'+Number(row.units||0)+'</td>':'')+'<td class="r az-meter az-meter-mtd"><span style="--meter:'+mtd+'%">'+peso0(row.mtd)+'</span></td><td class="r az-meter az-meter-ytd"><span style="--meter:'+ytd+'%">'+peso0(row.ytd)+'</span></td></tr>';}).join('')+'</tbody></table></div>';}
