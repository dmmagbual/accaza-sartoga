...
function closeControlHtml(d,shiftRows,shiftLoadError){
  ...
  var shiftUnavailable=!!shiftLoadError||!closed.length,shiftPlaceholder=shiftLoadError?'Could not load closed shifts':(!closed.length?'No closed shifts for this trading day':'Select a closed shift'),shiftNote=shiftLoadError?'Closed shifts could not be loaded. Check your connection or access, then reload the report.':(!closed.length?'Close a shift in Register Operations before running a shift reconciliation.':'Choose the exact closed register shift to reconcile.');
  ...
  return '<section class="pz-card" style="margin-bottom:1rem;background:'+tone+';border:1px solid #d7c8b2;"><div style="display:flex;justify-content:space-between;gap:.7rem;align-items:center;flex-wrap:wrap;"><div><div class="pz-lbl">Shared Admin ↔ Finance control</div><div class="pz-h" style="font-size:1.05rem;">Daily Financial Close · '+esc(closeStatusLabel(status))+'</div><div class="pz-sub">'+(row?('Revision '+row.revision+' · '+row.transactionCount+' orders · '+exceptions.length+' exception(s) · '+timing.length+' timing item(s)'):'Run the server-authoritative close to compare transaction sources, control accounts and subledgers.')+'</div></div><div style="display:flex;gap:.4rem;flex-wrap:wrap;"><button class="pz-btn ok" id="drRunClose"'+(loading?' disabled':'')+'>'+(loading?'Running…':'Run daily reconciliation')+'</button>'+(row&&['RECONCILED','RECONCILED_WITH_TIMING_ITEMS'].indexOf(status)>-1?'<button class="pz-btn ok" id="drCertifyClose">Manager certify</button>':'')+'</div></div>'
    ...
    +'<div style="margin-top:.65rem;display:flex;gap:.4rem;align-items:end;flex-wrap:wrap;"><div><span class="pz-lbl">Optional shift close</span><select class="pz-in" id="drCloseShift"'+(shiftUnavailable?' disabled aria-disabled="true"':'')+'><option value="">'+esc(shiftPlaceholder)+'</option>'+closed.map(function(s){return '<option value="'+esc(s.id)+'">'+esc(s.shiftReference||s.id)+' · '+esc(s.staff||'Register')+'</option>';}).join('')+'</select></div><button class="pz-btn sec" id="drRunShiftClose"'+(shiftUnavailable?' disabled aria-disabled="true"':'')+'>Run shift reconciliation</button><div class="pz-sub" style="flex-basis:100%;">'+esc(shiftNote)+'</div></div></section>';
}
...
function renderDailyReport(){
  ...
  function buildDay(sh,shiftLoadError){
    ...
    sales.forEach(function(s){var o=s.o;var c=drChannel(o);var ch=chan[c];ch.tx++;var nt;
      ...
      var occurredAt=drTransactionTimestamp(o,s.ts);txns.push({occurredAt:occurredAt,dateKey:drTransactionDateKey(occurredAt,o.date),dateLabel:drTransactionDateLabel(occurredAt,o.date),timeLabel:drTransactionTimeLabel(occurredAt,o.time),dateTime:drTransactionDateTime(occurredAt,o.date,o.time),id:o.id,channel:chan[c].lbl,method:pays.map(function(p){return p.method;}).join('+'),amount:Number(o.total)||0,refund:s.refund});
    });
    ...
    var shiftRows=Object.keys(sh).map(function(k){return Object.assign({id:k},sh[k]);}).filter(function(s){return shiftDay[s.id]===d;}).sort(function(a,b){return (a.openAt||0)-(b.openAt||0);}).map(function(s){var g=byShift[s.id]||{tx:0,net:0,cash:0};var open=s.status!=='closed';return {id:s.id,shiftReference:s.shiftReference||'',staff:s.staff||'',openAt:s.openAt,closeAt:s.closeAt||null,open:open,tx:g.tx,net:g.net,cash:g.cash,cashToSettle:(open?null:(s.cashToSettle!=null?Number(s.cashToSettle):null))};});
  ...
  function build(payouts,shiftRows,chan,byMethod,items,txns,refundsTot,netTot,sales,shiftLoadError){
    ...
    var html='<div class="pz-h">📆 Daily Report</div>'
      ...
      +closeControlHtml(d,shiftRows,shiftLoadError)
