...
var cashierHtml='<p class="az-note">Select the current month through today to compare MTD and YTD cashiers.</p>';if(ytd&&compactAnalytics.analyticsReady){var cashiers={},mtdCashiers=current.cashiers||{},ytdCashiers=ytd.cashiers||{};Object.keys(mtdCashiers).concat(Object.keys(ytdCashiers)).forEach(function(key){var row=ytdCashiers[key]||mtdCashiers[key]||{},mtd=mtdCashiers[key]||{},total=ytdCashiers[key]||{};cashiers[key]={name:row.name||key,mtd:Number(mtd.net)||0,ytd:Number(total.net)||0};});var rows=Object.keys(cashiers).map(function(key){return cashiers[key];}).sort(function(a,b){return b.ytd-a.ytd||a.name.localeCompare(b.name);}),maxCashier=Math.max.apply(null,rows.map(function(row){return Math.max(row.mtd,row.ytd);}).concat([1]));cashierHtml=rows.length?'<div class="az-chart-key"><span class="az-key-mtd">MTD</span><span class="az-key-ytd">YTD</span></div><div class="az-cashier-chart">'+rows.map(function(row){return '<div class="az-cashier-row"><div class="az-cashier-name">'+esc(row.name)+'</div><div class="az-cashier-bars"><div class="az-cashier-line"><span class="az-cashier-mtd" style="width:'+Math.round(row.mtd/maxCashier*100)+'%"></span></div><div class="az-cashier-line"><span class="az-cashier-ytd" style="width:'+Math.round(row.ytd/maxCashier*100)+'%"></span></div></div><div class="az-cashier-values"><span>'+peso0(row.mtd)+' MTD</span><span>'+peso0(row.ytd)+' YTD</span></div></div>';}).join('')+'</div>':'<p class="az-note">No completed cashier sales in this period.</p>';}else if(ytd)cashierHtml='<p class="az-note">Cashier YTD is being prepared from the bounded historical reporting summary.</p>';
var peakHtml=hourKeys.length?hourKeys.map(function(hour){var h=Number(hour),mtd=Number(hours[hour].net)||0,total=ytd&&ytd.hours&&ytd.hours[hour]?Number(ytd.hours[hour].net)||0:null;return bar((h%12||12)+(h<12?'am':'pm'),mtd,maxHour,peso0(mtd)+(total!=null?' MTD · '+peso0(total)+' YTD':''),'az-hour');}).join(''):'<p class="az-note">Peak-hour data is being prepared.</p>';
var weekdayHtml=[0,1,2,3,4,5,6].map(function(day){var mtd=Number(weekdays[day]&&weekdays[day].net)||0,total=ytd&&ytd.weekdays&&ytd.weekdays[day]?Number(ytd.weekdays[day].net)||0:null;return bar(dowN[day],mtd,maxDow,peso0(mtd)+(total!=null?' MTD · '+peso0(total)+' YTD':''),'az-day-'+day);}).join('');
