...
function renderDailyReport(){
  ...
  function build(payouts,shiftRows,chan,byMethod,items,txns,refundsTot,netTot,sales,shiftLoadError,reconciliation){
    ...
    var chRows=['instore','grabfood','foodpanda','online'].map(function(c){var x=chan[c];if(!x.tx)return '';return '<tr><td>'+x.lbl+'</td><td class="r">'+x.tx+'</td><td class="r">'+peso(x.gross)+'</td><td class="r">'+(x.disc?'−'+peso(x.disc):'—')+'</td><td class="r">'+(x.refund?'−'+peso(x.refund):'—')+'</td><td class="r">'+peso(x.net)+'</td></tr>';}).join('');
...
function printDailyReport(d,X){
  ...
  var ch=['instore','grabfood','foodpanda','online'].map(function(c){var x=X.chan[c];if(!x.tx)return '';return '<tr><td>'+x.lbl+' ('+x.tx+')<br><small>Gross '+peso(x.gross)+' · Disc −'+peso(x.disc)+' · Refund −'+peso(x.refund)+'</small></td><td style="text-align:right;">'+peso(x.net)+'</td></tr>';}).join('');
