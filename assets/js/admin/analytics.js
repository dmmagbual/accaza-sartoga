...
function orderCOGS(o){var corrected=o.correctedCogsSnapshot!=null,_x=Number(corrected?o.correctedExtraCost:o.extraCost)||0,lines=o.correctedLineItems||o.lineItems;
  if(corrected||o.cogsSnapshot!=null)return{cost:(Number(corrected?o.correctedCogsSnapshot:o.cogsSnapshot)||0)+(Number(o.costCorrectionTotal)||0)+_x,covered:o.cogsCovered!==false||(!!o.costCorrections&&!o.costPendingLines)};
...
function loadCompactAnalytics(from,to){var span=to-from,key=from+':'+to;if(compactAnalytics.key===key&&(compactAnalytics.loading||compactAnalytics.current))return;var currentPeriod={startAt:from,endAt:to-1},previousPeriod={startAt:from-span,endAt:from-1},readFrom=isCurrentMonthToDate(currentPeriod)?currentAnalyticsMonth().slice(0,4)+'-01':compactMonth(previousPeriod.startAt);compactAnalytics={key:key,loading:true,error:'',current:null,previous:null,ytd:null,analyticsReady:false,coverageComplete:false,missingMonths:[],months:null,currentPeriod:currentPeriod,previousPeriod:previousPeriod,schemaVersion:0,request:compactAnalytics.request+1};var request=compactAnalytics.request;A().readHistoricalSalesRollup({from:readFrom,to:compactMonth(currentPeriod.endAt)}).then(function(result){if(request!==compactAnalytics.request)return;if(!result||result.ready!==true||Number(result.schemaVersion)<2)throw new Error('Historical report summaries need one controlled maintenance run.');compactAnalytics.months=result.months||{};compactAnalytics.schemaVersion=Number(result.schemaVersion)||2;compactAnalytics.analyticsReady=result.analyticsReady===true;compactAnalytics.coverageComplete=result.coverageComplete===true;compactAnalytics.missingMonths=Array.isArray(result.missingMonths)?result.missingMonths:[];refreshCompactAnalytics();}).catch(function(error){if(request===compactAnalytics.request)compactAnalytics.error=String(error&&error.message||error);}).finally(function(){if(request===compactAnalytics.request){compactAnalytics.loading=false;if(isTab('analytics'))renderAnalytics();}});}
function compactDrinkItems(summary){var menu=(A()&&A().menuItemsMap)||{},types=(window.__posSettings&&window.__posSettings.catType)||{},items={};Object.keys((summary&&summary.items)||{}).forEach(function(key){var row=summary.items[key]||{},line={itemKey:key,name:row.name||key,categoryId:row.categoryId||''};if(!window.AccazaSales.isDrinkLine(line,{menuItems:menu,catType:types}))return;var itemKey=window.AccazaSales.drinkKey(line)||key,name=window.AccazaSales.drinkLabel(line,menu)||row.name||key,item=items[itemKey]||(items[itemKey]={itemKey:itemKey,name:name,units:0,net:0});item.name=name;item.units+=Number(row.units)||0;item.net+=Number(row.net)||0;});return Object.values(items).sort(function(a,b){return b.net-a.net||b.units-a.units||a.name.localeCompare(b.name);});}
...
function renderCompactAnalyticsV5(current,previous){var root=document.getElementById('analyticsRoot');if(!root)return;var days=Object.keys(current.days||{}).sort(),weekly={},hours=current.hours||{},weekdays=current.weekdays||{},ytd=compactAnalytics.ytd||null,maxDay=Math.max.apply(null,days.map(function(day){return Number(current.days[day].net)||0;}).concat([1]));days.forEach(function(day){var week=compactWeekKey(day);weekly[week]=(weekly[week]||0)+(Number(current.days[day].net)||0);});var weeks=Object.keys(weekly).sort(),maxWeek=Math.max.apply(null,weeks.map(function(week){return weekly[week];}).concat([1])),trend=previous.net>0?(current.net-previous.net)/previous.net*100:(current.net>0?100:0),aov=current.orders?current.net/current.orders:0,dowN=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
var comparisonReady=!!(ytd&&compactAnalytics.analyticsReady&&compactAnalytics.coverageComplete),missing=compactAnalytics.missingMonths||[],comparisonNote=!ytd?'<p class="az-note">Select the current month through today to compare MTD and YTD.</p>':missing.length?'<p class="az-note az-summary-hold"><b>YTD withheld:</b> '+esc(missing.join(', '))+' is not yet reconciled to the historical source records.</p>':'<p class="az-note">MTD/YTD detail is hidden until the date-level reporting summary is rebuilt. This prevents partial or duplicated totals.</p>',cashierHtml=comparisonNote,peakHtml=comparisonNote,weekdayHtml=comparisonNote,drinksHtml=comparisonNote;
if(comparisonReady){cashierHtml='<div class="az-cashier-pies">'+cashierPie('MTD · '+businessDate(compactAnalytics.currentPeriod.endAt).slice(0,7),current)+cashierPie('YTD · '+businessDate(compactAnalytics.currentPeriod.endAt).slice(0,4),ytd)+'</div>';var hourKeys=Object.keys(Object.assign({},hours,ytd.hours||{})).sort(function(a,b){return Number(a)-Number(b);}),peakRows=hourKeys.map(function(hour){var h=Number(hour);return{label:(h%12||12)+(h<12?'am':'pm'),mtd:Number(hours[hour]&&hours[hour].net)||0,ytd:Number(ytd.hours&&ytd.hours[hour]&&ytd.hours[hour].net)||0};});peakHtml=peakRows.length?peakHourChart(peakRows):'<p class="az-note">No completed peak-hour sales in this period.</p>';var weekdayRows=[0,1,2,3,4,5,6].map(function(day){return{label:dowN[day],mtd:Number(weekdays[day]&&weekdays[day].net)||0,ytd:Number(ytd.weekdays&&ytd.weekdays[day]&&ytd.weekdays[day].net)||0,day:day};});weekdayHtml=weekdayLineChart(weekdayRows);var drinkRows=compactDrinkItems(ytd).slice(0,12);drinksHtml=drinkRows.length?topDrinkGrid(drinkRows):'<p class="az-note">No completed drink sales in this period.</p>';}
...
root.innerHTML='<div class="pz-h">📊 Sales Analytics</div><div class="az-note">Quota-safe historical summaries for the selected period.</div><div class="az-kpis">'+kpi('Net sales',peso(current.net),trend)+kpi('Transactions',String(current.orders))+kpi('Average sale',peso(aov))+'</div><div class="az-sec">Daily net sales</div><div class="pz-card">'+(days.length?days.map(function(day){return bar(fmtD(day),Number(current.days[day].net)||0,maxDay,peso0(current.days[day].net));}).join(''):'<p class="az-note">No completed sales in this period.</p>')+'</div><div class="az-sec">Sales by cashier</div><div class="pz-card az-cashier-card">'+cashierHtml+'</div><div class="az-sec">Weekly sales</div><div class="pz-card az-week-card">'+weeklyHtml+'</div><div class="az-sec">Top 12 drinks · YTD</div><div class="pz-card az-drinks-card">'+drinksHtml+'</div><div class="az-sec">Peak hours</div><div class="pz-card az-peak-card">'+peakHtml+'</div><div class="az-sec">Sales by day of week</div><div class="pz-card az-weekday-card">'+weekdayHtml+'</div>';}
...
function cashierPie(label,summary){var rows=A().reconcileCashierSales(summary),order=['maria','rya','alex','louize'],colors=['#55785b','#b58e50','#7c6b9d','#4e7890'],total=Math.max(0,Number(summary.net)||0),cursor=0,stops=[];order.forEach(function(key,index){var pct=total>0?Math.max(0,rows[key].net)/total*100:0,next=index===order.length-1?100:cursor+pct;stops.push(colors[index]+' '+cursor.toFixed(2)+'% '+next.toFixed(2)+'%');cursor=next;});var pie=total>0?'conic-gradient('+stops.join(',')+')':'var(--cr)';return '<div class="az-cashier-pie-panel"><div class="az-cashier-pie" style="--cashier-pie:'+pie+'"><div><b>'+peso0(total)+'</b><span>Total</span></div></div><div class="az-cashier-detail"><h3>'+esc(label)+'</h3>'+order.map(function(key,index){var row=rows[key],share=total>0?row.net/total*100:0,note=key==='maria'?'<small>Includes OWNER and unassigned sales</small>':'';return '<div class="az-cashier-legend"><i style="--cashier-color:'+colors[index]+'"></i><span>'+esc(row.name)+note+'</span><b>'+peso0(row.net)+'<small>'+share.toFixed(1)+'%</small></b></div>';}).join('')+'</div></div>';}
function topDrinkGrid(rows){var max=Math.max.apply(null,rows.map(function(row){return Number(row.net)||0;}).concat([1])),half=Math.ceil(rows.length/2),ordered=[];for(var i=0;i<half;i++){if(rows[i])ordered.push({row:rows[i],rank:i+1});if(rows[i+half])ordered.push({row:rows[i+half],rank:i+half+1});}return '<div class="az-drink-grid">'+ordered.map(function(entry){var row=entry.row,width=Math.max(2,Math.round((Number(row.net)||0)/max*100));return '<div class="az-drink-row"><span class="az-drink-rank">'+String(entry.rank).padStart(2,'0')+'</span><span class="az-drink-name">'+esc(row.name)+'</span><span class="az-drink-track"><i style="width:'+width+'%"></i></span><span class="az-drink-units">'+Number(row.units||0)+' cups</span><b>'+peso0(row.net)+'</b></div>';}).join('')+'</div>';}
function peakHourChart(rows){var max=Math.max.apply(null,rows.map(function(row){return Math.max(row.mtd,row.ytd);}).concat([1]));return '<div class="az-chart-key"><span class="az-key-mtd">MTD</span><span class="az-key-ytd">YTD</span></div><div class="az-peak-hours">'+rows.map(function(row){var mtd=Math.max(0,Math.round(row.mtd/max*100)),ytd=Math.max(0,Math.round(row.ytd/max*100));return '<div class="az-peak-row"><span>'+esc(row.label)+'</span><div><i><b class="az-peak-mtd" style="width:'+mtd+'%"></b></i><i><b class="az-peak-ytd" style="width:'+ytd+'%"></b></i></div><em>'+peso0(row.mtd)+'<br>'+peso0(row.ytd)+'</em></div>';}).join('')+'</div>';}
function weekdayLineChart(rows){var max=Math.max.apply(null,rows.map(function(row){return Math.max(row.mtd,row.ytd);}).concat([1])),top=Math.max(1000,Math.ceil(max/10000)*10000),left=72,right=828,topY=24,bottom=194,step=(right-left)/6,y=function(value){return bottom-(Math.max(0,Number(value)||0)/top)*(bottom-topY);},points=function(key){return rows.map(function(row,index){return(left+index*step).toFixed(1)+','+y(row[key]).toFixed(1);}).join(' ');},grid=[0,.25,.5,.75,1].map(function(part){var gy=y(top*part);return '<line x1="'+left+'" y1="'+gy+'" x2="'+right+'" y2="'+gy+'"></line><text x="'+(left-10)+'" y="'+(gy+4)+'" text-anchor="end">'+peso0(top*part)+'</text>';}).join(''),labels=rows.map(function(row,index){var x=left+index*step;return '<text x="'+x+'" y="220" text-anchor="middle">'+esc(row.label)+'</text><circle class="az-weekday-ytd-point" cx="'+x+'" cy="'+y(row.ytd)+'" r="4"></circle><circle class="az-weekday-mtd-point" cx="'+x+'" cy="'+y(row.mtd)+'" r="4"></circle>';}).join('');return '<div class="az-chart-key"><span class="az-key-mtd">MTD trend</span><span class="az-key-ytd">YTD trend</span></div><svg class="az-weekday-chart" viewBox="0 0 900 250" role="img" aria-label="MTD and YTD net sales trend from Sunday through Saturday"><g class="az-weekday-grid">'+grid+'</g><polyline class="az-weekday-ytd-line" points="'+points('ytd')+'"></polyline><polyline class="az-weekday-mtd-line" points="'+points('mtd')+'"></polyline><g class="az-weekday-axis">'+labels+'<text x="450" y="245" text-anchor="middle">Day of week</text><text transform="translate(16 112) rotate(-90)" text-anchor="middle">Net sales</text></g></svg>';}
...
function editPayoutMetadata(payoutId){
  ...
  window.AccazaFormDialog.run({title:'Edit payout information',subtitle:'Changing the payout date also updates every linked Finance Books and cash-ledger date. Amounts, linked orders, and the receiving account stay unchanged.',submitLabel:'Save information',busyLabel:'Saving…',fields:[
    {name:'payoutDate',label:'Actual payout date',type:'date',required:true,value:payout.payoutDate||''},
  ...
  ]},function(v){return a.setPlatformPayoutDate({payoutId:payoutId,payoutDate:v.payoutDate||'',platformStatementReference:v.platformStatementReference||'',depositReference:v.depositReference||'',notes:v.notes||''}).then(function(r){return(r&&r.data)||r||{};});}).then(function(d){(window.accazaToast||function(){})(d.financialEffect==='linked_dates_updated'?'Payout information saved. All linked Finance Books dates were updated.':'Payout information saved.','ok');}).catch(function(e){var m=String((e&&e.message)||(e&&e.code)||e);if(m.indexOf('cancelled')<0)alert('Could not save payout information: '+m);});
}
function payoutAuditOrders(p){
  if(Array.isArray(p.orderSnapshots)&&p.orderSnapshots.length)return{rows:p.orderSnapshots,immutable:true,missing:[]};
  var missing=[],rows=(p.orderIds||[]).map(function(id){var o=ordersMap[id]||archMap[id];if(!o){missing.push(id);return null;}var d=poGrabDeductions(o);return{orderId:id,platformRef:o.platformRef||o.id||id,orderDate:o.date||'',orderedAt:Number(o.timestamp)||0,gross:poGross(o),merchantPromo:d.merchantPromo,deliveryFeeDiscount:d.deliveryFeeDiscount,platformDiscount:Number(o.platformDiscount)||0,commission:Number(o.commission)||0,wht:Number(o.platformWht)||0,vat:Number(o.platformVat)||0,adsMarketing:Number(o.platformAdsMarketing)||0,marketingFee:Number(o.platformMarketingFee)||0,expectedNet:poNet(o)};}).filter(Boolean);
  return{rows:rows,immutable:false,missing:missing};
}
function payoutAuditContent(p,printMode){
  var resolved=payoutAuditOrders(p),rows=resolved.rows,total=Math.round(rows.reduce(function(sum,row){return sum+(Number(row.expectedNet)||0);},0)*100)/100,expected=Math.round((Number(p.expectedNet)||0)*100)/100,agrees=Math.abs(total-expected)<0.01,account=payoutCashAccounts[p.accountId]||{},defs={};varAccounts().forEach(function(x){defs[x.id]=x;});
  var orderRows=rows.length?rows.map(function(row){return '<tr><td>'+esc(row.orderDate||((row.orderedAt&&new Date(row.orderedAt).toLocaleDateString('en-PH'))||'—'))+'</td><td>'+esc(row.platformRef||row.orderId||'—')+'</td><td class="r">'+peso(row.gross)+'</td><td class="r">'+peso(row.merchantPromo)+'</td><td class="r">'+peso(row.commission)+'</td><td class="r">'+peso(row.deliveryFeeDiscount)+'</td><td class="r">'+peso(row.wht)+'</td><td class="r">'+peso(row.vat)+'</td><td class="r">'+peso(row.adsMarketing)+'</td><td class="r">'+peso(row.marketingFee)+'</td><td class="r">'+peso(row.expectedNet)+'</td></tr>';}).join(''):'<tr><td colspan="11">No retained order details are available.</td></tr>';
  var allocationIds=Object.keys(p.allocations||{}),allocationRows=allocationIds.length?allocationIds.map(function(id){var meta=(p.allocationMeta||{})[id]||defs[id]||{},ref=meta.sourceRef||(p.allocationRefs||{})[id]||'';return '<tr><td>'+esc(meta.name||id)+'</td><td>'+esc(meta.type||'expense')+'</td><td>'+esc(ref||'—')+'</td><td class="r">'+peso((p.allocations||{})[id])+'</td></tr>';}).join(''):'<tr><td colspan="4">No payout-level allocations.</td></tr>';
  var reversal=p.reversed?('<div class="az-note" style="border-color:#d8a38a;background:#fff5f0;"><b>Reversed</b> '+esc(p.reversalReason||'No reason retained')+' · '+esc(p.reversalApprovalId||'approval unavailable')+'</div>'):'';
  var legacy=!resolved.immutable?'<div class="az-note" style="border-color:#d8be74;background:#fff9e8;">Legacy payout: order details are reconstructed from current retained order records. New settlements preserve an immutable snapshot.</div>':'';
  var missing=resolved.missing.length?'<div class="az-note" style="border-color:#d58b8b;background:#fff3f3;">Missing source order(s): '+resolved.missing.map(esc).join(', ')+'. The payout and journal remain retained, but these legacy order details cannot be reconstructed.</div>':'';
  return '<div class="pz-card" style="margin:.45rem 0;background:var(--cr);">'
    +'<div style="display:flex;justify-content:space-between;gap:.6rem;align-items:center;flex-wrap:wrap;"><div><b>'+esc((p.channel==='grabfood'?'GrabFood':'FoodPanda')+' payout audit')+'</b><div style="font-size:.72rem;color:var(--tl);">Payout ID '+esc(p.id)+'</div></div>'+(printMode?'':'<button class="pz-btn sec" data-poprint="'+esc(p.id)+'">Print audit</button>')+'</div>'
    +'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:.45rem;margin:.7rem 0;font-size:.78rem;"><div><b>Status</b><br>'+(p.reversed?'Reversed':'Settled')+'</div><div><b>Record Date</b><br>'+esc(new Date(p.settledAt||0).toLocaleString('en-PH'))+'</div><div><b>Payout Date</b><br>'+esc(p.payoutDate||'—')+'</div><div><b>Statement period</b><br>'+esc((p.periodStart||'open')+' to '+(p.periodEnd||'open'))+'</div><div><b>Statement reference</b><br>'+esc(p.platformStatementReference||'—')+'</div><div><b>Bank reference</b><br>'+esc(p.depositReference||'—')+'</div><div><b>Receiving account</b><br>'+esc(account.name||p.accountId||'Not applicable')+'</div><div><b>Recorded / approved by</b><br>'+esc((p.by||p.actorUid||'—')+' / '+(p.approvedBy||p.approvalId||'—'))+'</div></div>'
    +legacy+missing+reversal
    +'<div style="overflow-x:auto;margin-top:.6rem;"><table class="pnl-tbl"><thead><tr><th>Date</th><th>Order #</th><th class="r">Gross</th><th class="r">Merchant promo</th><th class="r">Commission</th><th class="r">Delivery discount</th><th class="r">WHT</th><th class="r">VAT</th><th class="r">Ads</th><th class="r">Marketing fee</th><th class="r">Expected net</th></tr></thead><tbody>'+orderRows+'<tr class="tot"><td colspan="10">Order expected-net total</td><td class="r">'+peso(total)+'</td></tr></tbody></table></div>'
    +'<div class="az-sec" style="margin-top:.75rem;">Payout-level allocations</div><table class="pz-tbl"><thead><tr><th>Account</th><th>Type</th><th>Source</th><th class="r">Amount</th></tr></thead><tbody>'+allocationRows+'</tbody></table>'
    +'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:.45rem;margin-top:.7rem;"><div class="pz-card">Orders expected <b>'+peso(expected)+'</b></div><div class="pz-card">Actual payout <b>'+peso(p.actualPayout)+'</b></div><div class="pz-card">Variance <b>'+peso(p.variance)+'</b></div><div class="pz-card">Order audit <b style="color:'+(agrees?'#237a57':'#b22d2d')+';">'+(agrees?'Reconciled':'Difference '+peso(total-expected))+'</b></div></div>'
    +'<div style="margin-top:.65rem;font-size:.74rem;color:var(--tl);"><b>Finance links:</b> settlement '+esc(p.movementId||('payout_'+p.id))+' · deposit '+esc(p.depositMovementId||'none')+' · reversal '+esc(p.reversed?('reverse_payout_'+p.id):'none')+' · approval '+esc(p.approvalId||'—')+'</div></div>';
}
function printPayoutAudit(payoutId){var p=payoutsMap[payoutId];if(!p)return;var w=window.open('','_blank','width=1100,height=780');if(!w){alert('Allow pop-ups to print the payout audit.');return;}w.document.write('<!doctype html><html><head><title>Payout audit '+esc(payoutId)+'</title><style>body{font-family:Arial,sans-serif;padding:20px;color:#222}table{border-collapse:collapse;width:100%;font-size:11px;margin:8px 0}th,td{border:1px solid #bbb;padding:5px;text-align:left}.r{text-align:right}.pz-card,.az-note{border:1px solid #bbb;padding:8px;margin:6px 0}.az-sec{font-weight:bold;margin-top:12px}.tot{font-weight:bold}button{padding:7px 14px}@media print{button{display:none}}</style></head><body>'+payoutAuditContent(Object.assign({id:payoutId},p),true)+'<div style="text-align:center;margin-top:12px;"><button onclick="window.print()">Print</button></div></body></html>');w.document.close();}
function renderPayouts(){
  ...
  var html='<div class="pz-h">💱 Platform Payout Reconciliation</div>'
    ...
    +'<details style="margin-bottom:1rem;"><summary style="cursor:pointer;font-weight:600;color:var(--bd);">⚙️ Manage variance accounts</summary>'
      ...
      +'</div></details>'
    +'<div class="az-sec" style="display:flex;gap:.6rem;align-items:end;justify-content:space-between;flex-wrap:wrap;"><span>Settled payouts — '+esc(chLbl)+'</span><label style="font-size:.72rem;font-weight:400;">Search payout, reference or order #<input id="poAuditSearch" class="pz-in" value="'+esc(poAuditSearch)+'" placeholder="Search history" style="width:220px;margin-left:.35rem;"/></label></div>'
    +'<div class="pz-card"><table class="pnl-tbl"><thead><tr><th>Record Date</th><th>Payout date</th><th>Reference</th><th class="r">Expected</th><th class="r">Actual</th><th class="r">Variance</th><th class="r">Orders</th><th></th></tr></thead><tbody>'+histRows+'</tbody></table></div>';
  root.innerHTML=html;
  ...
  root.querySelectorAll('[data-poreview]').forEach(function(b){b.onclick=function(){var row=document.getElementById('poAudit_'+b.getAttribute('data-poreview')),open=row&&row.style.display!=='none';if(row)row.style.display=open?'none':'table-row';b.textContent=open?'Review':'Close review';};});
  root.querySelectorAll('[data-poprint]').forEach(function(b){b.onclick=function(){printPayoutAudit(b.getAttribute('data-poprint'));};});
  var auditSearch=document.getElementById('poAuditSearch');if(auditSearch)auditSearch.onchange=function(){poAuditSearch=this.value||'';renderPayouts();};
  root.querySelectorAll('[data-podate]').forEach(function(inp){inp.onchange=function(){var pid=inp.getAttribute('data-podate'),val=inp.value||'',old=payoutsMap[pid]&&payoutsMap[pid].payoutDate||'';var a=A();if(!a||!a.setPlatformPayoutDate){alert('Service unavailable. Refresh the portal.');inp.value=old;return;}if(!val){alert('Payout date is required.');inp.value=old;return;}inp.disabled=true;a.setPlatformPayoutDate({payoutId:pid,payoutDate:val}).then(function(r){var d=(r&&r.data)||r||{};if(payoutsMap[pid]){payoutsMap[pid].payoutDate=val;payoutsMap[pid].accountingOccurredAt=new Date(val+'T12:00:00+08:00').getTime();}(window.accazaToast||function(){})(d.financialEffect==='linked_dates_updated'?'Payout date and all linked Finance Books records updated.':'Payout date saved.','ok');}).catch(function(e){inp.value=old;alert('Could not save payout date: '+((e&&e.message)||e));}).finally(function(){inp.disabled=false;});};});
...
function autoAdjustInventoryReconciliation(rng,btn){
  ...
  A().postFinancialCommand({action:'inventory_reconciliation_adjustment',commandId:commandId,preview:true,date:rng.t,adjustmentPurpose:'current-reconciliation',classificationConfirmed:true}).then(function(r){r=r&&r.data?r.data:r||{};
    ...
    btn.textContent='Posting current variance…';return A().postFinancialCommand({action:'inventory_reconciliation_adjustment',commandId:commandId,date:rng.t,expectedFingerprint:r.fingerprint,adjustmentPurpose:'current-reconciliation',classificationConfirmed:true});
  }).then(function(r){if(!r){btn.disabled=false;btn.textContent='Post current count variance → Gain / (Loss)';return;}r=r&&r.data?r.data:r||{};alert((r.duplicate?'Existing':'New')+' current inventory reconciliation recorded.\nMovement: '+r.movementId+'\n\nFinance Books will refresh automatically.');}).catch(function(e){alert('Current inventory variance was not posted: '+((e&&e.message)||(e&&e.code)||e));btn.disabled=false;btn.textContent='Post current count variance → Gain / (Loss)';});
}
