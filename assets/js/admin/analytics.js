...
function renderPayouts(){
  ...
  var allocRows=accs.map(function(ac){var needsRef=ac.id==='va_refund'||ac.id==='va_refund_recovery';return '<tr><td>'+esc(ac.name)+' <span style="font-size:0.7rem;color:var(--tl);">('+ac.type+')</span>'+(needsRef?'<div style="font-size:0.68rem;color:var(--tl);margin-top:0.15rem;">Original Grab order or statement reference required when used</div>':'')+'</td><td style="width:220px;"><div style="display:flex;gap:0.3rem;align-items:center;"><input class="pz-in" type="number" min="0" step="any" data-alloc="'+esc(ac.id)+'" data-atype="'+ac.type+'" value="" placeholder="0" style="text-align:right;"/><button class="pz-btn sec" data-allocfill="'+esc(ac.id)+'" title="Put the remaining unallocated amount here to balance" style="padding:0.15rem 0.4rem;font-size:0.72rem;white-space:nowrap;">⚖ Fill</button></div>'+(needsRef?'<input class="pz-in" data-allocref="'+esc(ac.id)+'" maxlength="120" placeholder="e.g. GF-521 / Grab statement line" style="margin-top:0.3rem;font-size:0.74rem;"/>':'')+'</td></tr>';}).join('');
  ...
  var html='<div class="pz-h">💱 Platform Payout Reconciliation</div>'
    ...
    +'<div class="pz-card"><table class="pnl-tbl"><thead><tr><th>Settled</th><th>Payout date</th><th class="r">Expected</th><th class="r">Actual</th><th class="r">Variance</th><th class="r">Orders</th><th></th></tr></thead><tbody>'+histRows+'</tbody></table></div>';
  root.innerHTML=html;
  ...
  document.getElementById('poSettle').onclick=function(){
    ...
    if(!window.AccazaFormDialog){alert('Form service unavailable. Refresh the portal.');return;}
    var allocs={},allocationRefs={},missingAllocRef='';root.querySelectorAll('[data-alloc]').forEach(function(i){var id=i.getAttribute('data-alloc'),v=Number(i.value)||0;if(v){allocs[id]=v;var refInp=root.querySelector('[data-allocref="'+id+'"]'),refVal=String((refInp&&refInp.value)||'').trim();if((id==='va_refund'||id==='va_refund_recovery')&&!refVal)missingAllocRef=((varAcctMap[id]||{}).name||id);if(refVal)allocationRefs[id]=refVal;}});if(missingAllocRef){alert('Enter the original Grab order or statement reference for '+missingAllocRef+'.');return;}
    var pid=uid('po_');
...
function inventoryReconciliationHtml(recon,ready,history){
  ...
  var action=!recon.balanced&&recon.unmappedCount===0&&Math.abs(recon.clearingBalance)<0.005?'<div style="margin-top:.75rem;"><button class="pz-btn" id="inventoryOpeningBalanceBtn">Post one-time opening inventory balance</button><div class="az-note" style="margin-top:.35rem;">Server recalculates every amount, records the actor, and prevents a second opening-balance post.</div></div>':'';
...
function postInventoryOpeningBalance(rng,btn){
  ...
  A().postFinancialCommand({action:'inventory_opening_balance',commandId:commandId,preview:true,date:rng.t}).then(function(r){r=r&&r.data?r.data:r||{};if(r.alreadyPosted)throw new Error('The inventory opening balance has already been posted.');var active=(r.rows||[]).filter(function(x){return Math.abs(Number(x.difference)||0)>=.005;}),detail=active.map(function(x){return x.code+' '+(x.difference>0?'Debit ':'Credit ')+peso(Math.abs(x.difference));}).join('\n'),offset=(r.totalDifference>=0?'Credit ':'Debit ')+peso(Math.abs(r.totalDifference));var message='Post the ONE-TIME opening inventory balance?\n\nStock value: '+peso(r.totalStock)+'\nBooks before: '+peso(r.totalBooks)+'\nAdjustment: '+peso(r.totalDifference)+'\n\n'+detail+'\n3900 Opening Balance / Retained Earnings '+offset+'\n\nThis creates a shared immutable Finance movement and cannot be posted twice.';if(!confirm(message)){btn.disabled=false;btn.textContent='Post one-time opening inventory balance';return null;}btn.textContent='Posting…';return A().postFinancialCommand({action:'inventory_opening_balance',commandId:commandId,date:rng.t,expectedDifference:r.totalDifference});}).then(function(r){if(!r)return;r=r&&r.data?r.data:r||{};alert('Opening inventory balance posted.\nAdjustment: '+peso(r.adjustment)+'\nMovement: '+r.movementId+'\n\nFinance Books will refresh automatically.');}).catch(function(e){alert('Opening inventory balance was not posted: '+((e&&e.message)||(e&&e.code)||e));btn.disabled=false;btn.textContent='Post one-time opening inventory balance';});
}
