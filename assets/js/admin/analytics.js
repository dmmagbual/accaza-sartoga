...
function renderPayouts(){
  ...
  var ordRows=inRange.length?inRange.map(function(e,i){var o=e.o,pr=o.platformRef||o.id||e.key;return '<tr><td style="text-align:center;"><input type="checkbox" data-poinc="'+i+'" checked/></td><td>'+esc(o.date||'')+'</td><td><button type="button" data-pocorrect="'+esc(pr)+'" class="pz-btn sec" style="padding:0.18rem 0.48rem;font-size:0.75rem;border-color:#b07a2b;color:#80520f;" title="Correct this unsettled order before settlement">'+esc(pr)+'</button></td><td class="r">'+peso(poGross(o))+'</td><td class="r">'+peso(Number(o.commission)||0)+'</td><td class="r">'+peso(poNet(o))+'</td><td class="r"><button class="pz-btn warn" data-povoid="'+esc(o.id||e.key)+'" data-povg="'+(poGross(o))+'" style="padding:0.15rem 0.5rem;font-size:0.72rem;" title="Void this order (e.g. a duplicate)">Void</button></td></tr>';}).join(''):'<tr><td colspan="7" class="az-note" style="padding:0.7rem;">No unsettled '+esc(chLbl)+' orders in this range.</td></tr>';
  var allocRows=accs.map(function(ac){return '<tr><td>'+esc(ac.name)+' <span style="font-size:0.7rem;color:var(--tl);">('+ac.type+')</span></td><td style="width:190px;"><div style="display:flex;gap:0.3rem;align-items:center;"><input class="pz-in" type="number" step="any" data-alloc="'+esc(ac.id)+'" data-atype="'+ac.type+'" value="" placeholder="0" style="text-align:right;"/><button class="pz-btn sec" data-allocfill="'+esc(ac.id)+'" title="Put the remaining unallocated amount here to balance" style="padding:0.15rem 0.4rem;font-size:0.72rem;white-space:nowrap;">⚖ Fill</button></div></td></tr>';}).join('');
