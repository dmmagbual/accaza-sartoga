...
function renderDailyReport(){
  ...
  function build(payouts,shiftRows,chan,byMethod,items,txns,refundsTot,netTot,sales,shiftLoadError,reconciliation){
    ...
    var chRows=['instore','grabfood','foodpanda','online'].map(function(c){var x=chan[c];if(!x.tx)return '';return '<tr><td>'+x.lbl+'</td><td class="r">'+x.tx+'</td><td class="r">'+peso(x.gross)+'</td><td class="r">'+(x.disc?'−'+peso(x.disc):'—')+'</td><td class="r">'+(x.refund?'−'+peso(x.refund):'—')+'</td><td class="r">'+(x.comm?'−'+peso(x.comm):'—')+'</td><td class="r">'+peso(x.net)+'</td></tr>';}).join('');
    ...
    var reconRows='<tr><td>Qualifying sales</td><td class="r">'+reconciliation.sales+'</td><td>'+reconStatus('Reconciled')+'</td><td>Same sale population feeds every section</td></tr>'
      +'<tr><td>Net sales · revenue basis</td><td class="r">'+peso(reconciliation.channelNet)+'</td><td>'+reconStatus('Reconciled')+'</td><td>Gross − discounts − refunds</td></tr>'
      +'<tr><td>Payment tender before refunds</td><td class="r">'+peso(reconciliation.tenderBeforeRefunds)+'</td><td>'+reconStatus('Explained difference')+'</td><td>Custody/tender basis</td></tr>'
      +'<tr><td>Less: refunds</td><td class="r">−'+peso(reconciliation.refunds)+'</td><td></td><td>Refunds reduce tender and revenue</td></tr>'
      +'<tr><td>Net tender after refunds</td><td class="r">'+peso(reconciliation.netTender)+'</td><td>'+reconStatus(reconciliation.tenderStatus)+'</td><td>Net tender − net sales = '+signed(reconciliation.tenderVariance)+'</td></tr>'
      +'<tr><td>Tender vs revenue variance</td><td class="r">'+signed(reconciliation.tenderVariance)+'</td><td>'+reconStatus(reconciliation.tenderStatus)+'</td><td>Different bases; investigate only if unexplained</td></tr>'
      +'<tr><td>Itemized line sales</td><td class="r">'+peso(reconciliation.itemizedLineSales)+'</td><td></td><td>Only orders with line-level detail</td></tr>'
      +'<tr><td>Less: discounts on itemized orders</td><td class="r">−'+peso(reconciliation.itemizedDiscounts)+'</td><td></td><td>Allocated to orders with item detail</td></tr>'
      +'<tr><td>Less: refunds on itemized orders</td><td class="r">−'+peso(reconciliation.itemizedRefunds)+'</td><td></td><td>Allocated to orders with item detail</td></tr>'
      +'<tr><td>Add: unitemized net sales</td><td class="r">+'+peso(reconciliation.unitemizedNet)+'</td><td>'+reconStatus(reconciliation.unitemizedOrders?'Explained difference':'Reconciled')+'</td><td>'+reconciliation.unitemizedOrders+' order(s) without item detail</td></tr>'
      +'<tr><td>Add: line-basis difference</td><td class="r">'+signed(reconciliation.lineBasisDifference)+'</td><td>'+reconStatus(Math.abs(reconciliation.lineBasisDifference)<.005?'Reconciled':'Explained difference')+'</td><td>Tax/rounding/line-vs-order basis bridge</td></tr>'
      +'<tr class="tot"><td>Itemized bridge total</td><td class="r">'+peso(reconciliation.itemBridgeTotal)+'</td><td>'+reconStatus(reconciliation.itemStatus)+'</td><td>Should equal net sales</td></tr>'
    ...
    var reconciliationHtml='<div class="az-sec">Sales Reconciliation</div><section class="pz-card dr-detail-card" id="drReconciliation" style="margin-bottom:.7rem;"><div class="az-note" style="margin-bottom:.65rem;"><b>Board control:</b> the same qualifying sales population is shown on three different bases. Revenue, tender, and item detail are bridged below so no variance is hidden.</div><div style="overflow-x:auto;"><table class="pz-tbl"><thead><tr><th>Control</th><th class="r">Amount</th><th>Status</th><th>Explanation</th></tr></thead><tbody>'+reconRows+'</tbody></table></div>'+(reconIssueRows?'<details style="margin-top:.65rem;"><summary><b>Variance detail ('+reconciliation.issues.length+' item(s))</b></summary><div style="overflow:auto;"><table class="pz-tbl"><thead><tr><th>Reason</th><th>Order</th><th>Date &amp; time</th><th class="r">Variance</th></tr></thead><tbody>'+reconIssueRows+'</tbody></table></div></details>':'<div class="az-note" style="margin-top:.65rem;color:#176b3a;"><b>No unexplained differences within ₱0.01.</b></div>')+'</section>';
    ...
    var html='<div class="pz-h">📆 Daily Report</div>'
      ...
      +closeControlHtml(d)
      +reconciliationHtml
      +'<div class="az-sec">Sales by channel</div><div class="pz-card dr-detail-card" id="drChannels" style="margin-bottom:0.7rem;"><div class="az-note" style="margin-bottom:.55rem;">Revenue basis: gross sales less customer discounts and refunds. Commission is shown separately as a platform expense and is not deducted from revenue.</div><div style="overflow-x:auto;"><table class="pz-tbl"><thead><tr><th>Channel</th><th class="r">Tx</th><th class="r">Gross</th><th class="r">Discounts</th><th class="r">Refunds</th><th class="r">Commission</th><th class="r">Net</th></tr></thead><tbody>'+(chRows||'<tr><td colspan="7" style="color:var(--tl);">No sales this day.</td></tr>')+'<tr class="tot"><td>Total</td><td class="r">'+channelTxTotal+'</td><td class="r">'+peso(channelGrossTotal)+'</td><td class="r">−'+peso(channelDiscountTotal)+'</td><td class="r">−'+peso(channelRefundTotal)+'</td><td class="r">−'+peso(channelCommissionTotal)+'</td><td class="r">'+peso(netTot)+'</td></tr></tbody></table></div></div>'
      +'<div class="az-sec">Payment tender by method (before refunds)</div><div class="pz-card dr-detail-card" id="drMethods" style="margin-bottom:0.7rem;"><table class="pz-tbl"><tbody>'+methodRows+'<tr class="tot"><td>Tender before refunds</td><td class="r">'+peso(paymentTotal)+'</td></tr><tr><td>Less: refunds</td><td class="r">−'+peso(reconciliation.refunds)+'</td></tr><tr class="tot"><td>Net tender after refunds</td><td class="r">'+peso(reconciliation.netTender)+'</td></tr></tbody></table><div class="az-note">Tender is a custody measure. The reconciliation above explains why it can differ from revenue net sales.</div></div>'
...
function printDailyReport(d,X){
  ...
  var ch=['instore','grabfood','foodpanda','online'].map(function(c){var x=X.chan[c];if(!x.tx)return '';return '<tr><td>'+x.lbl+' ('+x.tx+')<br><small>Gross '+peso(x.gross)+' · Disc −'+peso(x.disc)+' · Refund −'+peso(x.refund)+' · Comm −'+peso(x.comm)+'</small></td><td style="text-align:right;">'+peso(x.net)+'</td></tr>';}).join('');
  ...
  w.document.write('<html><head><title>Daily Report '+esc(d)+'</title><style>*{font-family:monospace;font-size:12px;color:#000;}body{padding:10px;}h2,h3{text-align:center;margin:2px 0;}table{width:100%;border-collapse:collapse;}td{padding:2px 0;}hr{border:none;border-top:1px dashed #000;}@media print{button{display:none;}}</style></head><body>'
    ...
    +'<div><b>Sales reconciliation</b></div><table>'+recRows+(recIssues?recIssues:'<tr><td>No unexplained differences within ₱0.01.</td><td></td></tr>')+'</table><hr>'
    ...
    +'<div><b>Payment tender by method (before refunds)</b></div><table>'+(me||'<tr><td>None</td></tr>')+'<tr><td><b>Tender before refunds</b></td><td style="text-align:right;"><b>'+peso(methodTotal)+'</b></td></tr><tr><td>Less: refunds</td><td style="text-align:right;">−'+peso(rec.refunds)+'</td></tr><tr><td><b>Net tender after refunds</b></td><td style="text-align:right;"><b>'+peso(rec.netTender)+'</b></td></tr></table><hr>'
...
function exportDailyXlsx(d,X){
  ...
  var ch=[['Channel','Tx','Gross','Discount','Refunds','Commission','Net']];['instore','grabfood','foodpanda','online'].forEach(function(c){var x=X.chan[c];ch.push([x.lbl,x.tx,x.gross,x.disc,x.refund,x.comm,x.net]);});
  ...
  ch.push(['TOTAL',ch.slice(1).reduce(function(s,r){return s+(Number(r[1])||0);},0),ch.slice(1).reduce(function(s,r){return s+(Number(r[2])||0);},0),ch.slice(1).reduce(function(s,r){return s+(Number(r[3])||0);},0),ch.slice(1).reduce(function(s,r){return s+(Number(r[4])||0);},0),ch.slice(1).reduce(function(s,r){return s+(Number(r[5])||0);},0),X.netTot]);
  ...
  var wb=XLSX.utils.book_new();[['Shifts',sf],['Reconciliation',re],['Channels',ch],['Methods',me],['Items',it],['Transactions',tx],['Expenses',ex]].forEach(function(p){XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet(p[1]),p[0]);});XLSX.writeFile(wb,'daily-report-'+d+'.xlsx');
}
function pnlFor(mk){
