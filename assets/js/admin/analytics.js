...
function refreshCompactAnalytics(){if(!compactAnalytics.months)return;compactAnalytics.current=A().addLiveSales(A().summarizeHistoricalSales(compactAnalytics.months,compactAnalytics.currentPeriod),Object.values(ordersMap),compactAnalytics.currentPeriod,window.AccazaSales);compactAnalytics.previous=A().summarizeHistoricalSales(compactAnalytics.months,compactAnalytics.previousPeriod);}
function loadCompactAnalytics(from,to){var span=to-from,key=from+':'+to;if(compactAnalytics.key===key&&(compactAnalytics.loading||compactAnalytics.current))return;compactAnalytics={key:key,loading:true,error:'',current:null,previous:null,months:null,currentPeriod:{startAt:from,endAt:to-1},previousPeriod:{startAt:from-span,endAt:from-1},schemaVersion:0,request:compactAnalytics.request+1};var request=compactAnalytics.request;A().readHistoricalSalesRollup({from:compactMonth(compactAnalytics.previousPeriod.startAt),to:compactMonth(compactAnalytics.currentPeriod.endAt)}).then(function(result){if(request!==compactAnalytics.request)return;if(!result||result.ready!==true||Number(result.schemaVersion)<2)throw new Error('Historical report summaries need one controlled maintenance run.');compactAnalytics.months=result.months||{};compactAnalytics.schemaVersion=Number(result.schemaVersion)||2;refreshCompactAnalytics();}).catch(function(error){if(request===compactAnalytics.request)compactAnalytics.error=String(error&&error.message||error);}).finally(function(){if(request===compactAnalytics.request){compactAnalytics.loading=false;if(isTab('analytics'))renderAnalytics();}});}
...
function renderCompactAnalyticsV5(current,previous){var root=document.getElementById('analyticsRoot');if(!root)return;var days=Object.keys(current.days||{}).sort(),weekly={},hours=current.hours||{},items=compactDrinkItems(current),maxDay=Math.max.apply(null,days.map(function(day){return Number(current.days[day].net)||0;}).concat([1]));days.forEach(function(day){var week=compactWeekKey(day);weekly[week]=(weekly[week]||0)+(Number(current.days[day].net)||0);});var weeks=Object.keys(weekly).sort(),maxWeek=Math.max.apply(null,weeks.map(function(week){return weekly[week];}).concat([1])),hourKeys=Object.keys(hours).sort(function(a,b){return Number(a)-Number(b);}),maxHour=Math.max.apply(null,hourKeys.map(function(hour){return Number(hours[hour].net)||0;}).concat([1])),trend=previous.net>0?(current.net-previous.net)/previous.net*100:(current.net>0?100:0),aov=current.orders?current.net/current.orders:0;
root.innerHTML='<div class="pz-h">📊 Sales Analytics</div><div class="az-note">Quota-safe historical summaries for the selected period.</div><div class="az-kpis">'+kpi('Net sales',peso(current.net),trend)+kpi('Transactions',String(current.orders))+kpi('Average sale',peso(aov))+'</div><div class="az-sec">Daily net sales</div><div class="pz-card">'+(days.length?days.map(function(day){return bar(fmtD(day),Number(current.days[day].net)||0,maxDay,peso0(current.days[day].net));}).join(''):'<p class="az-note">No completed sales in this period.</p>')+'</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;" class="pz-posgrid"><div><div class="az-sec">Weekly sales</div><div class="pz-card">'+(weeks.length?weeks.map(function(week){return bar('Week of '+fmtD(week),weekly[week],maxWeek,peso0(weekly[week]));}).join(''):'<p class="az-note">No weekly sales recorded.</p>')+'</div></div><div><div class="az-sec">Peak hours</div><div class="pz-card">'+(hourKeys.length?hourKeys.map(function(hour){var h=Number(hour);return bar((h%12||12)+(h<12?'am':'pm'),Number(hours[hour].net)||0,maxHour,peso0(hours[hour].net));}).join(''):'<p class="az-note">Peak-hour data is being prepared.</p>')+'</div></div></div><div class="az-sec">Top 10 drinks by net revenue</div><div class="pz-card">'+(items.length?'<table class="pz-tbl"><thead><tr><th>Drink</th><th>Units</th><th class="r">Net revenue</th></tr></thead><tbody>'+items.slice(0,10).map(function(item){return'<tr><td>'+esc(item.name)+'</td><td>'+Number(item.units).toLocaleString('en-PH')+'</td><td class="r">'+peso(item.net)+'</td></tr>';}).join('')+'</tbody></table>':'<p class="az-note">No completed drink sales in this period.</p>')+'</div>';}
