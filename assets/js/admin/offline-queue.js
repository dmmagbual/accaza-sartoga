(function(global){
  ...
  function closeReadiness(){return summary().then(function(s){var outstanding=Number(s.pending||0)+Number(s.syncing||0)+Number(s.failed||0);return{ready:outstanding===0,outstanding:outstanding,pending:Number(s.pending||0),syncing:Number(s.syncing||0),failed:Number(s.failed||0)};});}
  ...
  function flush(sync,onChange){
    ...
    return all().then(function(rows){var work=rows.filter(function(r){return r.status==='pending'||r.status==='failed'||r.status==='syncing';});var chain=Promise.resolve(),result={synced:0,failed:0};work.forEach(function(row){chain=chain.then(function(){return patch(row.id,{status:'syncing',attempts:(Number(row.attempts)||0)+1,lastError:''}).then(function(){if(onChange)onChange();return sync({transactionId:row.id,order:row.order,drawerDelta:row.drawerDelta});}).then(function(response){return removeIds([row.id]).catch(function(){return patch(row.id,{status:'synced',syncedAt:Date.now(),serverResult:(response&&response.data)||response||null}).catch(function(){});}).then(function(){result.synced++;if(onChange)onChange();});}).catch(function(error){var msg=String((error&&error.message)||error||'Sync failed').slice(0,500);return patch(row.id,{status:'failed',lastError:msg,failedAt:Date.now()}).then(function(){result.failed++;if(onChange)onChange();});});});});return chain.then(function(){return compactSynced().then(function(){return result;});});}).finally(function(){flushing=false;if(onChange)onChange();});
  }
  global.AccazaOfflineQueue={open:open,enqueue:enqueue,all:all,summary:summary,closeReadiness:closeReadiness,flush:flush,retry:function(id){return patch(id,{status:'pending',lastError:''});},drawerDelta:drawerDelta,storageHealth:storageHealth,isQuotaError:isQuotaError,compactSynced:compactSynced};
})(window);
