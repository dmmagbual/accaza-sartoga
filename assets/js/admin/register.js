...
function showZ(shift,z,existingWindow){
  ...
  var saleRows=saleList.map(function(o){var tag=o.voided?' [VOID]':(Number(o.refundAmount)>0?' [R '+peso(o.refundAmount)+']':o.preCompletionCashRefund?' [Cash refund '+peso(o.preCompletionCashRefund.amount)+']':'');var ch=(o.channel&&o.channel!=='instore')?(' '+esc(o.channel==='grabfood'?'GF':'FP')):'';return '<tr><td>'+esc(o.id)+'<div style="font-size:9px;">'+esc(saleDateTime(o))+ch+'</div></td><td style="text-align:right;">'+peso(o.total)+esc(tag)+'</td></tr>';}).join('');
