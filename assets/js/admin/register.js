...
function renderPayMethods(){
  ...
  a.get(a.ref(a.db,'posSettings')).then(function(s){
    ...
    var add=document.getElementById('pmAdd');if(add)add.onclick=function(){var nm=(document.getElementById('pmNew').value||'').trim();if(!nm)return;if(pm.some(function(x){return String(x.name).toLowerCase()===nm.toLowerCase();})){alert('That method already exists.');return;}pm.push({name:nm,active:true,cash:false,verificationPolicy:defaultVerificationPolicy(nm)});save(pm);};
  });
}
