...
function renderDiscrepancies(){
  ...
  var rows=all.length?all.map(function(d){
    ...
    return '<tr><td>'+when+'</td><td>'+(d.kind==='cash'?'💵 Cash':'📦 Inventory')+'</td><td>'+what+'</td><td style="text-align:right;">'+exp+'</td><td style="text-align:right;">'+act+'</td><td style="text-align:right;font-weight:600;">'+vr+'</td><td>'+badge+'</td><td>'+esc(d.staff||'')+'</td><td>'+(d.status==='reviewed'?('<span style="font-size:0.72rem;color:var(--tl);">✓ '+esc(d.reviewedBy||'')+(d.treatment?' · '+esc(String(d.treatment).replace(/_/g,' ')):'')+(d.note?' — '+esc(d.note):'')+'</span><div style="margin-top:.35rem;"><button class="pz-btn sec" data-dreopen="'+esc(d.id)+'" style="padding:0.2rem 0.55rem;">Correct reviewed record</button></div>'):'<div style="font-size:.68rem;color:#8a5a00;margin-bottom:.2rem;">Manager revalidation required</div><button class="pz-btn ok" data-drev="'+esc(d.id)+'" style="padding:0.2rem 0.55rem;">Resolve</button>')+'</td></tr>';
  ...
  root.querySelectorAll('[data-dreopen]').forEach(function(b){b.onclick=function(){reopenDiscrepancy(b.getAttribute('data-dreopen'));};});
...
function reopenDiscrepancy(id){var d=discMap[id],a=A();if(!d||d.status!=='reviewed')return;if(!a.reopenDiscrepancy||!a.managerApproval){alert('The record-correction service is not ready. Refresh and try again.');return;}var cash=d.kind==='cash';F().run({title:'Correct reviewed record',subtitle:cash?'The original cash variance will reopen only if no Finance correction, custody settlement, payable, or linked journal exists. Reopening never creates cash.':'The original inventory discrepancy will reopen only if no downstream treatment is linked. Reopening never changes stock or Finance.',submitLabel:'Reopen for correct treatment',busyLabel:'Checking links and reopening…',fields:[{name:'reason',label:'Why was this record reviewed in error?',type:'textarea',required:true,maxLength:500,placeholder:cash?'e.g. ₱270 on 28 Aug was a cash-count entry error; no correcting journal exists.':'State the corrected facts and source evidence.'}]},function(v){return a.managerApproval('reopen_discrepancy',id,cash?Math.abs(Number(d.variance)||0):null,v.reason).then(function(ap){return a.reopenDiscrepancy({discrepancyId:id,reason:v.reason,approvalId:ap.approvalId});});}).then(function(){alert('Record reopened. Select Resolve and record the correct treatment; the original record remains visible in the audit trail.');}).catch(function(e){if(String((e&&e.code)||e).indexOf('cancelled')<0)alert('Could not reopen record: '+((e&&e.message)||e));});}
function cashDifferenceCase(id,d){
  ...
  function detailFields(t,amount){var date=window.AccazaDate.key();
    if(t==='cash_recovered')return[{name:'destination',label:'Where was the recovered cash placed?',type:'select',required:true,value:'undeposited',options:cashOpts()},{name:'correctionMovementId',label:'Existing Finance journal that recorded this recovery',type:'select',required:true,options:recoveryJournalOpts(amount),help:'Select the journal already posted for this cash. Admin custody will be linked without posting another Finance entry.'},{name:'date',label:'Recovery date',type:'date',required:true,value:date}];
