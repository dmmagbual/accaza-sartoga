...
function renderPetty(){
    ...
    +'<div style="display:flex;gap:1rem;flex-wrap:wrap;margin-bottom:1rem;">'
      +'<div class="pz-card" style="flex:2;min-width:280px;"><div style="font-weight:600;color:var(--bd);margin-bottom:0.5rem;">Record a cash payment</div><p class="pz-sub" style="margin:-0.25rem 0 0.6rem;">For costs with no itemised supplier invoice, and for paying a supplier before delivery. Goods or an itemised invoice belong in Purchases; a bill already raised is settled in Finance Books → Payables.</p>'
        +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;">'
          ...
          +'<div id="pvSupplierWrap" style="display:none;"><span class="pz-lbl">Supplier</span><div style="display:flex;gap:.35rem;"><select class="pz-in" id="pvSupplier">'+cashPaymentSupplierOptions('')+'</select><button type="button" class="pz-btn sec" id="pvNewSupplier">＋ New supplier</button></div><div class="az-note">Select the shared supplier record used later in Purchases.</div><div style="margin-top:.5rem;"><span class="pz-lbl">Paid from</span><select class="pz-in" id="pvFundAcct">'+cashPaymentFundingOptions('undeposited')+'</select><div class="az-note">Pick the real account the money left from. Undeposited Collection is counted at shift close; other accounts (GCash, bank, Cash on Hand) are not and post straight to that account\'s balance.</div></div></div>'
  ...
  function syncPettyCategory(){if(!pt||!pc)return;var note=document.getElementById('pvEvidenceNote'),supplierWrap=document.getElementById('pvSupplierWrap'),requesterWrap=document.getElementById('pvRequesterWrap'),isAdvance=pt.value==='purchase_advance';if(pt.value==='expense'){pc.disabled=false;pc.innerHTML=catOpts;pc.value=lastExpenseCat;pc.title='Select the Finance Books expense category.';}else{lastExpenseCat=PETTY_CATS.some(function(x){return x.id===pc.value;})?pc.value:lastExpenseCat;pc.disabled=true;if(pt.value==='owner_withdrawal'){pc.innerHTML='<option value="owner_draw">Owner\'s Drawings (3100) — not an expense</option>';pc.title="Posts automatically to Owner's Drawings (3100).";}else{pc.innerHTML='<option value="purchase_allocation">Purchases — pending inventory allocation</option>';pc.title='Itemize and allocate this payment later in Purchases.';}}if(supplierWrap)supplierWrap.style.display=isAdvance?'block':'none';if(requesterWrap)requesterWrap.style.display=isAdvance?'none':'block';if(note)note.textContent=isAdvance?'Proof of payment is optional. Without it, the purchase purpose is retained as the manager-reviewed audit evidence.':'No attachment? Enter a clear explanation above for manager approval.';}
...
function createVoucher(){
  ...
  var fundingAccountId=transactionType==='purchase_advance'?(fv('pvFundAcct')||'undeposited'):'';
...
function renderOps(){
  ...
  html+='<div class="az-sec">Recent sales — void / refund</div><div class="pz-card"><table class="pz-tbl"><thead><tr><th>Order</th><th>Time</th><th>Total</th><th>Status</th><th></th></tr></thead><tbody>'
    +(recent.length?recent.map(function(o){var st=o.voided?'<span style="color:#e63946;">VOID</span>':(Number(o.refundAmount)>0?'<span style="color:#e67e00;">Refunded '+peso(o.refundAmount)+'</span>':o.preCompletionCashRefund?'<span style="color:#e67e00;">Cash refund '+peso(o.preCompletionCashRefund.amount)+'</span>':'OK'),electronic=directRows(o).length&&!paysOf(o).some(function(p){return String(p.method||'').toLowerCase()==='cash';}),i=(o.channel||'instore')==='instore';return '<tr><td>'+esc(o.id)+'<div style="font-size:0.7rem;color:var(--tl);">'+esc((o.staff||''))+'</div></td><td>'+esc(o.time||'')+'</td><td>'+peso(o.total)+'</td><td>'+st+'</td><td style="white-space:nowrap;">'+(o.voided?'':(electronic&&i?'<button class="pz-btn ok" style="padding:0.2rem 0.5rem;" data-paid-change="'+o.id+'">Change paid order</button> ':'')+'<button class="pz-btn sec" style="padding:0.2rem 0.5rem;" data-refund="'+o.id+'">Refund</button> <button class="pz-btn warn" style="padding:0.2rem 0.5rem;" data-void="'+o.id+'">Void</button>')+'</td></tr>';}).join(''):'<tr><td colspan="5" class="az-note" style="padding:0.8rem;">'+(activeShift?'No sales in this shift yet.':'No open shift — a shift’s sales show here while it’s open and clear when it closes.')+'</td></tr>')
...
function changePaidOrder(oid){
  ...
  if(!activeShift||o.shiftId!==activeShift.id)return alert('Use the manager refund for an earlier shift.');
  if(['cashier_verified','manager_validated','confirmed'].indexOf(o.paymentStatus)<0)return alert('Verify the payment first.');
  var n=Math.round(((+o.total||0)-(+o.refundAmount||0))*100)/100,s=o.status==='Archived'?o.prevStatus:o.status,done=s==='Completed'||s==='Received',f=[{name:'correctedTotal',label:'Corrected order total',type:'number',required:true,min:0,step:.01,value:n,validate:function(v){return +v>=0&&+v<n?'':'Enter less than '+peso(n)+'.';}},{name:'reason',label:'What changed?',type:'textarea',required:true,maxLength:300},{name:'customerAcknowledgement',label:'Customer acknowledgement',required:true,maxLength:160,help:'Name, signature, or receipt reference.'},{name:'confirmed',label:'I will hand the calculated cash refund to the customer',type:'checkbox',required:true}];
  if(done)f.push({name:'inventoryOutcome',label:'Prepared item outcome',type:'select',required:true,options:[{value:'no_stock_return',label:'Used or discarded — no stock return'},{value:'separate_inventory_review',label:'Returned — review stock separately'}]});
  F().run({title:'Paid order · cash refund',subtitle:oid+' · '+(done?'processed':'not processed'),submitLabel:'Post refund',busyLabel:'Posting…',fields:f},function(v){var c=Math.round(+v.correctedTotal*100)/100,a=Math.round((n-c)*100)/100;if(!confirm('Hand '+peso(a)+' cash to the customer?'))throw Error('cancelled');return A().processOrderAdjustment({action:'cash_order_change_refund',orderId:oid,correctedTotal:c,reason:v.reason,customerAcknowledgement:v.customerAcknowledgement,inventoryOutcome:v.inventoryOutcome||''}).then(function(r){var x=(r&&r.data)||r||{},cash=+x.amount||a;if(denomTrackingOnR()){var m=makeChangeD(cash,drawerNow());saveDrawer(subD(drawerNow(),m.denoms));if(!m.ok)alert('Count and reconcile the drawer now.');}window.__posLog('cash-order-change-refund',oid,peso(cash)+' · '+v.reason);});}).then(function(){alert('Refund posted to register cash and Finance Books.');}).catch(function(e){if(String(e&&e.message).indexOf('cancelled')<0)alert((e&&e.message)||e);});
}
