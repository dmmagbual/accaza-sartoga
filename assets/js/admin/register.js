...
function renderOps(){
  ...
  html+='<div class="az-sec">Recent sales — void / refund</div><div class="pz-card"><table class="pz-tbl"><thead><tr><th>Order</th><th>Time</th><th>Total</th><th>Status</th><th></th></tr></thead><tbody>'
    +(recent.length?recent.map(function(o){var st=o.voided?'<span style="color:#e63946;">VOID</span>':(Number(o.refundAmount)>0?'<span style="color:#e67e00;">Refunded '+peso(o.refundAmount)+'</span>':o.preCompletionCashRefund?'<span style="color:#e67e00;">Cash refund '+peso(o.preCompletionCashRefund.amount)+'</span>':'OK'),pay=paysOf(o),method=pay.length===1?String(pay[0].paymentMethod||pay[0].method||'').split(' · ')[0].trim().toLowerCase():'',supported=method==='gcash'||method==='bank transfer',i=(o.channel||'instore')==='instore',canChange=supported&&i&&o.preparationStatus==='not_prepared'&&!o.completedOrderCorrectionId&&!Number(o.refundAmount);return '<tr><td>'+esc(o.id)+'<div style="font-size:0.7rem;color:var(--tl);">'+esc((o.staff||''))+'</div></td><td>'+esc(o.time||'')+'</td><td>'+peso(o.total)+'</td><td>'+st+'</td><td style="white-space:nowrap;">'+(o.voided?'':(canChange?'<button class="pz-btn ok" style="padding:0.2rem 0.5rem;" data-paid-change="'+o.id+'">Change completed order</button> ':'')+'<button class="pz-btn sec" style="padding:0.2rem 0.5rem;" data-refund="'+o.id+'">Refund</button> <button class="pz-btn warn" style="padding:0.2rem 0.5rem;" data-void="'+o.id+'">Void</button>')+'</td></tr>';}).join(''):'<tr><td colspan="5" class="az-note" style="padding:0.8rem;">'+(activeShift?'No sales in this shift yet.':'No open shift — a shift’s sales show here while it’s open and clear when it closes.')+'</td></tr>')
