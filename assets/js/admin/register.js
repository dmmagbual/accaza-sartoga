...
function renderPetty(){
  ...
  var pvn=document.getElementById('pvNewSupplier');if(pvn)pvn.onclick=function(){createCashPaymentSupplier().then(function(x){var sel=document.getElementById('pvSupplier');if(sel){sel.innerHTML=cashPaymentSupplierOptions(x.supplierId);sel.value=x.supplierId;}}).catch(function(e){if(String((e&&e.code)||e).indexOf('cancelled')<0)alert('Could not create supplier: '+((e&&e.message)||e));});};
...
function createVoucher(){
  ...
  compressImage(file,function(img){
    nextVoucherNo(function(no){
      ...
      a.set(a.ref(a.db,'pettyCashVouchers/'+id),{voucherNo:no,date:date,amount:amount,transactionType:transactionType,category:transactionType==='purchase_advance'?'Supplier payment pending inventory allocation':category,supplierId:transactionType==='purchase_advance'?supplierId:'',supplierName:transactionType==='purchase_advance'?requester:'',requesterName:requester,recipient:requester,purpose:purpose,remainingAmount:transactionType==='purchase_advance'?amount:null,approverName:approver,receiptImg:img||'',status:'pending',createdBy:(activeShift&&activeShift.staff)||'Admin',createdAt:Date.now(),schemaVersion:2}).then(function(){window.__posLog('petty-create',no,peso(amount));renderPetty();}).catch(function(e){alert('Could not save voucher: '+e);if(btn)btn.disabled=false;});
    });
  });
}
