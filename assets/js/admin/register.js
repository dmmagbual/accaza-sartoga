...
function linkStaffIdentity(id){var s=staffList[id];if(!s)return;F().run({title:'Link '+s.name+' to Firebase login',subtitle:'Paste the Firebase Authentication UID and set the staff member’s own POS PIN. This replaces any readable legacy PIN.',submitLabel:'Link staff login',busyLabel:'Linking…',fields:[{name:'accountUid',label:'Firebase Authentication UID',required:true,maxLength:160,value:s.accountUid||''},{name:'pin',label:'New POS PIN (4–6 digits)',type:'password',required:true,maxLength:6,validate:function(v){return /^[0-9]{4,6}$/.test(v)?'':'Enter a 4–6 digit PIN.';}}]},function(v){return A().managePosStaffIdentity({staffId:id,accountUid:v.accountUid,pin:v.pin});}).then(function(){alert('Staff login linked. This person can now only open and use their own POS shift.');}).catch(function(e){if(String((e&&e.code)||e).indexOf('cancelled')<0)alert('Could not link staff login: '+((e&&e.message)||e));});}
