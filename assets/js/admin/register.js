(function(){
'use strict';
var staffList={},activeShift=null,shiftsMap={},activityMap={},heldMap={},ordersMap={},discMap={},booksChartMap={},cashAccountsMap={},financialMovementsMap={},toleranceCfg={cashPeso:20,invPct:5},fixedFloatCfg=null,logCollapsed=true,cardShiftId=null;
...
function renderDiscrepancies(){
  ...
  var rows=all.length?all.map(function(d){
    ...
    return '<tr'+(d.status==='reviewed'?' style="opacity:0.55;"':'')+'><td>'+when+'</td><td>'+(d.kind==='cash'?'💵 Cash':'📦 Inventory')+'</td><td>'+what+'</td><td style="text-align:right;">'+exp+'</td><td style="text-align:right;">'+act+'</td><td style="text-align:right;font-weight:600;">'+vr+'</td><td>'+badge+'</td><td>'+esc(d.staff||'')+'</td><td>'+(d.status==='reviewed'?('<span style="font-size:0.72rem;color:var(--tl);">✓ '+esc(d.reviewedBy||'')+(d.treatment?' · '+esc(String(d.treatment).replace(/_/g,' ')):'')+(d.note?' — '+esc(d.note):'')+'</span>'):'<div style="font-size:.68rem;color:#8a5a00;margin-bottom:.2rem;">Manager revalidation required</div><button class="pz-btn ok" data-drev="'+esc(d.id)+'" style="padding:0.2rem 0.55rem;">Resolve</button>')+'</td></tr>';
...
async function finalizeClose(shift,counted,counts){
  ...
  if(Math.abs(z.variance)>=.005){var did=uid('disc_');writes['discrepancies/'+did]={kind:'cash',expected:z.expectedCash,actual:counted,variance:z.variance,value:z.variance,type:z.variance<0?'shortage':'overage',overTolerance:Math.abs(z.variance)>snapshot.tolerance,shiftId:shift.id,staff:shift.staff||'',status:'open',financialStatus:'pending_manager_reconciliation',pendingMovementId:'shift_variance_'+shift.id,ts:closedAt};}
