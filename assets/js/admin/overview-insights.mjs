...
function createOverviewInsights(deps){
  ...
  function loadYear(){if(!deps.readRollingSales)return;var r=overviewRollingYearRange(),key=r.from+':'+r.to;if(state.yearKey===key)return;var request=++state.yearRequest;state.yearKey=key;state.yearSales=null;state.yearError='';renderYearChart();Promise.resolve(deps.readRollingSales(r)).then(function(rows){if(request!==state.yearRequest)return;state.yearSales=rows||[];renderYearChart();}).catch(function(){if(request!==state.yearRequest)return;state.yearKey='';state.yearError='The 12-month sales history could not load. Refresh to try again.';renderYearChart();});}
