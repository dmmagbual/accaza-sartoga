...
function overviewRollingYearRange(today){var key=/^\d{4}-\d{2}-\d{2}$/.test(today||'')?today:overviewDateKey(),p=key.split('-'),first=new Date(Date.UTC(Number(p[0]),Number(p[1])-12,1)),from=first.getUTCFullYear()+'-'+String(first.getUTCMonth()+1).padStart(2,'0')+'-01';return{from:from,to:key,start:Date.parse(from+'T00:00:00+08:00'),end:Date.parse(key+'T23:59:59.999+08:00')};}
function overviewChannelKey(o){var channel=String(o&&o.channel||'').trim().toLowerCase(),source=String(o&&o.source||'').trim().toLowerCase();if(channel==='grabfood')return'grabfood';if(channel==='foodpanda')return'foodpanda';if(channel==='online'||source==='online')return'online';if(channel==='instore')return'instore';return'';}
function buildChannelBreakdown(rows){var keys=['online','instore','grabfood','foodpanda'],channels={},unclassified={orders:0,net:0};keys.forEach(function(k){channels[k]={key:k,orders:0,net:0};});(rows||[]).forEach(function(o){var key=overviewChannelKey(o),net=window.AccazaSales.amounts(o).net,bucket=channels[key]||unclassified;bucket.orders++;bucket.net+=net;});return{channels:keys.map(function(k){return channels[k];}),unclassified:unclassified};}
function buildRollingYear(rows,r){var buckets={},labels=[],start=new Date(r.start+8*3600000);start.setUTCHours(0,0,0,0);start.setUTCDate(1);for(var i=0;i<12;i++){var d=new Date(start);d.setUTCMonth(start.getUTCMonth()+i);var key=d.getUTCFullYear()+'-'+String(d.getUTCMonth()+1).padStart(2,'0');labels.push(key);buckets[key]={key:key,net:0,orders:0};}(rows||[]).forEach(function(o){var d=new Date(window.AccazaSales.stamp(o)+8*3600000),key=d.getUTCFullYear()+'-'+String(d.getUTCMonth()+1).padStart(2,'0'),bucket=buckets[key];if(bucket){bucket.net+=window.AccazaSales.amounts(o).net;bucket.orders++;}});return labels.map(function(k){return buckets[k];});}
function buildRollingYearFromMonths(rows,r,liveOrders){var buckets=buildRollingYear([],r).map(function(bucket){var row=(rows||{})[bucket.key]||{};return{key:bucket.key,net:(Number(row.netCents)||0)/100,orders:Number(row.orders)||0};}),map={};buckets.forEach(function(bucket){map[bucket.key]=bucket;});(liveOrders||[]).filter(function(order){return window.AccazaSales.qualifies(order);}).forEach(function(order){var stamp=window.AccazaSales.stamp(order);if(stamp<r.start||stamp>r.end)return;var d=new Date(stamp+8*3600000),key=d.getUTCFullYear()+'-'+String(d.getUTCMonth()+1).padStart(2,'0'),bucket=map[key];if(bucket){bucket.net+=window.AccazaSales.amounts(order).net;bucket.orders++;}});return buckets;}
...
function createOverviewInsights(deps){
  var state={metric:'units',latest:null,bound:false,rankingFrom:'',rankingTo:'',rankingOpen:false,lastFocus:null,rankingSales:null,rankingKey:'',rankingRequest:0,rankingError:'',yearSales:null,yearMonths:null,yearKey:'',yearRequest:0,yearError:''};
  ...
  function renderChannels(rows){var result=buildChannelBreakdown(rows),labels={online:'Online Orders',instore:'In-store',grabfood:'GrabFood',foodpanda:'FoodPanda'},colors={online:'#3b8fd4',instore:'#2f4937',grabfood:'#2a9d5c',foodpanda:'#d85f78'},total=result.channels.reduce(function(s,x){return s+x.net;},0),max=Math.max.apply(null,result.channels.map(function(x){return x.net;}).concat([1])),el=document.getElementById('channelBreakdown');if(!el)return;el.innerHTML=result.channels.map(function(x){var share=total?x.net/total*100:0,aov=x.orders?x.net/x.orders:0;return'<div class="overview-channel-row"><span class="overview-channel-name"><i class="overview-channel-dot" style="background:'+colors[x.key]+'"></i>'+labels[x.key]+'</span><span class="overview-channel-track"><i class="overview-channel-fill" style="width:'+(x.net/max*100).toFixed(2)+'%;background:'+colors[x.key]+'"></i></span><span class="overview-channel-values"><strong>'+money(x.net)+'</strong><small>'+x.orders+' order'+(x.orders===1?'':'s')+' · '+Math.round(share)+'% · AOV '+money(aov)+'</small></span></div>';}).join('')+(result.unclassified.orders?'<p class="overview-channel-warning">'+result.unclassified.orders+' completed order'+(result.unclassified.orders===1?' is':'s are')+' not included because the sales channel was not recorded.</p>':'');}
  function renderYearChart(){var canvas=document.getElementById('overviewYearChart'),note=document.getElementById('overviewYearNote');if(!canvas)return;if(!state.yearSales&&!state.yearMonths){canvas.getContext('2d').clearRect(0,0,canvas.width,canvas.height);if(note)note.textContent=state.yearError||'Loading 12-month completed sales…';return;}var r=overviewRollingYearRange(),months=state.yearMonths?buildRollingYearFromMonths(state.yearMonths,r,state.latest&&state.latest.orders):buildRollingYear(state.yearSales,r),W=Math.max(canvas.offsetWidth||720,620),H=230;canvas.width=W;canvas.height=H;var ctx=canvas.getContext('2d');ctx.clearRect(0,0,W,H);var rev=months.map(function(x){return x.net;}),cnt=months.map(function(x){return x.orders;}),maxR=Math.max.apply(null,rev.concat([1])),maxC=Math.max.apply(null,cnt.concat([1])),pad={l:52,r:18,t:22,b:38},cw=W-pad.l-pad.r,ch=H-pad.t-pad.b,gap=cw/12,bw=Math.max(12,gap*.5);ctx.strokeStyle='rgba(68,55,45,.10)';ctx.lineWidth=1;[0,.25,.5,.75,1].forEach(function(p){var y=pad.t+ch*(1-p);ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(W-pad.r,y);ctx.stroke();ctx.fillStyle='#81746a';ctx.font='10px Inter,sans-serif';ctx.textAlign='right';ctx.fillText(money(maxR*p),pad.l-5,y+3);});rev.forEach(function(v,i){var x=pad.l+i*gap+(gap-bw)/2,bh=v/maxR*ch;ctx.fillStyle='rgba(176,141,87,.82)';ctx.fillRect(x,pad.t+ch-bh,bw,bh);});ctx.strokeStyle='#3b8fd4';ctx.lineWidth=2;ctx.beginPath();cnt.forEach(function(v,i){var x=pad.l+i*gap+gap/2,y=pad.t+ch*(1-v/maxC);i?ctx.lineTo(x,y):ctx.moveTo(x,y);});ctx.stroke();cnt.forEach(function(v,i){var x=pad.l+i*gap+gap/2,y=pad.t+ch*(1-v/maxC);ctx.fillStyle='#3b8fd4';ctx.beginPath();ctx.arc(x,y,3,0,Math.PI*2);ctx.fill();ctx.fillStyle='#285f91';ctx.font='9px Inter,sans-serif';ctx.textAlign='center';ctx.fillText(String(v),x,Math.max(10,y-7));});months.forEach(function(x,i){var parts=x.key.split('-'),d=new Date(Date.UTC(Number(parts[0]),Number(parts[1])-1,1));ctx.fillStyle='#675b51';ctx.font='10px Inter,sans-serif';ctx.textAlign='center';ctx.fillText(d.toLocaleDateString('en-PH',{month:'short'}),pad.l+i*gap+gap/2,H-17);ctx.font='8px Inter,sans-serif';ctx.fillStyle='#95887d';ctx.fillText(parts[0],pad.l+i*gap+gap/2,H-6);});if(note){var total=months.reduce(function(s,x){return s+x.net;},0),orders=months.reduce(function(s,x){return s+x.orders;},0);note.textContent='12-month total: '+money(total)+' from '+orders+' completed order'+(orders===1?'':'s')+'. Current month is month to date.';}}
  function stopYear(){if(state.stopYear){state.stopYear();state.stopYear=null;state.yearKey='';state.yearRequest++;}}
  function loadYear(){
    if(!deps.readMonthlyRollup&&!deps.watchRollingSales&&!deps.readRollingSales)return;
    var r=overviewRollingYearRange(),key=r.from+':'+r.to;if(state.yearKey===key&&(!state.stopYear||state.stopYear.active!==false))return;
    stopYear();var request=++state.yearRequest;state.yearKey=key;state.yearSales=null;state.yearMonths=null;state.yearError='';renderYearChart();
    function received(rows){if(request!==state.yearRequest)return;state.yearSales=rows||[];state.yearError='';renderYearChart();}
    function failed(){if(request!==state.yearRequest)return;state.yearSales=null;state.yearError='The 12-month sales history could not load. Reopen Overview to try again.';renderYearChart();}
    if(deps.readMonthlyRollup)Promise.resolve(deps.readMonthlyRollup(r)).then(function(result){if(request!==state.yearRequest)return;if(!result||result.ready!==true){state.yearError='The 12-month summary is being prepared. Detailed Sales History remains available.';renderYearChart();return;}state.yearMonths=result.months||{};state.yearError='';renderYearChart();}).catch(failed);
    else if(deps.watchRollingSales)state.stopYear=deps.watchRollingSales(r,received,failed);
    else Promise.resolve(deps.readRollingSales(r)).then(received).catch(failed);
  }
  ...
  function paint(){
    bind();if(!state.latest)return;loadYear();var data=state.latest,r=range(),complete=data.historyComplete===true,label=document.getElementById('overviewRangeLabel');if(label)label.textContent=r.label;
    if(!complete){renderFullRanking();['overviewNetSales','overviewGrossSales','overviewTransactions','overviewAverageSale'].forEach(function(id){var el=document.getElementById(id);if(el)el.textContent='—';});var orderStatus=deps.historyStatus?deps.historyStatus('orders'):null,archiveStatus=deps.historyStatus?deps.historyStatus('archivedOrders'):null,loadError=(orderStatus&&orderStatus.error)||(archiveStatus&&archiveStatus.error),message=loadError?'Could not load the selected sales period: '+String(loadError&&loadError.message||loadError):'Loading the selected sales period…',pending=document.getElementById('overviewDataNote');if(pending)pending.textContent=message;['topItemsList','payMixList','channelBreakdown'].forEach(function(id){var el=document.getElementById(id);if(el)el.textContent=loadError?message:'Loading…';});var canvas=document.getElementById('revenueChart');if(canvas)canvas.getContext('2d').clearRect(0,0,canvas.width,canvas.height);return;}
    var periodSales=(data.sales||[]).filter(function(o){return inRange(o,r);});
