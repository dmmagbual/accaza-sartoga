...
export function createCustomerOrderTracker(deps={}){
  ...
  function render(){
    ...
    el.querySelectorAll('.confirm-recv-btn').forEach(function(btn){btn.addEventListener('click',function(){const oid=this.dataset.orderid;document.getElementById('receivedOrderId').textContent=oid;document.getElementById('confirmReceivedPopup').classList.add('show');document.getElementById('confirmReceivedBtn').onclick=async function(){var b=this;b.disabled=true;try{await callables.confirmOrderReceived({orderId:oid});document.getElementById('confirmReceivedPopup').classList.remove('show');dismissAlert();}catch(e){alert('Could not confirm receipt: '+((e&&e.message)||e));}finally{b.disabled=false;}};});});
  }
