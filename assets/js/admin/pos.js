(function(){
'use strict';
var inventoryMap={}, inventorySkuMap={}, purchaseInvoicesMap={}, purchaseShiftMap={}, recipesMap={}, posMeta={vat:false,vatRate:12}, optRecipesMap={}, usageMap={}, channelPricesMap={}, posAvailMap={}, inventoryMovementsMap={};
...
function addIngredient(){
  ...
  var o={name:name,unit:document.getElementById('invUnit').value,type:type,category:(document.getElementById('invCat')||{}).value||'',inventoryAccount:inventoryAccount,costAccount:costAccount,stock:0,reorder:Number(document.getElementById('invReorder').value)||0,cost:0,updatedAt:Date.now()};
...
function purchInit(){ if(!window.__purch){ window.__purch={supplier:'',ref:'',date:window.AccazaDate.key(),by:((window.__posShift&&window.__posShift.staff)||'Admin'),pay:'pending',acct:'',advanceId:'',due:'',lines:[purchBlank()]}; } }
...
function allPurchaseAdvances(){var rows=[];Object.keys(purchaseShiftMap).forEach(function(shiftId){var s=purchaseShiftMap[shiftId]||{};(s.payOuts||[]).forEach(function(x){if(x&&x.type==='purchase_advance')rows.push(Object.assign({shiftId:shiftId,staff:s.staff||''},x,{remaining:purchaseAdvanceRemaining(x)}));});});return rows.sort(function(a,b){return (b.ts||0)-(a.ts||0);});}
function openPurchaseAdvances(){return allPurchaseAdvances().filter(function(x){return x.status!=='cancelled'&&x.remaining>0;});}
function purchaseAdvanceRegisterHtml(){var rows=allPurchaseAdvances();if(!rows.length)return '';return '<div class="pz-h" style="margin-top:1.4rem;">Register purchase cash</div><p class="pz-sub">Each supplier purchase is allocated separately. The remaining amount stays as cash entrusted for purchase in Finance Books.</p><div class="pz-card" style="overflow:auto;"><table class="pz-table" style="min-width:850px;width:100%;"><thead><tr><th>Released</th><th>Recipient / purpose</th><th class="r">Advance</th><th>Itemized purchases</th><th class="r">Allocated</th><th class="r">Remaining</th></tr></thead><tbody>'+rows.map(function(x){var allocations=Object.keys(x.allocations||{}).map(function(id){var a=x.allocations[id]||{},p=purchaseInvoicesMap[id]||{};return '<div><b>'+esc(a.supplier||p.supplier||'Supplier')+'</b> · '+esc(a.ref||p.ref||id)+' · '+peso(a.amount)+'</div>';}).join('')||'<span class="az-note">Pending purchase details</span>',allocated=Math.round(((Number(x.amount)||0)-x.remaining)*100)/100;return '<tr><td>'+esc(x.ts?new Date(x.ts).toLocaleDateString('en-PH'):'—')+'<div class="tiny">'+esc(x.staff||'')+'</div></td><td><b>'+esc(x.recipient||'—')+'</b><div class="tiny">'+esc(x.purpose||'')+'</div></td><td class="r">'+peso(x.amount)+'</td><td>'+allocations+'</td><td class="r">'+peso(allocated)+'</td><td class="r"><b>'+peso(x.remaining)+'</b></td></tr>';}).join('')+'</tbody></table></div>';}
...
function purchaseStatusLabel(p){if(p.reversed)return 'Reversed';if(p.payMode==='advance')return 'Paid from register purchase cash';if(p.payMode==='paid')return 'Paid';if(p.payMode==='account')return 'On account';if(p.payMode==='pending')return 'Invoice pending';return 'Legacy — liability missing';}
...
function renderPurchases(){
  ...
  var cf=window.__cf; var accs=(cf&&cf.accounts&&cf.accounts())||[];
  var advances=openPurchaseAdvances(),advanceOpts=advances.map(function(x){return '<option value="'+esc(x.id)+'"'+(P.advanceId===x.id?' selected':'')+'>'+peso(x.remaining)+' remaining of '+peso(x.amount)+' · '+esc(x.recipient||'')+' · '+esc(x.purpose||'')+' ('+esc(x.staff||'shift')+')</option>';}).join('');
  ...
  var payBlock='<div class="purchase-section purchase-payment"><div class="purchase-section-head"><span class="purchase-step">2</span><div><b>Payment</b><small>Choose how this whole invoice will be settled.</small></div></div>'
    +'<div class="purchase-payment-options">'
      ...
      +'<label class="purchase-payment-option"><input type="radio" name="ppay" data-pf="pay" value="paid"'+(P.pay==='paid'?' checked':'')+(accs.length?'':' disabled')+'/><span><b>Paid now</b><small>'+(accs.length?'Deduct from the selected Finance Books cash account.':'Add a cash account in Finance Books first.')+'</small>'+(accs.length?('<select class="pz-in" id="purAcct">'+accOpts+'</select>'):'')+'</span></label>'
      +'<label class="purchase-payment-option"><input type="radio" name="ppay" data-pf="pay" value="advance"'+(P.pay==='advance'?' checked':'')+(advances.length?'':' disabled')+'/><span><b>Paid from register purchase cash</b><small>'+(advances.length?'Link the cashier’s documented cash release; the drawer will not be deducted again.':'No unlinked purchase cash advances are available.')+'</small>'+(advances.length?('<select class="pz-in" id="purAdvance">'+advanceOpts+'</select>'):'')+'</span></label>'
...
function editIngredient(id){
  ...
  mask.querySelector('#eiSave').onclick=function(){
    ...
    var upd={name:(mask.querySelector('#eiName').value||'').trim()||i.name,unit:mask.querySelector('#eiUnit').value,type:type,category:(mask.querySelector('#eiCat')||{}).value||'',inventoryAccount:inventoryAccount,costAccount:costAccount,cogsAccount:null,reorder:Number(mask.querySelector('#eiReorder').value)||0,updatedAt:Date.now()};
