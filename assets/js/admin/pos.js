...
function renderOnlineOrders(){
  ...
  function card(o){var verified=['cashier_verified','manager_validated','confirmed'].indexOf(o.paymentStatus)>=0,captured=!!o.shiftId,proof=o.proofPath?'<button class="pz-btn sec" data-online-proof="'+esc(o.id)+'">View payment proof</button>':'',action='';
    ...
    else action='<span class="pos-online-state captured">'+(o.paymentStatus==='cashier_verified'?'Cashier verified · manager review pending':'Payment validated')+' · '+esc(o.status)+'</span><button class="pz-btn" data-online-change="'+esc(o.id)+'">Refund</button>'+onlineStatusAction(o);
  ...
  root.querySelectorAll('[data-online-change]').forEach(b=>b.onclick=()=>window.__posPaidOrderChange(b.dataset.onlineChange));
