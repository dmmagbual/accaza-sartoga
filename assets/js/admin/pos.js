...
function purchaseStatusLabel(p){if(p.reversed)return 'Reversed';if(p.payMode==='owner_funded')return 'Paid personally by owner/partner · capital contributed';if(p.payMode==='advance')return 'Allocated from payment pending inventory allocation';if(p.payMode==='paid')return 'Paid';if(p.payMode==='account')return 'On account';if(p.payMode==='pending')return 'Invoice pending';return 'Legacy — liability missing';}
...
function renderPurchases(){
  ...
  var payBlock='<div class="purchase-section purchase-payment"><div class="purchase-section-head"><span class="purchase-step">2</span><div><b>Payment</b><small>Choose how this whole invoice will be settled.</small></div></div>'
    +'<div class="purchase-payment-options">'
      ...
      +'<label class="purchase-payment-option"><input type="radio" name="ppay" data-pf="pay" value="owner_funded"'+(P.pay==='owner_funded'?' checked':'')+'/><span><b>PAID PERSONALLY BY OWNER/PARTNER</b><small>Increase inventory and credit Owner’s Capital. No business cash movement or supplier payable is created.</small></span></label>'
...
function postPurchases(){
  ...
  if(P.pay==='owner_funded'&&!(A()&&A().postFinancialCommand)){alert('Owner/partner capital posting service is not ready. Refresh the portal and try again.');return;}
  ...
  Promise.resolve(Object.keys(seedUpdates).length?a.update(a.ref(a.db),seedUpdates):null).then(function(){return postMovements(movementRows);}).then(function(){return a.update(a.ref(a.db),updates);}).then(function(){
    ...
    if(P.pay==='owner_funded'&&a.postFinancialCommand)return a.postFinancialCommand({action:'purchase_owner_funded',commandId:'purchase_owner_'+invoiceId,invoiceId:invoiceId,date:date});
    ...
    if(window.__posLog)window.__posLog('purchase',(P.supplier||'Supplier'),lines.length+' item(s) · '+peso(invTotal)+(P.pay==='paid'?' · paid':P.pay==='owner_funded'?' · paid personally by owner/partner · capital contributed':P.pay==='account'?' · on account':' · invoice pending'));
