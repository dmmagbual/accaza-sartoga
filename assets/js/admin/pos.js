...
function costingContext(extra){return Object.assign({inventory:inventoryMap,recipes:recipesMap,menuItems:(A()&&A().menuItemsMap)||{},sharedBaseIngredients:(window.__posSettings&&window.__posSettings.sharedBaseIngredients)||{},optionCosts:optCostStore(),optionRecipes:optRecipesMap,optionGroups:(A()&&A().optionGroupsMap)||{},packagingRules:packagingRulesMap,packagingAssignments:(window.__posSettings&&window.__posSettings.packagingAssignments)||{}},extra||{});}
...
function openStdCosting(){
  ...
  function draw(){
    ...
    var drift=ings().filter(function(x){return !ingIsArchived(x)&&x.stdCost!=null&&x.stdCost!==''&&Math.abs((Number(x.stdCost)||0)-(Number(x.cost)||0))>0.00001;})
    ...
    document.getElementById('stSetWac').onclick=function(){ if(!confirm('Set every item’s standard cost to its current weighted-average? This snapshots today’s WAC as the standard (useful before switching to Manual).'))return; var upd={}; ings().forEach(function(x){if(ingIsArchived(x))return;upd['inventory/'+x.id+'/stdCost']=Number(x.cost)||0;}); a.update(a.ref(a.db),upd).then(function(){alert('Standards set to current WAC for '+Object.keys(upd).length+' item(s).');draw();}).catch(function(e){alert('Could not update: '+((e&&e.code)||e));}); };
...
function purchaseExpenseAccounts(){return Object.keys(booksChartMap||{}).map(function(k){return booksChartMap[k];}).filter(function(x){return x&&x.active!==false&&String(x.type||'').toLowerCase()==='expense'&&/^6\d{3}$/.test(String(x.code))&&String(x.code)!=='6110';}).sort(function(a,b){return String(a.code).localeCompare(String(b.code));});}
function purchaseExpenseAccount(code){var x=booksChartMap&&booksChartMap[String(code)];return x&&x.active!==false&&String(x.type||'').toLowerCase()==='expense'&&/^6\d{3}$/.test(String(code))&&String(code)!=='6110'?x:null;}
function purchInit(){ if(!window.__purch){ window.__purch={supplierId:'',supplier:'',ref:'',date:window.AccazaDate.key(),by:((window.__posShift&&window.__posShift.staff)||'Admin'),description:'',pay:'pending',acct:'',advanceId:'',due:'',ownerName:'',ownerTreatment:'capital',lines:[purchBlank()]}; } }
...
function showSupplierList(){
  var old=document.getElementById('supplierListMask');if(old)old.remove();
  var rows=allSuppliersForList();
  var m=document.createElement('div');m.id='supplierListMask';m.className='pz-mask show';
  m.innerHTML='<div class="pz-modal" style="max-width:640px;"><div style="display:flex;justify-content:space-between;gap:1rem;"><div class="pz-h">Suppliers</div><button class="pz-btn sec" data-supplier-list-close>\u2715</button></div><p class="pz-sub">One shared supplier record used across Purchases, Cash Payments, Payables and Finance Books.</p><div class="pz-card" style="overflow:auto;max-height:60vh;"><table class="pz-table" style="width:100%;"><thead><tr><th>Name</th><th>Status</th><th>Actions</th></tr></thead><tbody>'+(rows.length?rows.map(function(x){
    var survivor=x.mergedInto&&supplierMap[x.mergedInto],status=x.mergedInto?('<span style="color:#7a6a58;">Merged into '+esc((survivor&&survivor.name)||x.mergedInto)+'</span>'):(x.active===false?'<span style="color:#c0392b;">Inactive</span>':'<span style="color:#155724;">Active</span>');
    var actions=x.mergedInto?'<span class="tiny">Kept for the audit trail</span>':('<button class="pz-btn sec" data-supplier-rename="'+esc(x.id)+'" style="padding:0.2rem 0.5rem;">Rename</button> <button class="pz-btn sec" data-supplier-merge="'+esc(x.id)+'" style="padding:0.2rem 0.5rem;">Merge</button> '+(x.active===false?'<button class="pz-btn ok" data-supplier-reactivate="'+esc(x.id)+'" style="padding:0.2rem 0.5rem;">Reactivate</button>':'<button class="pz-btn warn" data-supplier-deactivate="'+esc(x.id)+'" style="padding:0.2rem 0.5rem;">Deactivate</button>')+' <button class="pz-btn warn" data-supplier-delete="'+esc(x.id)+'" style="padding:0.2rem 0.5rem;">Delete</button>');
    return '<tr><td>'+esc(x.name||'')+'</td><td>'+status+'</td><td style="white-space:nowrap;">'+actions+'</td></tr>';}).join(''):'<tr><td colspan="3">No suppliers yet.</td></tr>')+'</tbody></table></div><button class="pz-btn ok" data-supplier-list-close style="width:100%;margin-top:0.8rem;">Close</button></div>';
  document.body.appendChild(m);
  m.querySelectorAll('[data-supplier-list-close]').forEach(function(b){b.onclick=function(){m.remove();};});
  m.querySelectorAll('[data-supplier-rename]').forEach(function(b){b.onclick=function(){var id=b.getAttribute('data-supplier-rename'),row=supplierMap[id]||{};F().run({title:'Rename supplier',subtitle:'Historical purchases, payments and payables keep their point-in-time name snapshot. Only the shared master record changes.',submitLabel:'Save name',busyLabel:'Saving\u2026',fields:[{name:'name',label:'Official supplier name',required:true,maxLength:120,value:row.name||''}]},function(v){return A().manageSupplier({action:'update',supplierId:id,name:v.name});}).then(function(r){var d=(r&&r.data)||r||{};supplierMap[id]=Object.assign({},supplierMap[id]||{},{name:d.name||row.name});m.remove();showSupplierList();renderPurchases();}).catch(function(e){if(String((e&&e.code)||e).indexOf('cancelled')<0)alert('Could not rename supplier: '+((e&&e.message)||e));});};});
  m.querySelectorAll('[data-supplier-deactivate]').forEach(function(b){b.onclick=function(){var id=b.getAttribute('data-supplier-deactivate'),row=supplierMap[id]||{};if(!confirm('Deactivate '+(row.name||'this supplier')+'? It will no longer be selectable for new purchases, cash payments, or bills. Existing linked records are unaffected.'))return;A().manageSupplier({action:'deactivate',supplierId:id}).then(function(){supplierMap[id]=Object.assign({},supplierMap[id]||{},{active:false});m.remove();showSupplierList();renderPurchases();}).catch(function(e){alert('Could not deactivate supplier: '+((e&&e.message)||e));});};});
  m.querySelectorAll('[data-supplier-merge]').forEach(function(b){b.onclick=function(){
    var id=b.getAttribute('data-supplier-merge'),row=supplierMap[id]||{};
    var others=allSuppliersForList().filter(function(x){return x.id!==id&&!x.mergedInto&&x.active!==false;});
    if(!others.length){alert('There is no other active supplier to merge into.');return;}
    F().run({title:'Merge '+(row.name||'supplier')+' into another supplier',subtitle:'Use this for a duplicate of the same real supplier. Purchases, payables, advances, stock receipts, batches and approved brands are repointed to the surviving record. Every posted Finance movement and the supplier name printed on each historical document stay exactly as they are — no cash, stock or journal amount moves.',submitLabel:'Merge suppliers',busyLabel:'Merging…',fields:[
      {name:'targetId',label:'Surviving supplier',type:'select',required:true,value:'',options:[{value:'',label:'— choose the record to keep —'}].concat(others.map(function(x){return {value:x.id,label:x.name||x.id};})),help:'“'+(row.name||id)+'” becomes inactive and is kept as a merged record in the audit trail.'},
      {name:'reason',label:'Reason for the merge',required:true,maxLength:300,help:'Recorded in the operational audit trail.'}
    ]},function(v){if(!v.targetId)throw new Error('Choose the supplier record to keep.');return A().manageSupplier({action:'merge',supplierId:id,targetId:v.targetId,reason:v.reason});})
    .then(function(r){var d=(r&&r.data)||r||{};supplierMap[id]=Object.assign({},supplierMap[id]||{},{mergedInto:d.mergedInto,active:false});m.remove();showSupplierList();renderPurchases();alert('Merged into '+(d.survivingName||'the surviving supplier')+'. '+(d.repointed||0)+' linked record(s) now point to it.');})
    .catch(function(e){if(String((e&&e.code)||e).indexOf('cancelled')<0)alert('Could not merge supplier: '+((e&&e.message)||e));});};});
  m.querySelectorAll('[data-supplier-delete]').forEach(function(b){b.onclick=function(){
    var id=b.getAttribute('data-supplier-delete'),row=supplierMap[id]||{};
    A().manageSupplier({action:'references',supplierId:id}).then(function(r){
      var d=(r&&r.data)||r||{};
      if((d.total||0)>0){alert('“'+(row.name||id)+'” is used by '+d.total+' record(s), so it cannot be deleted. Merge it into the correct supplier instead — that keeps every purchase, payable and receipt linked.');return null;}
      return F().run({title:'Delete '+(row.name||'supplier'),subtitle:'Nothing references this record — no purchases, payables, advances, stock receipts, batches or approved brands — so it can be removed outright. A supplier with any history must be merged or deactivated instead.',submitLabel:'Delete supplier',busyLabel:'Deleting…',fields:[{name:'reason',label:'Reason for deleting',required:true,maxLength:300,help:'Recorded in the operational audit trail.'}]},function(v){return A().manageSupplier({action:'delete',supplierId:id,reason:v.reason});})
        .then(function(){delete supplierMap[id];m.remove();showSupplierList();renderPurchases();});
    }).catch(function(e){if(String((e&&e.code)||e).indexOf('cancelled')<0)alert('Could not delete supplier: '+((e&&e.message)||e));});};});
  m.querySelectorAll('[data-supplier-reactivate]').forEach(function(b){b.onclick=function(){var id=b.getAttribute('data-supplier-reactivate');A().manageSupplier({action:'reactivate',supplierId:id}).then(function(){supplierMap[id]=Object.assign({},supplierMap[id]||{},{active:true});m.remove();showSupplierList();renderPurchases();}).catch(function(e){alert('Could not reactivate supplier: '+((e&&e.message)||e));});};});
}

function purchaseAdvanceMatchesSupplier(x,supplierId,supplier){if(x.supplierId&&supplierId){var a=purchaseSupplierResolvedId(x.supplierId),b=purchaseSupplierResolvedId(supplierId);return !!a&&a===b;}if(x.supplierId||supplierId)return false;var wanted=purchaseSupplierKey(supplier);return !wanted||purchaseSupplierKey(x.supplierName||x.recipient)===wanted;}
...
function openPurchaseAdvances(supplierId,supplier){return allPurchaseAdvances().filter(function(x){return x.status!=='cancelled'&&x.remaining>0&&purchaseAdvanceMatchesSupplier(x,supplierId,supplier);});}
function createPurchaseSupplier(defaultName){return F().run({title:'Create supplier',subtitle:'Creates one shared supplier identity for Cash Payments, Purchases, Payables, inventory allocations and Finance Books.',submitLabel:'Create supplier',busyLabel:'Creating…',fields:[{name:'name',label:'Official supplier name',required:true,maxLength:120,value:defaultName||''}]},function(v){return A().manageSupplier({action:'create',name:v.name});}).then(function(r){return (r&&r.data)||r||{};});}
function purchaseAdvanceRegisterHtml(){var rows=allPurchaseAdvances();if(!rows.length)return '';return '<div class="pz-h" style="margin-top:1.4rem;">Payments pending inventory allocation</div><p class="pz-sub">Cash has already been paid. Managers allocate each itemized supplier purchase separately; the remaining amount stays unallocated in Finance Books.</p><div class="pz-card" style="overflow:auto;"><table class="pz-table" style="min-width:850px;width:100%;"><thead><tr><th>Paid</th><th>Source / payee</th><th class="r">Payment</th><th>Itemized purchases</th><th class="r">Allocated</th><th class="r">Unallocated</th></tr></thead><tbody>'+rows.map(function(x){var allocations=Object.keys(x.allocations||{}).map(function(id){var a=x.allocations[id]||{},p=purchaseInvoicesMap[id]||{};return '<div><b>'+esc(a.supplier||p.supplier||'Supplier')+'</b> · '+esc(a.ref||p.ref||id)+' · '+peso(a.amount)+'</div>';}).join('')||'<span class="az-note">Pending inventory allocation</span>',allocated=Math.round(((Number(x.amount)||0)-x.remaining)*100)/100,at=x.ts||x.createdAt||0;return '<tr><td>'+esc(at?new Date(at).toLocaleDateString('en-PH'):'—')+'<div class="tiny">'+(x.advanceSource==='revolving'?esc(purchaseFundingLabel(x.fundingAccountId)):esc(x.staff||'Register'))+'</div></td><td><b>'+esc(x.recipient||'—')+'</b><div class="tiny">'+esc(x.purpose||'')+'</div></td><td class="r">'+peso(x.amount)+'</td><td>'+allocations+'</td><td class="r">'+peso(allocated)+'</td><td class="r"><b>'+peso(x.remaining)+'</b></td></tr>';}).join('')+'</tbody></table></div>';}
...
function bindPurchaseQuantityCorrection(root){
  ...
  var payBlock='<div class="purchase-section purchase-payment"><div class="purchase-section-head"><span class="purchase-step">2</span><div><b>Payment</b><small>Choose how this whole invoice will be settled.</small></div></div>'
    +'<div class="purchase-payment-options">'
      ...
      +'<label class="purchase-payment-option"><input type="radio" name="ppay" data-pf="pay" value="advance"'+(P.pay==='advance'?' checked':'')+(!advanceLoading&&!advanceLoadError&&advances.length?'':' disabled')+'/><span><b>Apply supplier advance payment</b><small>'+(advanceLoadError?'Supplier advances could not load. Refresh Admin and try again.':(advanceLoading?'Loading approved supplier advances…':(advances.length?'Approved, unallocated payments for this supplier are shown. This clears the advance without paying twice.':(P.supplier?'No approved advance remains for this supplier.':'Enter the supplier first to find its approved advances.'))))+'</small>'+(!advanceLoading&&!advanceLoadError&&advances.length&&P.pay==='advance'?('<select class="pz-in" id="purAdvance">'+advanceOpts+'</select>'):'')+'<button type="button" class="pz-btn sec" id="purCreateAdvance">Record a new supplier advance</button></span></label>'
    ...
    +'</div></div>';
  root.innerHTML='<div class="purchase-page-head"><div><div class="pz-h">Purchases <span>Goods received</span></div><p class="pz-sub">Record a supplier delivery and update stock in one clear receiving sheet.</p></div><div class="purchase-head-note">Receiving goods or an itemised supplier invoice? Record it here — whatever way it was paid. Settling a bill already raised: Finance Books → Payables. Paying a supplier before delivery, or a cost with no itemised invoice: Cash Payments.</div></div>'
  ...
  bindPurchaseQuantityCorrection(root);
...
/* ══════════ RECIPES ══════════ */
function recipeMirrorSingleServingDraft(d,item){if(!recipeItemIsSingleServing(item))return d;function mirror(r){var q=r.dS!==''&&r.dS!=null?r.dS:(r.dM!==''&&r.dM!=null?r.dM:r.dL);r.dS=r.dM=r.dL=q;}(d.base||[]).forEach(mirror);Object.keys(d.choiceAdd||{}).forEach(function(g){Object.keys(d.choiceAdd[g]||{}).forEach(function(lk){((d.choiceAdd[g][lk]||{}).ings||[]).forEach(mirror);});});return d;}
function recipeDraftRaw(d,item){d=recipeMirrorSingleServingDraft(d||{},item);function clean(r,op){var inv=inventoryMap[r.ing]||{},x={ing:r.ing,unit:r.unit||inv.unit||'',dispS:r.dS===''?null:r.dS,dispM:r.dM===''?null:r.dM,dispL:r.dL===''?null:r.dL,when:r.when&&Object.keys(r.when).length?r.when:undefined,useFor:Array.isArray(r.useFor)&&r.useFor.length?r.useFor:undefined};if(op)x.op=r.op||undefined;return x;}function valid(r){return r&&r.ing&&['S','M','L'].some(function(sz){return r['d'+sz]!=null&&r['d'+sz]!=='';});}var rec={base:(d.base||[]).filter(valid).map(function(r){return clean(r);}),sharedBase:(d.sharedBase||[]).map(function(x){return{ing:typeof x==='string'?x:x.ing};}).filter(function(x){return x.ing;}),updatedAt:Date.now()};Object.keys(d.choiceAdd||{}).forEach(function(g){var group={};Object.keys(d.choiceAdd[g]||{}).forEach(function(lk){var e=d.choiceAdd[g][lk]||{},rows=(e.ings||[]).filter(valid).map(function(r){return clean(r,true);});if(rows.length)group[lk]={label:e.label||lk,ings:rows};});if(Object.keys(group).length){rec.choiceAdd=rec.choiceAdd||{};rec.choiceAdd[g]=group;}});return rec;}
...
function recipeChoiceOverrideApplies(d,ing,c,tempId,tempLabel){var row=recipeChoiceOverrideRow(d,ing,c),when=row&&row.when||{};return !!row&&(!Object.keys(when).length||when[tempId]===tempLabel);}
function setRecipeChoiceOverrideTemperature(d,ing,c,tempId,tempLabel,tempChoices,on){var row=recipeChoiceOverrideRow(d,ing,c);if(!row)return false;var when=row.when||{},other=(tempChoices||[]).map(function(x){return x.label;}).filter(function(x){return x!==tempLabel;})[0];if(on){if(when[tempId]&&when[tempId]!==tempLabel)delete row.when;else{row.when=row.when||{};row.when[tempId]=tempLabel;}}else if(!Object.keys(when).length&&other){row.when={};row.when[tempId]=other;}else if(when[tempId]===tempLabel){var entry=d.choiceAdd[c.groupId][optKey(c.label)];entry.ings=entry.ings.filter(function(x){return x!==row;});if(!entry.ings.length)delete d.choiceAdd[c.groupId][optKey(c.label)];}return true;}
function recipeChoiceOverrideView(d,r,temperatureGroup,selState){var choice={groupId:r.groupId,label:r.label},has=recipeChoiceOverridden(d,r.ingredientId,choice),temp=temperatureGroup&&selState[temperatureGroup.id],on=has&&temperatureGroup&&recipeChoiceOverrideApplies(d,r.ingredientId,choice,temperatureGroup.id,temp);return{choice:choice,has:has,temp:temp,on:on,label:'Shared choice'+(on?' · overridden for '+r.label:has&&temp?' · inherited for '+temp:'')};}
function bindEffectiveChoiceIncludes(ed,d,item,syncAll){ed.querySelectorAll('[data-effective-choice-include]').forEach(function(b){b.onchange=function(){syncAll();var c={groupId:b.dataset.choiceGroup,label:b.dataset.choiceLabel},temp=(A().optionGroupsMap||{})[b.dataset.tempGroup]||{};setRecipeChoiceOverrideTemperature(d,b.dataset.effectiveChoiceInclude,c,b.dataset.tempGroup,b.dataset.tempLabel,temp.choices||[],b.checked);drawRecipeEditor(item);};});}
function markInheritedChoiceOverride(d,g,lk,ix,ing){var shared=optCostStore()[g]&&optCostStore()[g][lk],row=d.choiceAdd&&d.choiceAdd[g]&&d.choiceAdd[g][lk]&&(d.choiceAdd[g][lk].ings||[])[ix];if(!row||!(shared&&shared.ings||[]).some(function(x){return x.ing===ing;}))return false;row.op='choice_override';row.when=recipeTemperatureScope(g);return true;}
function bindChoiceIngredientSelectors(ed,d,item,syncAll){ed.querySelectorAll('select[data-caf]').forEach(function(s){s.onchange=function(){var tr=s.closest('[data-carow]'),g=tr&&tr.dataset.caG,lk=tr&&tr.dataset.caL,ix=0;if(tr)Array.from(tr.parentNode.querySelectorAll('[data-carow]')).some(function(row){if(row===tr)return true;if((row.querySelector('[data-caf="ing"]')||{}).value)ix++;return false;});syncAll();if(s.dataset.caf==='ing'&&g&&lk)markInheritedChoiceOverride(d,g,lk,ix,s.value);drawRecipeEditor(item);};});}
function recipeTemperatureScope(excludeGid){var groups=(A()&&A().optionGroupsMap)||{},sel=window.__recCostSel||{},tempId=Object.keys(groups).filter(function(id){return id!==excludeGid&&String((groups[id]||{}).name||'').toLowerCase()==='temperature';})[0],label=tempId&&sel[tempId];return tempId&&label&&!Array.isArray(label)?(function(){var out={};out[tempId]=label;return out;})():{};}
function recipeTemperatureGroup(groups){return (groups||[]).filter(function(g){return String(g.name||'').toLowerCase()==='temperature';})[0]||null;}
function recipeTemperatureControl(r,tempGroup,activeTemp){var labels=Object.keys(r.when||{}).map(function(id){return r.when[id];}),active=!labels.length||labels.indexOf(activeTemp)>=0,html=r.op==='choice_override'&&tempGroup?'<small style="display:flex;gap:.55rem;margin-top:.25rem;color:var(--tl)"><span>Use for:</span>'+(tempGroup.choices||[]).map(function(c){return '<label><input type="checkbox" data-ca-temp-scope="'+esc(c.label)+'"'+((!labels.length||labels.indexOf(c.label)>=0)?' checked':'')+'> '+esc(c.label)+'</label>';}).join('')+'</small>':'';return{labels:labels,active:active,html:html};}
function bindRecipeTemperatureControls(ed,syncAll,draw,item){ed.querySelectorAll('[data-ca-temp-scope]').forEach(function(cb){cb.onchange=function(){var tr=cb.closest('[data-carow]'),picked=[];tr.querySelectorAll('[data-ca-temp-scope]:checked').forEach(function(x){picked.push(x.getAttribute('data-ca-temp-scope'));});if(!picked.length){cb.checked=true;alert('Keep at least Hot or Iced selected. Remove the override row if neither temperature should use it.');return;}var gid=tr.getAttribute('data-ca-temp-group'),all=tr.querySelectorAll('[data-ca-temp-scope]').length,when={};if(picked.length<all)when[gid]=picked[0];tr.setAttribute('data-ca-when',JSON.stringify(when));syncAll();draw(item);};});}
function setRecipeChoiceOverride(d,ing,gid,label,on,when){var lk=optKey(label),shared=optCostStore()[gid]&&optCostStore()[gid][lk];if(!shared)return false;d.choiceAdd=d.choiceAdd||{};d.choiceAdd[gid]=d.choiceAdd[gid]||{};var own=d.choiceAdd[gid][lk]||(d.choiceAdd[gid][lk]={label:label,ings:[]}),sharedTarget;(shared.ings||[]).forEach(function(x){var inv=inventoryMap[x.ing]||{},u=x.unit||inv.unit||'',row=(own.ings||[]).filter(function(y){return y.ing===x.ing;})[0],values={ing:x.ing,unit:u,dS:x.dispS!=null?x.dispS:x.qtyS,dM:x.dispM!=null?x.dispM:x.qtyM,dL:x.dispL!=null?x.dispL:x.qtyL,op:x.op};if(x.ing===ing)sharedTarget=values;if(!row)own.ings.push(values);});var target=own.ings.filter(function(x){return x.ing===ing;})[0];if(!target)return false;if(on){target.op='choice_override';target.when=when||{};}else Object.assign(target,sharedTarget||{op:undefined,when:undefined});return true;}
...
function recipeServingScopeLabels(tempGroup){var labels=(tempGroup&&tempGroup.choices||[]).map(function(c){return c.label;}).filter(function(label){return /^(hot|iced)$/i.test(label||'');});if(labels.indexOf('Blended')<0)labels.push('Blended');return labels;}
function sharedChoiceRowScopes(row,tempGroup){var all=recipeServingScopeLabels(tempGroup);if(Array.isArray(row&&row.useFor)&&row.useFor.length)return row.useFor.filter(function(label){return all.indexOf(label)>=0;});var when=row&&row.when||{},legacy=tempGroup&&when[tempGroup.id];return legacy?[legacy]:all;}
function sharedChoiceTemperatureScope(row,rows,tempGroup,currentGroupId){
  ...
  var labels=recipeServingScopeLabels(tempGroup),when=row.when||{},picked=sharedChoiceRowScopes(row,tempGroup);
...
function sharedChoiceScopeFromRow(tr,tempGroup){
  ...
  if(!boxes.length||picked.length===boxes.length)return{valid:true,when:{},useFor:undefined};
  if(!picked.length)return{valid:false,when:{},useFor:undefined};
  return{valid:true,when:{},useFor:picked};
}
function sharedChoiceScopeError(optionCosts,tempGroup){
  if(!tempGroup)return'';var error='';Object.keys(optionCosts||{}).some(function(gid){return Object.keys(optionCosts[gid]||{}).some(function(key){var rows=optionCosts[gid][key].ings||[],seen={};rows.forEach(function(row){if(row&&row.ing)(seen[row.ing]=seen[row.ing]||[]).push(row);});return Object.keys(seen).some(function(ing){var list=seen[ing];if(list.length<2)return false;var coverage={};return list.some(function(row){return sharedChoiceRowScopes(row,tempGroup).some(function(label){if(coverage[label]){error='Assign each duplicate ingredient to a different serving style.';return true;}coverage[label]=1;return false;});});});});});return error;
}
function sharedChoiceCostText(rows,tempGroup,groupId){var scoped=tempGroup&&tempGroup.id!==groupId&&(rows||[]).some(function(r){return sharedChoiceDuplicate(rows,r.ing);});return scoped?recipeServingScopeLabels(tempGroup).map(function(label){return esc(label)+' — S '+peso(ocChoiceCost(rows,'S',tempGroup,label))+' · M '+peso(ocChoiceCost(rows,'M',tempGroup,label))+' · L '+peso(ocChoiceCost(rows,'L',tempGroup,label));}).join(' | '):'cost/serving — S '+peso(ocChoiceCost(rows,'S'))+' · M '+peso(ocChoiceCost(rows,'M'))+' · L '+peso(ocChoiceCost(rows,'L'));}
...
function ocChoiceCost(ings,size,tempGroup,tempLabel){var c=0;(ings||[]).forEach(function(r){if(!r||!r.ing||tempLabel&&sharedChoiceRowScopes(r,tempGroup).indexOf(tempLabel)<0)return;var q=r['qty'+size];if(q==null||q==='')q=0;c+=(Number(q)||0)*ingCost(r.ing);});return c;}
...
function drawRecipeEditor(item){
  var ed=document.getElementById('recEditor'); if(!ed)return;
  var d=recipeDraft,single=recipeItemIsSingleServing(item);recipeMirrorSingleServingDraft(d,item);var size=single?'S':(recSize||'M');
  ...
  var baseRows=d.base.map(function(r,ix){
    ...
    return '<tr data-base-row><td>'+ingSelect(r.ing,'data-brow="'+ix+'" data-bfield="ing"')+scope.html+'</td>'
      ...
      +(single?'<td class="r">'+qc('S')+'</td>':'<td class="r">'+qc('S')+'</td><td class="r">'+qc('M')+'</td><td class="r">'+qc('L')+'</td>')
  ...
  var sizeBtns=single?'':['S','M','L'].map(function(sz){return '<button class="pz-btn '+(sz===size?'ok':'sec')+'" data-recsize="'+sz+'" style="padding:0.25rem 0.8rem;">'+sz+'</button>';}).join(' ');
  var grand=baseTotal;
    ...
    var choices=(g.choices||[]).filter(function(c){return caSelected(g,c);}).map(function(c){
      ...
      var ingRows=rows.map(function(r,ix){
        ...
        return '<tr data-carow data-ca-g="'+esc(g.id)+'" data-ca-l="'+esc(lk)+'" data-ca-label="'+esc(c.label)+'" data-ca-op="'+esc(r.op||'')+'" data-ca-when="'+esc(JSON.stringify(r.when||{}))+'" data-ca-temp-group="'+esc(temperatureGroup&&temperatureGroup.id||'')+'" data-ca-ix="'+ix+'"'+(scope.active?'':' style="opacity:.58"')+'>'
          ...
          +'<td class="r"><input class="pz-in" type="number" step="any" min="0" style="width:100%;max-width:90px;text-align:right;" data-caf="dS" value="'+(r.dS!=null&&r.dS!==''?r.dS:'')+'" placeholder="0"/></td>'
          +(single?'':'<td class="r"><input class="pz-in" type="number" step="any" min="0" style="width:100%;max-width:90px;text-align:right;" data-caf="dM" value="'+(r.dM!=null&&r.dM!==''?r.dM:'')+'" placeholder="0"/></td><td class="r"><input class="pz-in" type="number" step="any" min="0" style="width:100%;max-width:90px;text-align:right;" data-caf="dL" value="'+(r.dL!=null&&r.dL!==''?r.dL:'')+'" placeholder="0"/></td>')
      ...
      return '<div data-ca-choice data-ca-g="'+esc(g.id)+'" data-ca-l="'+esc(lk)+'" style="border-top:1px solid var(--cd);padding:0.4rem 0;">'
        ...
        +'<span data-cacost="'+esc(g.id)+'|'+esc(lk)+'" style="font-size:0.72rem;color:var(--tl);">'+(single?'cost/serving — '+peso(caDraftCost(rows,'S')):'extra — S '+peso(caDraftCost(rows,'S'))+' · M '+peso(caDraftCost(rows,'M'))+' · L '+peso(caDraftCost(rows,'L')))+'</span></div>'
        +(ingRows?'<table class="pz-tbl" style="margin:0.3rem 0;width:100%;table-layout:fixed;">'+(single?'':recipeCols)+'<thead><tr><th>Extra ingredient</th><th>Recipe unit</th>'+(single?'<th class="r">Quantity/serving</th>':'<th class="r">S</th><th class="r">M</th><th class="r">L</th>')+'<th class="r">Amount</th></tr></thead><tbody>'+ingRows+'</tbody></table>':'')
  ...
  var previewNorm=Costing().normalizeRecipe(recipeDraftRaw(d,item),inventoryMap);
  ...
  var effectivePrice=Number(single?item.priceS:(size==='L'?item.priceL:(size==='M'?item.priceM:item.priceS)))||0;
  caGroupsAll.forEach(function(g){var v=selState[g.id];(Array.isArray(v)?v:(v?[v]:[])).forEach(function(lb){selectedLabels.push(lb);var choice=(g.choices||[]).filter(function(c){return c.label===lb;})[0]||{};effectivePrice+=Number(choice.price)||0;selectedChoices.push({groupId:g.id,groupName:g.name||'Option',label:lb});});});
  ...
  if(single){var flatRows=Object.keys(effective).map(function(k){var r=effective[k],src=effectiveSection(r);return '<tr><td><b>'+esc(r.name)+'</b><small style="display:block;color:var(--tl)">'+esc(src)+'</small></td><td class="r">'+num(r.qS)+' '+esc(r.unit)+'</td><td class="r">'+peso(r.unitCost)+'</td><td class="r">'+peso(r.cS)+'</td></tr>';}).join('');effectiveTable='<div style="overflow-x:auto;margin-top:.7rem"><table class="pz-tbl" style="font-size:.76rem"><thead><tr><th>Ingredient and source</th><th class="r">Quantity/serving</th><th class="r">Unit cost</th><th class="r">Cost</th></tr></thead><tbody>'+flatRows+'</tbody><tfoot><tr><th>Total COGS per serving</th><th></th><th></th><th class="r">'+peso(effectiveTotals.S)+'</th></tr></tfoot></table></div>';}
  ...
  var drinkCard='<div class="pz-card" style="margin-bottom:1rem;border:2px solid var(--bd);">'
    +'<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.4rem;margin-bottom:0.4rem;"><span style="font-weight:700;color:var(--bd);">'+(single?'Effective Price &amp; Cost':'Effective Recipe')+' · '+esc(item.name)+'</span>'+(single?'<span class="pz-lbl">Effective selling price · '+peso(effectivePrice)+'</span>':'<span><span class="pz-lbl" style="display:inline;margin-right:0.4rem;">Preview size</span>'+sizeBtns+'</span>')+'</div>'
    ...
    +'<div style="border-top:2px solid var(--bd);margin-top:0.4rem;padding-top:0.5rem;display:flex;justify-content:space-between;align-items:center;"><span style="font-weight:700;color:var(--bd);">COST PER '+(single?'SERVING':'DRINK / '+size)+'</span><span style="font-weight:700;font-size:1.2rem;color:var(--bd);">'+peso(drinkTotal)+'</span></div>'+(single?'<div style="display:flex;justify-content:space-between;margin-top:.3rem"><span>Gross profit · '+num(effectivePrice?Math.round(((effectivePrice-drinkTotal)/effectivePrice)*1000)/10:0)+'%</span><b>'+peso(effectivePrice-drinkTotal)+'</b></div>':'')
    +'</div>';
  ed.innerHTML=drinkCard
    +'<div class="pz-card" style="margin-bottom:1rem;">'
      +'<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.5rem;margin-bottom:0.6rem;"><div style="font-weight:600;color:var(--bd);">Recipe for “'+esc(item.name)+'”'+(cat?' · <span style="color:var(--tl);font-size:0.82rem;">'+esc(A().getCatLabel?A().getCatLabel(cat):cat)+(ct?' ('+ct+')':'')+'</span>':'')+'</div>'+(single?'<span class="pz-lbl">Quantity per serving</span>':'<div><span class="pz-lbl" style="display:inline;margin-right:0.4rem;">Cost for size</span>'+sizeBtns+'</div>')+'</div>'
      +'<label style="display:inline-flex;align-items:center;gap:0.35rem;font-size:0.76rem;color:var(--tl);margin-bottom:0.5rem;cursor:pointer;"><input type="checkbox" id="recNoNeed"'+(item.noRecipe?' checked':'')+'/> No recipe needed (hide from the not-costed flag)</label>'
      +(ings().length?'':'<p class="pz-low" style="font-size:0.8rem;">Add items in the Inventory tab first.</p>')
      +'<span class="pz-lbl">Shared base ingredients — select what this '+(single?'item':'drink')+' uses</span>'+sharedPicker
      ...
      +'<table class="pz-tbl" style="margin-bottom:0.4rem;width:100%;table-layout:fixed;">'+(single?'':recipeCols)+'<thead><tr><th>Ingredient</th><th>Recipe unit</th>'+(single?'<th class="r">Quantity/serving</th>':'<th class="r">S</th><th class="r">M</th><th class="r">L</th>')+'<th class="r">Amount'+(single?'':' ('+size+')')+'</th></tr></thead><tbody>'+(baseRows||'<tr><td colspan="'+(single?'4':'6')+'" style="color:var(--tl);padding:0.5rem;">No ingredients yet.</td></tr>')+'</tbody></table>'
      ...
      +'<div style="border-top:2px solid var(--bd);margin-top:0.8rem;padding-top:0.6rem;display:flex;justify-content:space-between;align-items:center;"><span style="font-weight:700;color:var(--bd);">BASE COST / '+(single?'SERVING':size)+'</span><span style="font-weight:700;font-size:1.1rem;color:var(--bd);">'+peso(grand)+'</span></div>'
  ...
  function syncChoiceAdd(){
    ...
    ed.querySelectorAll('[data-carow]').forEach(function(tr){
      ...
      var qS=v('dS');next[g][lk].ings.push({ing:ing,unit:u,dS:qS,dM:single?qS:v('dM'),dL:single?qS:v('dL'),op:op,when:Object.keys(when).length?when:undefined});
    });
    d.choiceAdd=next;
  }
...
function packSnapshotData(){
  var a=A(),payload={recipes:packDraftClone(recipesMap||{}),menuItems:packDraftClone((a&&a.menuItemsMap)||{}),optionGroups:packDraftClone((a&&a.optionGroupsMap)||{}),packagingRules:packDraftClone(packagingRulesMap||{}),packagingAssignments:packDraftClone((window.__posSettings&&window.__posSettings.packagingAssignments)||{})};
  if(!Object.keys(payload.recipes).length||!Object.keys(payload.menuItems).length)throw new Error('Recipe data is still loading.');
  return payload;
}
...
function posOrderItemsHtml(o){
  ...
  var current=Array.isArray(o.correctedLineItems)&&o.correctedLineItems.length?o.correctedLineItems:o.lineItems,lines=Array.isArray(current)&&current.length?current.map(function(li){return{name:li.name||li.itemKey||'Item',size:li.size||'',options:Array.isArray(li.optLabels)?li.optLabels:[],qty:Math.max(1,Number(li.qty)||1)};}):posLegacyItemLines(o.correctedItems||o.items).map(function(text){var qm=text.match(/\s+x(\d+)\s*$/i);return{name:qm?text.slice(0,qm.index).trim():text,size:'',options:[],qty:qm?Math.max(1,Number(qm[1])||1):1};});
...
function renderActiveOrders(){
  ...
  function completedCard(o){var channel=activeChannelLabel(o),pre=Number(o.preCompletionCashRefund&&o.preCompletionCashRefund.amount)||0,instore=o.channel==='instore'||!o.channel,canChange=instore&&o.preparationStatus==='not_prepared'&&!o.completedOrderCorrectionId&&!Number(o.refundAmount)&&completedCorrectionPaymentKind(o),actions=canChange?'<button class="pz-btn sec" data-completed-correction="'+esc(o.id)+'">Change completed order</button>':'';return '<article class="pos-shift-sale channel-'+esc(o.channel||'instore')+'"><div class="pos-shift-sale-head"><div><span class="pos-channel-tag">'+esc(channel)+'</span><b>#'+esc(o.id)+'</b><small>'+esc(o.name||o.staff||'Walk-in customer')+'</small></div><div><strong>'+peso((Number(o.total)||0)-(Number(o.refundAmount)||0))+'</strong><span>✓ '+esc(o.completedOrderCorrectionId?'Corrected':o.status)+'</span></div></div>'+posOrderItemsHtml(o)+'<div class="pos-active-foot"><span>'+esc(o.payment||'Payment recorded')+(Number(o.refundAmount)>0?' · Cash refund '+peso(o.refundAmount):pre?' · Cash refund '+peso(pre):'')+'</span>'+actions+'</div></article>';}
  ...
  root.querySelectorAll('[data-completed-correction]').forEach(function(b){b.onclick=function(){var o=onlineOrdersMap[b.getAttribute('data-completed-correction')];if(o)beginCompletedOrderCorrection(o);};});
}
...
function renderPosCart(options){
    ...
    +(correction?'<div style="margin-top:0.7rem;padding:0.55rem;background:#e8f5ec;border:1px solid #b8dfc4;border-radius:6px;font-size:0.78rem;color:#155724;"><b>Original '+esc(correction.paymentKind)+' payment stays recorded.</b><br>POS will calculate any cash refund from '+peso(correction.originalTotal)+'.</div>':isPlat
      ...
      : '<div style="margin-top:0.7rem;display:flex;justify-content:space-between;align-items:center;"><span class="pz-lbl" style="margin:0;">Payment</span><label style="font-size:0.74rem;color:var(--tl);cursor:pointer;"><input type="checkbox" id="posSplitChk"/> Split</label></div>'
        +'<div id="posPaySingle"><select class="pz-in" id="posPay" style="margin-top:0.3rem;">'+posActiveMethods().map(function(m){return '<option value="'+m.name+'">'+m.name+'</option>';}).join('')+'</select>'
          ...
          +'<div id="posRefWrap" style="display:none;margin-top:0.5rem;"><div id="posAccountWrap" style="margin-bottom:0.45rem;"></div><span class="pz-lbl">Confirmed amount received ₱</span><input class="pz-in" id="posPaidAmount" type="number" min="0" step=".01" placeholder="Defaults to order total"/><span class="pz-lbl" style="margin-top:.45rem;">Transaction reference — required</span><input class="pz-in" id="posPayRef" placeholder="e.g. wallet ref / bank txn ref"/><div style="font-size:0.72rem;color:var(--tl);margin-top:0.2rem;">Enter what actually arrived in GCash or the bank. If the corrected order is lower, POS will calculate the cash refund before completion.</div></div></div>'
  ...
  function draftElectronicPayments(){
    ...
    var method=pay?pay.value:'Cash',acct=document.getElementById('posReceivingAccount'),received=Math.round((Number((document.getElementById('posPaidAmount')||{}).value)||tot)*100)/100;return isCashMethod(method)?[]:[resolvedPayment(method,acct&&acct.value,received,String((document.getElementById('posPayRef')||{}).value||'').trim())].filter(Boolean);
  }
  function refreshChargeAction(){
    ...
    if(shift&&!posSellerState().ok){button.textContent='Join the shift to take payment';button.style.background='';}
  ...
  function renderReceivingAccount(){var wrap=document.getElementById('posAccountWrap');if(!wrap)return;var opts=paymentAccountOptions(pay.value),saved=posDraft.posReceivingAccount&&posDraft.posReceivingAccount.value;if(!opts.length){wrap.innerHTML='<div style="color:#c0392b;font-weight:600;font-size:.75rem;">No receiving account is assigned in POS Settings. This method cannot be charged.</div>';return;}wrap.innerHTML='<span class="pz-lbl">Receiving account</span><select class="pz-in" id="posReceivingAccount">'+opts.map(function(a){return '<option value="'+esc(a.id)+'"'+(a.id===saved?' selected':'')+'>'+esc(a.name)+'</option>';}).join('')+'</select>'+(opts.length>1?'<div style="font-size:.7rem;color:var(--tl);margin-top:.15rem;">Select the account where this customer transfer was actually received.</div>':'');var sel=document.getElementById('posReceivingAccount');if(sel)sel.onchange=invalidatePaymentVerification;}
  function syncPayMethod(changed){var isc=isCashMethod(pay.value);document.getElementById('posCashWrap').style.display=isc?'block':'none';var rw=document.getElementById('posRefWrap');if(rw)rw.style.display=isc?'none':'block';if(!isc)renderReceivingAccount();updateKeep();if(changed)invalidatePaymentVerification();}pay.onchange=function(){syncPayMethod(true);};syncPayMethod(false);
  ...
  function renderSplit(){
    ...
    cont.innerHTML=splitRows.map(function(r,i){var opts=posActiveMethods().map(function(m){return '<option'+(r.method===m.name?' selected':'')+'>'+m.name+'</option>';}).join('');var row='<div style="display:flex;gap:0.3rem;margin-bottom:0.3rem;"><select class="pz-in" data-pm="'+i+'" style="flex:1;">'+opts+'</select><input class="pz-in" data-pa="'+i+'" type="number" step="any" style="width:100px;" value="'+r.amount+'"/>'+(splitRows.length>1?'<button class="pz-btn warn" data-pd="'+i+'" style="padding:0.2rem 0.45rem;">✕</button>':'')+'</div>';
      ...
      return row;}).join('');
    function splitInfo(){var assigned=splitRows.reduce(function(s,r){return s+(Number(r.amount)||0);},0);document.getElementById('posSplitInfo').innerHTML='Assigned '+peso(assigned)+' / Total '+peso(grandTotal())+' · '+(Math.abs(assigned-grandTotal())<0.01?'<span style="color:#2a9d5c;">balanced</span>':'<span style="color:#e63946;">off by '+peso(grandTotal()-assigned)+'</span>');}
    splitInfo();
    ...
    cont.querySelectorAll('[data-pa]').forEach(function(inp){inp.oninput=function(){splitRows[+inp.getAttribute('data-pa')].amount=Number(inp.value)||0;posPaymentVerification=null;splitInfo();refreshChargeAction();};});
  ...
  document.getElementById('posCharge').onclick=async function(){
    ...
    if(splitChk.checked){ var assigned=splitRows.reduce(function(s,r){return s+(Number(r.amount)||0);},0); if(Math.abs(assigned-tot)>0.01){alert('Split payments must add up to the total.');return;}if(splitRows.some(function(r){return !(Number(r.amount)>0);})){alert('Each split payment must be greater than zero.');return;} if(splitRows.some(function(r){return !isCashMethod(r.method)&&!String(r.ref||'').trim();})){alert('Enter a reference number for every non-cash payment before charging.');return;}if(splitRows.some(function(r){return !isCashMethod(r.method)&&!resolvedPayment(r.method,r.receivingAccountId,r.amount,r.ref);})){alert('Assign a valid receiving account to every non-cash payment in POS Settings.');return;}
      ...
      if(!isc){ var ref1=(document.getElementById('posPayRef').value||'').trim(),acct1=(document.getElementById('posReceivingAccount')||{}).value||'',received=Math.round((Number((document.getElementById('posPaidAmount')||{}).value)||tot)*100)/100; if(!ref1){alert('Enter the '+m+' reference number before charging.');return;}var resolved=resolvedPayment(m,acct1,received,ref1);if(!resolved){alert('Assign a valid receiving account to '+m+' in POS Settings before charging.');return;}payments=[resolved]; }
    ...
    var verificationSignature=paymentVerificationSignature(payments,tot),direct=directPaymentRows(payments),verificationPolicy=paymentVerificationPolicy(payments),cashierVerification=null,receiptSignature=JSON.stringify(direct.map(function(p){return[p.method,p.receivingAccountId,Math.round((+p.amount||0)*100)/100,p.ref];})),receiptVerified=posPaymentVerification&&posPaymentVerification.receiptSignature===receiptSignature;
    if(direct.length&&verificationPolicy==='cashier_manager'&&(!posPaymentVerification||posPaymentVerification.signature!==verificationSignature&&!receiptVerified)){
      var receivedTotal=Math.round(direct.reduce(function(s,p){return s+(+p.amount||0);},0)*100)/100;try{cashierVerification=await cashierVerificationGate(payments,receivedTotal,'In-store payment received');}catch(e){return;}
      posPaymentVerification={required:true,reference:cashierVerification.reference||'',signature:paymentVerificationSignature(payments,tot),receiptSignature:receiptSignature,receivedAmount:receivedTotal};var paidField=document.getElementById('posPaidAmount');if(paidField&&!paidField.value){paidField.value=receivedTotal;posDraft.posPaidAmount={value:String(receivedTotal),checked:false,type:'number'};}
    ...
    var prepaidRefund=null,ePaid=Math.round(direct.reduce(function(s,p){return s+(+p.amount||0);},0)*100)/100,aPaid=Math.round(payments.reduce(function(s,p){return s+(+p.amount||0);},0)*100)/100,excess=Math.round((aPaid-tot)*100)/100;
    if(direct.length&&excess<-.009){alert('Payments are '+peso(-excess)+' short of the corrected order total.');return;}
    if(excess>.009){if(splitChk.checked||direct.length!==1||payments.length!==1){alert('Before-completion cash refund supports one GCash, Maya, or bank payment only.');return;}if(window.__online===false){alert('Reconnect before recording or paying this cash refund.');return;}if(verificationPolicy!=='cashier_manager'||!receiptVerified){alert('Verify the full electronic payment before returning cash.');return;}var refundDenoms={};if(denomTrackingOn()){var changePlan=makeChange(excess,shiftDrawer());if(!changePlan.ok){alert('The drawer cannot provide exactly '+peso(excess)+' without using unavailable notes or coins.');return;}refundDenoms=changePlan.denoms;}var rv;try{rv=await F().run({title:'Complete corrected sale · cash refund',subtitle:'Received '+peso(ePaid)+' electronically · corrected sale '+peso(tot)+' · return '+peso(excess)+' cash',submitLabel:'Complete sale and record refund',busyLabel:'Saving sale and refund…',fields:[{name:'reason',label:'What changed?',type:'textarea',required:true,maxLength:300,placeholder:'Example: Large Iced Latte changed to Regular Iced Latte'},{name:'customerAcknowledgement',label:'Customer acknowledgement',required:true,maxLength:160,placeholder:'Customer name, receipt note, or signature reference'},{name:'confirmed',label:'I understand cash is handed over only after POS confirms this transaction',type:'checkbox',required:true}]},function(v){return v;});}catch(e){return;}prepaidRefund={amount:excess,paidAmount:ePaid,refundMethod:'Cash',reason:rv.reason,customerAcknowledgement:rv.customerAcknowledgement,shiftId:shift.id,denoms:refundDenoms};}
    ...
    await chargeSale(sub,tot,payments,null,discountApproval,cashierVerification,prepaidRefund);
...
async function completeCompletedOrderCorrection(total){
  ...
  var a=A(),correctionApproval,refundApproval;if(!a||!a.managerApproval)return alert('Independent manager approval is unavailable. Refresh POS and try again.');try{correctionApproval=await a.managerApproval('correct_completed_order',c.orderId,c.originalTotal,'Approve completed-order item correction: '+v.reason,{requireIndependent:true});if(refund>0)refundApproval=await a.managerApproval('completed_order_cash_refund',c.orderId,refund,'Approve cash refund for completed-order correction: '+v.reason,{requireIndependent:true});}catch(error){if(String((error&&error.message)||error).indexOf('cancelled')<0)alert('Manager approval failed: '+String((error&&error.message)||error));return;}
  var response;try{response=await a.processOrderAdjustment({action:'complete_order_item_correction',orderId:c.orderId,requestId:c.requestId,lineItems:lines,expectedTotal:total,reason:v.reason,customerAcknowledgement:v.customerAcknowledgement,drawerDelta:drawerDelta,approvalId:correctionApproval.approvalId,refundApprovalId:refundApproval&&refundApproval.approvalId||''});}catch(error){alert('The completed order was NOT changed. Do not hand over cash. '+String(error&&error.message||error));return;}var data=response&&response.data||response||{};
