...
function renderActiveOrders(){
  ...
  function completedCard(o){var channel=activeChannelLabel(o),pre=Number(o.preCompletionCashRefund&&o.preCompletionCashRefund.amount)||0,instore=o.channel==='instore'||!o.channel,notPrepared=instore&&o.preparationStatus==='not_prepared',canChange=notPrepared&&!o.completedOrderCorrectionId&&!Number(o.refundAmount)&&completedCorrectionPaymentKind(o),prep=notPrepared?'Not prepared':o.preparationStatus==='preparing'?'Preparing':o.preparationStatus==='not_applicable'?'Completed':'Preparation status unavailable',actions=notPrepared?'<span style="display:flex;gap:.35rem;flex-wrap:wrap;">'+(canChange?'<button class="pz-btn sec" data-completed-correction="'+esc(o.id)+'">Change completed order</button>':'')+'<button class="pz-btn ok" data-start-instore-prep="'+esc(o.id)+'">Start preparing</button></span>':'';return '<article class="pos-shift-sale channel-'+esc(o.channel||'instore')+'"><div class="pos-shift-sale-head"><div><span class="pos-channel-tag">'+esc(channel)+'</span><b>#'+esc(o.id)+'</b><small>'+esc(o.name||o.staff||'Walk-in customer')+'</small></div><div><strong>'+peso((Number(o.total)||0)-(Number(o.refundAmount)||0))+'</strong><span>✓ '+esc(o.completedOrderCorrectionId?'Corrected':o.status)+' · '+esc(prep)+'</span></div></div>'+posOrderItemsHtml(o)+'<div class="pos-active-foot"><span>'+esc(o.payment||'Payment recorded')+(Number(o.refundAmount)>0?' · Cash refund '+peso(o.refundAmount):pre?' · Cash refund '+peso(pre):'')+'</span>'+actions+'</div></article>';}
  ...
  root.querySelectorAll('[data-start-instore-prep]').forEach(function(b){b.onclick=function(){var id=b.getAttribute('data-start-instore-prep');b.disabled=true;b.textContent='Starting…';A().processOrderAdjustment({action:'start_instore_preparation',orderId:id}).then(function(){(window.accazaToast||function(){})('Preparation started · order changes are now locked','ok');}).catch(function(e){alert('Could not start preparation: '+((e&&e.message)||e));b.disabled=false;renderActiveOrders();});};});
}
