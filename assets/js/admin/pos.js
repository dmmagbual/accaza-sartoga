...
function purchaseStatusLabel(p){if(p.reversed)return 'Reversed';if(p.payMode==='advance')return 'Allocated from payment pending inventory allocation';if(p.payMode==='paid')return 'Paid';if(p.payMode==='account')return 'On account';if(p.payMode==='pending')return 'Invoice pending';return 'Legacy — liability missing';}
