...
function purchaseSupplierById(id){return id&&supplierMap[id]&&supplierMap[id].active!==false?Object.assign({id:id},supplierMap[id]):null;}
...
function purchaseAdvanceMatchesSupplier(x,supplierId,supplier){if(x.supplierId&&supplierId)return x.supplierId===supplierId;if(x.supplierId||supplierId)return false;var wanted=purchaseSupplierKey(supplier);return !wanted||purchaseSupplierKey(x.supplierName||x.recipient)===wanted;}
...
function renderPurchases(){
  ...
  var cf=window.__cf; var accs=(cf&&cf.accounts&&cf.accounts())||[],payAccs=accs.filter(function(x){return !x.disabled;});
  var selectedSupplier=purchaseSupplierById(P.supplierId);if(selectedSupplier)P.supplier=selectedSupplier.name||'';var advances=openPurchaseAdvances(P.supplierId,P.supplier),advanceOpts=advances.map(function(x){return '<option value="'+esc(x.id)+'"'+(P.advanceId===x.id?' selected':'')+'>'+peso(x.remaining)+' remaining of '+peso(x.amount)+' · '+esc(x.recipient||'')+' · '+esc(x.purpose||'')+' ('+(x.advanceSource==='revolving'?'Cash Payments':esc(x.staff||'register shift'))+')</option>';}).join('');
