...
function verifyOnlinePayment(oid,button){
  var o=onlineOrdersMap[oid],a=A();if(!o){alert('Order not found. Refresh the POS and try again.');return;}if(o.paymentStatus!=='pending'){alert('This payment is no longer awaiting verification.');return;}if(!a||!a.processOrderAdjustment||!a.managerApproval){alert('Payment verification is unavailable. Refresh the POS and try again.');return;}
  var old=button&&button.textContent;if(button){button.disabled=true;button.textContent='Verifying…';}
  a.managerApproval('confirm_payment',oid,Number(o.total)||0,'Verify payment received').then(function(ap){return a.processOrderAdjustment({action:'confirm_payment',orderId:oid,approvalId:ap.approvalId});}).then(function(){if(window.__posLog)window.__posLog('verify-payment',oid,peso(o.total)+' · '+(o.payment||''));(window.accazaToast||function(){})('Payment verified','ok');}).catch(function(e){if(String((e&&e.message)||e).indexOf('cancelled')<0)alert('Payment confirmation failed: '+((e&&e.message)||e));}).finally(function(){if(button&&document.body.contains(button)){button.disabled=false;button.textContent=old;}});
}
...
function renderActiveOrders(){
  ...
  function card(o){var stage=o.status==='Pending'?'Confirmed':o.status,idx=stages.indexOf(stage),next=o.status==='Pending'?'Confirmed':o.status==='Confirmed'?'Preparing':o.status==='Preparing'?'Ready':o.status==='Ready'?'Completed':'',channel=activeChannelLabel(o);
...
function renderOnlineOrders(){
  ...
  function card(o){var verified=o.paymentStatus==='confirmed',captured=!!o.shiftId,proof=o.proofPath?'<button class="pz-btn sec" data-online-proof="'+esc(o.id)+'">View payment proof</button>':'',action='';
...
function renderPosCart(){
  ...
  document.getElementById('posCharge').onclick=async function(){
    ...
    await chargeSale(sub,tot,payments,null,discountApproval);
...
function chargeSale(sub,total,payments,platform,discountApproval){
