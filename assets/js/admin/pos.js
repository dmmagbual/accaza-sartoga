...
var INV_ACCOUNT_PAIRS=[['1200','5000','Coffee & Beans'],['1210','5010','Milk & Dairy'],['1220','5020','Syrups & Flavors'],['1230','5040','Cups & Packaging'],['1240','5030','Food & Pastries']];
function invCatDefaultAccounts(c){var n=String(c&&c.name||'').toLowerCase();if(/coffee|bean/.test(n))return['1200','5000'];if(/milk|dairy/.test(n))return['1210','5010'];if(/syrup|flavor|powder|tea/.test(n))return['1220','5020'];if(/packag|cup|lid|straw|napkin|container/.test(n))return['1230','5040'];if(/food|pastr|bakery|kitchen|meal/.test(n))return['1240','5030'];return['',''];}
function invCatAccounts(c){var d=invCatDefaultAccounts(c);return{inventoryAccount:String(c&&c.inventoryAccount||d[0]||''),cogsAccount:String(c&&c.cogsAccount||d[1]||'')};}
function invAccountOptions(selected,side){return '<option value="">— Unmapped —</option>'+INV_ACCOUNT_PAIRS.map(function(p){var v=p[side];return '<option value="'+v+'"'+(selected===v?' selected':'')+'>'+v+' · '+esc((side===0?'Inventory — ':'COGS — ')+p[2])+'</option>';}).join('');}
function seedInvCats(){ if(Object.keys(invCatsMap()).length)return; var a=A(); if(!a)return; var seed={}; [['Coffee','cogs','1200','5000'],['Milk','cogs','1210','5010'],['Syrup','cogs','1220','5020'],['Powder','cogs','1220','5020'],['Tea','cogs','1220','5020'],['Packaging','cogs','1230','5040'],['Food & Pastries','cogs','1240','5030'],['Cleaning','overhead','',''],['Office','overhead','','']].forEach(function(p,i){seed['cat_'+p[0].toLowerCase().replace(/[^a-z0-9]+/g,'_')]={name:p[0],kind:p[1],inventoryAccount:p[2],cogsAccount:p[3],order:i};}); a.update(a.ref(a.db,'posSettings/invCategories'),seed).catch(function(){}); }
...
function renderInventory(){
  ...
  var unmapped=list.filter(function(i){var c=invCatsMap()[i.category],m=invCatAccounts(c);return !c||(c.kind!=='overhead'&&(!m.inventoryAccount||!m.cogsAccount));});
  ...
  var rows=shown.map(function(i){
    ...
    return '<tr>'
      ...
      +'<td style="font-size:0.78rem;">'+(i.category?(esc(invCatName(i.category))+(invCatKind(i.category)==='overhead'?' <span style="color:#8a5a00;font-size:0.66rem;">overhead</span>':(function(){var m=invCatAccounts(invCatsMap()[i.category]);return m.inventoryAccount&&m.cogsAccount?' <span style="color:#267354;font-size:0.66rem;">'+m.inventoryAccount+' / '+m.cogsAccount+'</span>':' <span style="color:#b44336;font-size:0.66rem;">unmapped</span>'})())):'<span style="color:var(--tl);">—</span>')+'</td>'
  ...
  root.innerHTML=
    ...
    +'<p class="pz-sub">Each inventory row is the common SKU used by recipes. Its category controls both the Inventory Asset and COGS account used in Finance / Books.'+(low.length?' <b class="pz-low">'+low.length+' low.</b>':'')+(neg.length?' <b class="pz-low">'+neg.length+' negative.</b>':'')+(uncat.length?' <b style="color:#8a5a00;">'+uncat.length+' uncategorized.</b>':'')+(unmapped.length?' <b style="color:#b44336;">'+unmapped.length+' without accounting mapping.</b>':'')+(missingBrand.length?' <b style="color:#8a5a00;">'+missingBrand.length+' recipe item'+(missingBrand.length===1?'':'s')+' without an approved purchasing brand.</b>':'')+'</p>'
...
function openCatManager(){
  ...
  function draw(){
    var cats=invCats();
    var rows=cats.map(function(c){var m=invCatAccounts(c);return '<div style="display:grid;grid-template-columns:1.2fr .8fr 1fr 1fr auto;gap:0.4rem;align-items:center;margin-bottom:0.35rem;" data-catrow="'+esc(c.id)+'"><input class="pz-in" data-cf="name" value="'+esc(c.name)+'"/><select class="pz-in" data-cf="kind"><option value="cogs"'+(c.kind!=='overhead'?' selected':'')+'>Inventory / COGS</option><option value="overhead"'+(c.kind==='overhead'?' selected':'')+'>Overhead</option></select><select class="pz-in" data-cf="inventory">'+invAccountOptions(m.inventoryAccount,0)+'</select><select class="pz-in" data-cf="cogs">'+invAccountOptions(m.cogsAccount,1)+'</select><button class="pz-btn warn" data-catdel="'+esc(c.id)+'" style="padding:0.2rem 0.5rem;">✕</button></div>';}).join('');
    mask.innerHTML='<div style="background:#fff;border-radius:10px;max-width:980px;width:100%;max-height:90vh;overflow:auto;padding:1.2rem;">'
      ...
      +'<p class="pz-sub" style="margin:0.3rem 0 0.6rem;">Every inventory category selects the asset account holding its stock value and the matching COGS account used when recipes consume it. Overhead categories do not use inventory accounts.</p><div style="display:grid;grid-template-columns:1.2fr .8fr 1fr 1fr auto;gap:.4rem;font-size:.68rem;font-weight:700;color:var(--tl);margin-bottom:.3rem;"><span>Category</span><span>Treatment</span><span>Inventory asset</span><span>COGS account</span><span></span></div>'
    ...
    mask.querySelector('#cmSave').onclick=function(){var ups={},bad=[];mask.querySelectorAll('[data-catrow]').forEach(function(r,ix){var id=r.getAttribute('data-catrow'),nm=(r.querySelector('[data-cf="name"]').value||'').trim(),k=r.querySelector('[data-cf="kind"]').value,ia=r.querySelector('[data-cf="inventory"]').value,ca=r.querySelector('[data-cf="cogs"]').value;if(k!=='overhead'&&(!ia||!ca))bad.push(nm||id);if(nm)ups[id]={name:nm,kind:k,inventoryAccount:k==='overhead'?'':ia,cogsAccount:k==='overhead'?'':ca,order:ix};});if(bad.length){alert('Choose both accounting accounts for: '+bad.join(', '));return;}a.update(a.ref(a.db,'posSettings/invCategories'),ups).then(function(){if(isTab('inventory'))renderInventory();alert('Categories and account mappings saved. Use Sync all Finance transactions in Books to reclassify historical COGS.');}).catch(function(e){alert('Could not save: '+((e&&e.code)||e));});};
