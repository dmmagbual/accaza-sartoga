...
function drawRecipeEditor(item){
  ...
  var effectiveRows=Object.keys(effective).map(function(k){var r=effective[k],src=String(r.source).indexOf('base_shared')===0?'Shared base':String(r.source).indexOf('base_')===0?'Recipe-specific override':r.source==='packaging'?'Serve-style packaging':(r.source==='option_global'?'Shared choice':'Recipe-specific choice')+(r.label?' · '+r.label:'');var canOverride=r.source==='base_shared'||r.source==='option_global';var action=canOverride?'<button class="pz-btn sec" data-effective-override="'+esc(r.ingredientId)+'" data-override-source="'+esc(r.source)+'" data-override-group="'+esc(r.groupId)+'" data-override-label="'+esc(r.label)+'" style="padding:.18rem .55rem">Override</button>':(String(r.source).indexOf('base_override')===0||r.source==='option_recipe'?'<span style="color:var(--tl)">Overridden</span>':'');return '<tr><td><b>'+esc(r.name)+'</b><small style="display:block;color:var(--tl)">'+esc(src)+'</small></td><td class="r">'+num(r.qS)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cS)+'</small></td><td class="r">'+num(r.qM)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cM)+'</small></td><td class="r">'+num(r.qL)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cL)+'</small></td><td class="r">'+peso(r.unitCost)+'</td><td class="r">'+action+'</td></tr>';}).join('');
  var effectiveTable='<div style="overflow-x:auto"><table class="pz-tbl" style="font-size:.76rem"><thead><tr><th>Ingredient and source</th><th class="r">S qty / value</th><th class="r">M qty / value</th><th class="r">L qty / value</th><th class="r">Unit cost</th><th class="r">Action</th></tr></thead><tbody>'+(effectiveRows||'<tr><td colspan="6">Select shared ingredients or add a recipe-specific ingredient.</td></tr>')+'</tbody><tfoot><tr><th>Effective total</th><th class="r">'+peso(effectiveTotals.S)+'</th><th class="r">'+peso(effectiveTotals.M)+'</th><th class="r">'+peso(effectiveTotals.L)+'</th><th></th><th></th></tr></tfoot></table></div>';
  ...
  var calcGroups=caGroupsAll.map(function(g){var sv=selState[g.id];var isMulti=g.type==='multi';
    var control=isMulti?(g.choices||[]).map(function(c){var on=Array.isArray(sv)&&sv.indexOf(c.label)>-1;return '<label style="display:inline-flex;gap:.3rem;align-items:center;margin:0 .7rem .3rem 0"><input type="checkbox" data-rcmulti-check="'+esc(g.id)+'" data-rclabel="'+esc(c.label)+'"'+(on?' checked':'')+'> '+esc(c.label)+'</label>';}).join(''):'<select class="pz-in" data-rcselect="'+esc(g.id)+'">'+(g.choices||[]).map(function(c){return '<option'+(sv===c.label?' selected':'')+'>'+esc(c.label)+'</option>';}).join('')+'</select>';
  ...
  ed.querySelectorAll('[data-rcselect]').forEach(function(s){s.onchange=function(){syncAll();window.__recCostSel=window.__recCostSel||{};window.__recCostSel[s.getAttribute('data-rcselect')]=s.value;drawRecipeEditor(item);};});
  ...
  ed.querySelectorAll('[data-effective-override]').forEach(function(b){b.onclick=function(){syncAll();var ing=b.getAttribute('data-effective-override'),source=b.getAttribute('data-override-source'),gid=b.getAttribute('data-override-group'),label=b.getAttribute('data-override-label');if(source==='base_shared'){var row=sharedLibrary.filter(function(x){return x.ing===ing;})[0],inv=inventoryMap[ing]||{};if(row&&!d.base.some(function(x){return x.ing===ing;}))d.base.push({ing:ing,unit:inv.unit||'',dS:row.qtyS,dM:row.qtyM,dL:row.qtyL});}else if(source==='option_global'){var lk=optKey(label),entry=optCostStore()[gid]&&optCostStore()[gid][lk];if(entry){d.choiceAdd=d.choiceAdd||{};d.choiceAdd[gid]=d.choiceAdd[gid]||{};d.choiceAdd[gid][lk]={label:label,ings:(entry.ings||[]).map(function(row){var inv=inventoryMap[row.ing]||{};return {ing:row.ing,unit:inv.unit||'',dS:row.qtyS,dM:row.qtyM,dL:row.qtyL,op:row.op};})};}}drawRecipeEditor(item);setTimeout(function(){var details=document.querySelector('#recEditor details');if(details)details.open=true;},0);};});
