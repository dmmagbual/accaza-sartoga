...
function postPurchases(){
  ...
  Promise.resolve(Object.keys(seedUpdates).length?a.update(a.ref(a.db),seedUpdates):null).then(function(){return postMovements(movementRows);}).then(function(){return a.update(a.ref(a.db),updates);}).then(function(){
    ...
    if(window.__posLog)window.__posLog('purchase',(P.supplier||'Supplier'),lines.length+' item(s) · '+peso(invTotal)+(P.pay==='paid'?' · paid':P.pay==='owner_funded'?' · paid personally by '+(P.ownerName||'owner/partner')+' · '+(P.ownerTreatment==='reimburse'?'reimburse later':'capital contribution'):P.pay==='account'?' · on account':' · invoice pending'));
