...
function purchBlank(){return {mode:'existing',ing:'',skuId:'',recipeItem:true,newName:'',newUnit:'ml',newType:'base',newInventoryAccount:'',newCostAccount:'',expenseDescription:'',expenseAccount:'6075',brand:'',recvUnit:'',qty:'',costMode:'unit',unitCost:'',lineTotal:'',expiry:'',lot:''};}
...
function correctedPurchaseDraft(inv){return {supplier:inv.supplier||'',ref:inv.ref||'',date:window.AccazaDate.key(),by:inv.by||((window.__posShift&&window.__posShift.staff)||'Admin'),pay:inv.payMode==='none'?'pending':(inv.payMode||'pending'),acct:'',due:inv.due||'',ownerName:inv.ownerName||'',ownerTreatment:inv.ownerTreatment||'capital',lines:(inv.lines||[]).map(function(x){return x.lineType==='expense'?Object.assign(purchBlank(),{mode:'expense',expenseDescription:x.itemName||'',expenseAccount:x.expenseAccount||'6075',qty:x.qty||1,costMode:'total',lineTotal:x.total||''}):Object.assign(purchBlank(),{mode:'existing',ing:x.itemId||'',skuId:x.skuId||'',brand:x.skuBrand||'',recvUnit:x.unit||'',qty:x.qty||'',costMode:'total',lineTotal:x.total||''});})};}
var PURCH_UNITS=['ml','l','g','kg','pcs','pack','box','ream','roll','set'];
function purchCalc(ln){
  if(ln.mode==='expense'){var expenseQty=Number(ln.qty)||0,expenseTotal=(ln.costMode==='total')?(Number(ln.lineTotal)||0):expenseQty*(Number(ln.unitCost)||0);return {inv:null,stockUnit:'',recvUnit:'',qty:expenseQty,stockAdd:0,lineTotal:Math.round(expenseTotal*100)/100,before:0,oldCost:0,newCost:0};}
...
function purchasePaymentAccountLabel(id){if(id==='cash_on_hand'||id==='register')return 'Cash on Hand';if(id==='undeposited')return 'Undeposited Collection';var rows=(window.__cf&&window.__cf.accounts&&window.__cf.accounts())||[],row=rows.find(function(x){return x.id===id;});return row&&row.name||id||'cash account';}
...
function renderPurchases(){
  ...
  var lineHtml=P.lines.map(function(ln,i){
    ...
    var firstCell,typeCell='',unitCell,skuCell='',brandCell='',expenseLine=ln.mode==='expense';
    if(expenseLine){
    ...
    var prev=c?(expenseLine?('Expense · '+peso(c.lineTotal)+' · no inventory created'):('+'+num(c.stockAdd)+' '+esc(c.stockUnit)+' · new avg '+peso(c.newCost)+'/'+esc(c.stockUnit)+' · line '+peso(c.lineTotal))):'';
    return '<div class="purchase-line">'
      +'<div class="purchase-line-head"><div class="purchase-line-title"><span class="purchase-line-number">'+(i+1)+'</span><span>'+(expenseLine?'One-time expense':'Stock item')+'</span></div><div class="purchase-line-mode">'
      ...
      +'<div class="purchase-line-fields">'
        ...
        +typeCell
        +(expenseLine?'':skuCell)
        +(expenseLine||ln.mode==='new'?'':brandCell)
        ...
        +(expenseLine?'':'<div><span class="pz-lbl">Expiry (opt.)</span><input class="pz-in" type="date" data-pf="expiry" data-pi="'+i+'" value="'+esc(ln.expiry||'')+'" style="width:140px;"/></div><div><span class="pz-lbl">Lot # (opt.)</span><input class="pz-in" data-pf="lot" data-pi="'+i+'" value="'+esc(ln.lot||'')+'" placeholder="batch/lot" style="width:100px;"/></div>')
      +'</div>'
  ...
  var payBlock='<div class="purchase-section purchase-payment"><div class="purchase-section-head"><span class="purchase-step">2</span><div><b>Payment</b><small>Choose how this whole invoice will be settled.</small></div></div>'
    +'<div class="purchase-payment-options">'
      ...
      +'<label class="purchase-payment-option"><input type="radio" name="ppay" data-pf="pay" value="paid"'+(P.pay==='paid'?' checked':'')+(payAccs.length?'':' disabled')+'/><span><b>Paid now</b><small>'+(payAccs.length?'Balances match the Finance Books Balance Sheet. Undeposited Collection posts a disbursement; Cash Float is view-only.':'No available Balance Sheet cash account.')+'</small>'+(payAccs.length?('<select class="pz-in" id="purAcct">'+accOpts+'</select>'):'')+'</span></label>'
      ...
      +'<label class="purchase-payment-option"><input type="radio" name="ppay" data-pf="pay" value="advance"'+(P.pay==='advance'?' checked':'')+(advances.length?'':' disabled')+'/><span><b>Allocate from payment pending inventory allocation</b><small>'+(advances.length?'Link this detailed purchase without paying cash a second time.':'No unallocated supplier payments are available.')+'</small>'+(advances.length?('<select class="pz-in" id="purAdvance">'+advanceOpts+'</select>'):'')+'</span></label>'
  ...
  root.innerHTML='<div class="purchase-page-head"><div><div class="pz-h">Purchases <span>Goods received</span></div><p class="pz-sub">Record a supplier delivery and update stock in one clear receiving sheet.</p></div><div class="purchase-head-note">Approved brands keep receipts accurate while stock and costing stay pooled under the common item.</div></div>'
    ...
    +'</div></div>'+payBlock
    +'<div class="purchase-section purchase-items"><div class="purchase-section-head"><span class="purchase-step">3</span><div><b>Items received or expensed</b><small>Add stock received or a one-time supply used immediately. Expense lines skip brand and inventory.</small></div><button class="pz-btn sec purchase-add-line" id="purAddLine">＋ Add item</button></div><div class="purchase-lines">'+lineHtml+'</div></div>'
...
function postPurchases(){
  ...
  var lines=P.lines.filter(function(ln){return (ln.mode==='expense'?(ln.expenseDescription||'').trim():(ln.mode==='new'?(ln.newName||'').trim():ln.ing))&&(Number(ln.qty)||0)>0;});
  ...
  if(P.pay==='advance'&&!P.advanceId){alert('Select the purchase cash advance released by the cashier.');return;}
  ...
  Promise.resolve(Object.keys(seedUpdates).length?a.update(a.ref(a.db),seedUpdates):null).then(function(){return postMovements(movementRows);}).then(function(){return a.update(a.ref(a.db),updates);}).then(function(){
    ...
    alert('Purchase posted. Stock lines updated inventory; one-time expense lines posted only to Finance Books. ✅');
