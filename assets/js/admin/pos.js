...
function openRecipe(key){
  ...
  var _sel={}; (A().getItemOptionGroups?A().getItemOptionGroups(item):[]).forEach(function(g){
    var choices=g.choices||[], selected=choices[0];
    if(g.required&&g.type!=='multi'&&choices.length){
      if(String(g.name||'').trim().toLowerCase()==='sweetness') selected=choices.find(function(c){ return String(c.label||'').trim().toLowerCase()==='regular'; })||selected;
      _sel[g.id]=selected.label;
    }
  });
...
function drawRecipeEditor(item){
  ...
  var baseRows=d.base.map(function(r,ix){
    ...
    return '<tr><td>'+ingSelect(r.ing,'data-brow="'+ix+'" data-bfield="ing"')+'</td>'
      ...
      +'<td class="r" style="white-space:nowrap;font-weight:600;">'+peso(amt)+' <button class="pz-btn warn" style="padding:0.2rem 0.45rem;font-weight:400;" data-brem="'+ix+'">✕</button></td></tr>';
  }).join('');
  ...
  var caGroups=caGroupsAll.filter(function(g){var own=d.choiceAdd&&d.choiceAdd[g.id];return own&&Object.keys(own).some(function(k){return own[k]&&own[k].ings&&own[k].ings.length;});});
  ...
  var caCards=caGroups.map(function(g){
    var choices=(g.choices||[]).filter(function(c){var e=d.choiceAdd&&d.choiceAdd[g.id]&&d.choiceAdd[g.id][optKey(c.label)];return e&&e.ings&&e.ings.length;}).map(function(c){
  ...
  var caSection=caCards?('<div style="border-top:2px solid var(--cd);margin-top:0.8rem;padding-top:0.6rem;"><div style="font-weight:700;color:var(--bd);margin-bottom:0.2rem;">Active choice overrides</div><p class="pz-sub" style="margin-top:0;">Only choices explicitly overridden for this drink appear here. All other choices remain inherited from Shared Ingredients.</p>'+caCards+'</div>'):'';
  ...
  function selectedDetail(line){return esc(line.ingredientName)+' · '+num(line.quantityPerServing)+' '+esc(line.stockUnit)+' × '+peso(line.unitCost);}
  ...
  function effectiveRow(r){var section=effectiveSection(r),src=section==='additional'?(r.label||'Selected add-on'):String(r.source).indexOf('base_shared')===0?'Shared base':String(r.source).indexOf('base_')===0?'Recipe-specific override':r.source==='packaging'?'Serve-style packaging':(r.source==='option_global'?'Shared required choice':'Recipe-specific required choice')+(r.label?' · '+r.label:'');var requiredSelections=selectedChoices.filter(function(c){return (previewGroups[c.groupId]||{}).required;});var action=r.source==='base_shared'?'<label style="display:inline-flex;align-items:center;gap:.25rem;margin:.1rem;white-space:nowrap"><input type="checkbox" data-effective-include="'+esc(r.ingredientId)+'" checked> Include</label>'+requiredSelections.map(function(c){return '<button class="pz-btn sec" data-effective-replace="'+esc(r.ingredientId)+'" data-replace-group="'+esc(c.groupId)+'" data-replace-label="'+esc(c.label)+'" style="padding:.18rem .45rem;margin:.1rem">Override for '+esc(c.label)+'</button>';}).join(''):(section!=='additional'&&r.source==='option_global'?'<button class="pz-btn sec" data-effective-override="'+esc(r.ingredientId)+'" data-override-source="'+esc(r.source)+'" data-override-group="'+esc(r.groupId)+'" data-override-label="'+esc(r.label)+'" style="padding:.18rem .55rem">Override</button>':(String(r.source).indexOf('base_override')===0||r.source==='option_recipe'?'<span style="color:var(--tl)">Overridden</span>':''));return '<tr><td><b>'+esc(r.name)+'</b><small style="display:block;color:var(--tl)">'+esc(src)+'</small></td><td class="r">'+num(r.qS)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cS)+'</small></td><td class="r">'+num(r.qM)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cM)+'</small></td><td class="r">'+num(r.qL)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cL)+'</small></td><td class="r">'+peso(r.unitCost)+'</td><td class="r">'+action+'</td></tr>';}
  function sectionTable(kind,title,note,empty){var list=Object.keys(effective).map(function(k){return effective[k];}).filter(function(r){return effectiveSection(r)===kind;}),tot={S:0,M:0,L:0};list.forEach(function(r){['S','M','L'].forEach(function(sz){tot[sz]+=Number(r['c'+sz])||0;});});var rows=list.map(effectiveRow).join('');return '<section style="margin-top:.75rem"><div style="display:flex;justify-content:space-between;gap:.5rem;align-items:end"><div><b style="color:var(--bd)">'+title+'</b>'+(note?'<small style="display:block;color:var(--tl)">'+note+'</small>':'')+'</div><b>'+peso(tot[recSize||'M'])+'</b></div><div style="overflow-x:auto"><table class="pz-tbl" style="font-size:.76rem"><thead><tr><th>Ingredient and source</th><th class="r">S qty / value</th><th class="r">M qty / value</th><th class="r">L qty / value</th><th class="r">Unit cost</th><th class="r">Action</th></tr></thead><tbody>'+(rows||'<tr><td colspan="6" style="color:var(--tl)">'+empty+'</td></tr>')+'</tbody><tfoot><tr><th>'+title+' total</th><th class="r">'+peso(tot.S)+'</th><th class="r">'+peso(tot.M)+'</th><th class="r">'+peso(tot.L)+'</th><th></th><th></th></tr></tfoot></table></div></section>';}
  var effectiveTable=sectionTable('base','Effective base ingredients','Shared base plus required-choice replacements.','Select shared ingredients or add a recipe-specific ingredient.')+sectionTable('packaging','Packaging','Selected automatically from the serving style.','No packaging is assigned for this selection.')+sectionTable('additional','Additional ingredients','Added only when an optional choice is selected.','No additional ingredients selected.');
  ...
  ed.innerHTML=drinkCard
    +'<div class="pz-card" style="margin-bottom:1rem;">'
      ...
      +'<details style="margin-top:.7rem;"'+(baseRows?' open':'')+'><summary style="cursor:pointer;font-weight:700;color:var(--bd)">Override or add a recipe-specific ingredient</summary><p class="pz-sub">Use this only when this drink differs from the shared base. The recipe-specific row replaces the same shared ingredient; it never stacks.</p>'
      ...
      +'<button class="pz-btn sec" id="recAddBase" style="padding:0.3rem 0.7rem;">+ recipe-specific ingredient</button></details>'
  ...
  ed.querySelectorAll('select[data-caf]').forEach(function(s){s.onchange=function(){syncAll();drawRecipeEditor(item);};});
  ...
  ed.querySelectorAll('[data-effective-override]').forEach(function(b){b.onclick=function(){syncAll();var ing=b.getAttribute('data-effective-override'),source=b.getAttribute('data-override-source'),gid=b.getAttribute('data-override-group'),label=b.getAttribute('data-override-label');if(source==='base_shared'){var row=sharedLibrary.filter(function(x){return x.ing===ing;})[0],inv=inventoryMap[ing]||{},u=(row&&row.unit)||inv.unit||'';if(row&&!d.base.some(function(x){return x.ing===ing;}))d.base.push({ing:ing,unit:u,dS:row.dispS!=null?row.dispS:row.qtyS,dM:row.dispM!=null?row.dispM:row.qtyM,dL:row.dispL!=null?row.dispL:row.qtyL});}else if(source==='option_global'){var lk=optKey(label),entry=optCostStore()[gid]&&optCostStore()[gid][lk];if(entry){d.choiceAdd=d.choiceAdd||{};d.choiceAdd[gid]=d.choiceAdd[gid]||{};d.choiceAdd[gid][lk]={label:label,ings:(entry.ings||[]).map(function(row){var inv=inventoryMap[row.ing]||{},u=row.unit||inv.unit||'';return {ing:row.ing,unit:u,dS:row.dispS!=null?row.dispS:row.qtyS,dM:row.dispM!=null?row.dispM:row.qtyM,dL:row.dispL!=null?row.dispL:row.qtyL,op:row.op};})};}}drawRecipeEditor(item);setTimeout(function(){var details=document.querySelector('#recEditor details');if(details)details.open=true;},0);};});
  ed.querySelectorAll('[data-effective-include]').forEach(function(cb){cb.onchange=function(){if(cb.checked)return;syncAll();var ing=cb.getAttribute('data-effective-include');d.sharedBase=(d.sharedBase||[]).filter(function(x){return (typeof x==='string'?x:x.ing)!==ing;});drawRecipeEditor(item);};});
  ed.querySelectorAll('[data-effective-replace]').forEach(function(b){b.onclick=function(){syncAll();var ing=b.getAttribute('data-effective-replace'),gid=b.getAttribute('data-replace-group'),label=b.getAttribute('data-replace-label'),lk=optKey(label),shared=optCostStore()[gid]&&optCostStore()[gid][lk],base=sharedLibrary.filter(function(x){return x.ing===ing;})[0],inv=inventoryMap[ing]||{},u=(base&&base.unit)||inv.unit||'';d.choiceAdd=d.choiceAdd||{};d.choiceAdd[gid]=d.choiceAdd[gid]||{};if(!d.choiceAdd[gid][lk])d.choiceAdd[gid][lk]={label:label,ings:(shared&&shared.ings||[]).map(function(x){return {ing:x.ing,unit:x.unit||(inventoryMap[x.ing]||{}).unit||'',dS:x.dispS!=null?x.dispS:x.qtyS,dM:x.dispM!=null?x.dispM:x.qtyM,dL:x.dispL!=null?x.dispL:x.qtyL,op:x.op};})};var rows=d.choiceAdd[gid][lk].ings,existing=rows.filter(function(x){return x.ing===ing;})[0],values={ing:ing,unit:u,dS:base&&base.dispS!=null?base.dispS:base&&base.qtyS,dM:base&&base.dispM!=null?base.dispM:base&&base.qtyM,dL:base&&base.dispL!=null?base.dispL:base&&base.qtyL,op:'replace'};if(existing)Object.assign(existing,values);else rows.push(values);drawRecipeEditor(item);};});
...
function ocSync(){
  ...
  root.querySelectorAll('[data-oc-row]').forEach(function(tr){
    ...
    next[g]=next[g]||{}; next[g][lk]=next[g][lk]||{label:lbl,ings:[]};
    var row={ing:ing,unit:u,op:tr.getAttribute('data-oc-op')||undefined};['S','M','L'].forEach(function(sz){var display=v('disp'+sz);row['disp'+sz]=display;row['qty'+sz]=convertToStock(display,u,item);});next[g][lk].ings.push(row);
  });
