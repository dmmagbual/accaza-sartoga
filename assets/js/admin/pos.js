...
function renderPurchases(){
  ...
  hb('purSupplier','supplier');hb('purRef','ref');hb('purDate','date');hb('purBy','by');hb('purDescription','description');
  var ps=document.getElementById('purSupplier');if(ps)ps.onchange=function(){P.supplier=ps.value;renderPurchases();};
  ...
  root.querySelectorAll('[data-purchase-edit]').forEach(function(b){b.onclick=function(){var id=b.getAttribute('data-purchase-edit'),p=Object.assign({id:id},purchaseInvoicesMap[id]||{});if(p.reversed){alert('This purchase is already reversed.');return;}F().run({title:'Edit purchase details',subtitle:'Supplier, reference, due date, received-by and description update directly with an audit record. Changing the purchase date affects the accounting period, so Accaza will reverse the original and prepare a corrected replacement.',submitLabel:'Save changes',busyLabel:'Saving…',fields:[{name:'supplier',label:'Supplier',required:true,value:p.supplier||'',maxLength:120},{name:'ref',label:'Invoice / reference',required:true,value:p.ref||'',maxLength:120},{name:'date',label:'Purchase date',type:'date',required:true,value:p.date||''},{name:'due',label:'Due date',type:'date',value:p.due||''},{name:'by',label:'Received by',value:p.by||'',maxLength:120},{name:'description',label:'Description / notes',type:'textarea',value:p.description||'',maxLength:240},{name:'reason',label:'Reason for change',type:'textarea',required:true,maxLength:300},{name:'periodWarning',label:'I understand changing the purchase date changes the accounting period and requires reversal and reposting',type:'checkbox',required:true}]},function(v){if(v.date===p.date)return A().managePurchaseCorrection({action:'correct_details',invoiceId:id,supplier:v.supplier,ref:v.ref,due:v.due,by:v.by,description:v.description,reason:v.reason}).then(function(){return{metadataOnly:true};});return A().managePurchaseCorrection({action:'reverse',invoiceId:id,reason:v.reason,ownerAmend:true}).then(function(){var draft=correctedPurchaseDraft(p);draft.supplier=v.supplier;draft.ref=v.ref;draft.date=v.date;draft.due=v.due;draft.by=v.by;draft.description=v.description;window.__purch=draft;return{metadataOnly:false};});}).then(function(result){if(result.metadataOnly){alert('Purchase details updated. Amounts, stock, inventory valuation and Finance Books postings were unchanged.');renderPurchases();}else{renderPurchases();alert('The original purchase was reversed. The corrected purchase with the new date is prepared above—review it and press Receive all to repost it in the correct accounting period.');}}).catch(function(e){var s=String((e&&e.message)||(e&&e.code)||e);if(s.indexOf('cancelled')<0)alert('Could not edit purchase: '+s);});};});
...
function postPurchases(){
  ...
  if(P.pay==='advance'&&!openPurchaseAdvances(P.supplier).some(function(x){return x.id===P.advanceId;})){alert('The selected advance is unavailable or belongs to another supplier. Refresh and select the correct supplier advance.');return;}
