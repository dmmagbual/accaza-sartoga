...
function drawRecipeEditor(item){
  ...
  function effectiveRow(r){var section=effectiveSection(r),isInherited=inheritedRow(r),src=section==='additional'?(r.label||'Selected add-on'):r.source==='packaging'?'Serve-style packaging':isInherited?(r.source==='base_shared'?'Shared base':'Shared base · overridden for '+r.label):String(r.source).indexOf('base_')===0?'Recipe-specific ingredient':(r.source==='option_global'?'Shared required choice':'Recipe-specific required choice')+(r.label?' · '+r.label:'');var include=isInherited?'<input type="checkbox" data-effective-include="'+esc(r.ingredientId)+'" checked aria-label="Include '+esc(r.name)+'">':'';var overrides=requiredSelections.map(function(c){if(!isInherited)return '<td></td>';var on=replacementFor(r.ingredientId,c);return '<td style="text-align:center"><input type="checkbox" data-effective-replace="'+esc(r.ingredientId)+'" data-replace-group="'+esc(c.groupId)+'" data-replace-label="'+esc(c.label)+'"'+(on?' checked':'')+' aria-label="Override '+esc(r.name)+' for '+esc(c.label)+'"></td>';}).join('');return '<tr><td><b>'+esc(r.name)+'</b><small style="display:block;color:var(--tl)">'+esc(src)+'</small></td><td class="r">'+num(r.qS)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cS)+'</small></td><td class="r">'+num(r.qM)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cM)+'</small></td><td class="r">'+num(r.qL)+' '+esc(r.unit)+'<small style="display:block">'+peso(r.cL)+'</small></td><td class="r">'+peso(r.unitCost)+'</td><td style="text-align:center">'+include+'</td>'+overrides+'</tr>';}
