...
function renderPurchases(){
  ...
  root.querySelectorAll('[data-purchase-duplicate]').forEach(function(b){b.onclick=function(){var id=b.getAttribute('data-purchase-duplicate'),p=purchaseInvoicesMap[id]||{};F().run({title:'Reverse duplicate purchase',subtitle:'Purchase '+(p.ref||id)+' · '+peso(p.total)+'. This reverses its inventory and linked financial entry while preserving the record.',submitLabel:'Request approval & reverse',busyLabel:'Reversing…',fields:[{name:'reason',label:'Reversal reason',type:'textarea',required:true,maxLength:300,value:'Duplicate purchase entry'},{name:'confirmed',label:'I reviewed Details and confirmed this is the duplicate record',type:'checkbox',required:true}]},function(v){return A().managerApproval('reverse_purchase',id,p.total,v.reason).then(function(ap){return A().managePurchaseCorrection({action:'reverse',invoiceId:id,reason:v.reason,approvalId:ap.approvalId});});}).then(function(){alert('Duplicate purchase reversed. Its inventory and linked financial entry were reversed; the audit record remains visible.');}).catch(function(e){if(String((e&&e.code)||e).indexOf('cancelled')<0)alert('Could not reverse duplicate: '+((e&&e.message)||e));});};});
}
