...
import{createHistoricalPeriodStore}from'./historical-period-store.mjs?v=589';
...
function createSubscriptionHub(database,ops){
  ...
  var scopes={
    ...
    suppliers:['purchases','petty'],inventorySku:['inventory','purchases'],booksChart:['discrepancy','purchases','petty'],posDeviceHealth:['liveoperations'],shiftCloseReceipts:['liveoperations'],ownerDailySummaries:['liveoperations'],
  ...
  function resetEntry(entry){clearLinger(entry);if(entry.unsub)entry.unsub();entry.unsub=null;entry.generation=(entry.generation||0)+1;entry.live={};entry.older={};entry.archiveCursor=null;entry.last=null;entry.loading=true;entry.error=null;entry.hasOlder=true;entry.historyCached=false;entry.historyStale=false;entry.historyUpdatedAt=0;}
  ...
  function attach(entry){
    ...
    function receive(snapshot,hasOlder,historyMeta){
      ...
      if(HISTORY_BOUNDS[entry.path]){entry.live=snapshot.val()||{};entry.hasOlder=typeof hasOlder==='boolean'?hasOlder:!(p&&(salesPath(entry.path)||entry.path==='financialMovements'))&&Object.keys(entry.live).length>=HISTORY_BOUNDS[entry.path].limit;if(historyMeta){entry.historyCached=historyMeta.cached===true;entry.historyStale=historyMeta.stale===true;entry.historyUpdatedAt=Number(historyMeta.updatedAt)||0;}dispatch(entry,facade(entry));}
    ...
    if(entry.path==='archivedOrders'&&ops.readHistoricalOrders){
      if(p){entry.unsub=historicalPeriods.watch(p,(rows,meta)=>receive({val:()=>rows},!!(meta&&meta.hasMore),meta),failed);return;}
      ...
      async function refreshArchive(marker){
        ...
        try{
          ...
          if(ids&&ids.length){var patch=await ops.readHistoricalOrders({mode:'ids',purpose:'admin_archive_patch',ids:ids});if(stopped||generation!==(entry.generation||0))return;var rows=patch.orders||{};ids.forEach(function(id){if(rows[id]){if(entry.older[id]&&!entry.live[id])entry.older[id]=rows[id];else entry.live[id]=rows[id];}else{delete entry.live[id];delete entry.older[id];}});}
    ...
    if(p&&entry.path==='financialMovements'&&activeScope==='saleshistory'){
      // Archived orders carry a compact, source-linked Finance summary from their
      // Firestore replica. Only movements whose own posting date is inside the selected
      // period stay live here. During the controlled replica backfill, only legacy rows
      // still missing that summary use the old exact-source fallback.
      var base=null,sources={},stopped=false;
      function publish(){if(stopped||!base)return;var a=entries.archivedOrders;if(!a||a.loading||a.error)return;var missing={};[a.live,a.older].forEach(function(map){Object.keys(map||{}).forEach(function(k){var row=map[k]||{};if(!row._historicalLedger)missing[String(row.id||k)]=true;});});Object.keys(sources).forEach(function(id){if(!missing[id])delete sources[id];});var ids=Object.keys(missing).filter(function(id){return !(id in sources);});ids.forEach(function(id){sources[id]={rows:null};get(query(ref(database,'financialMovements'),orderByChild('sourceId'),startAt(id),endAt(id))).then(function(s){if(!stopped&&sources[id]){sources[id].rows=s.val()||{};publish();}},failed);});if(Object.values(sources).some(function(v){return v.rows===null;})){entry.loading=true;return;}receive({val:function(){return Object.assign.apply(Object,[{}].concat(Object.values(sources).map(function(v){return v.rows;})).concat([base]));}});}
      entry.refreshSources=publish;var stopBase=onValue(liveTarget(entry.path),function(snapshot){base=snapshot.val()||{};publish();},failed);
