import{watchSalesPeriod,periodKey}from'./sales-period-data.mjs?v=425';
...
function createSubscriptionHub(database,ops){
  ...
  function attach(entry){
    entry.loading=true;entry.error=null;var generation=entry.generation||0,p=selectedPeriod();
    entry.periodKey=p?periodKey(p):'';
    function failed(error){if(generation!==(entry.generation||0))return;entry.error=error;entry.loading=false;reportError(entry.path,error);}
