  ...
  journal(){
    ...
    const rows = display.map(g=>{
      ...
      return `<tr>
        ...
        <td class="journal-actions">${e.revision?`<button class="btn sm ghost" onclick="App.cashJournalHistory(\'${e.id}\')">History · r${e.revision}</button>`:""}${!closed&&suspenseAdvanceEligible?`<button class="btn sm primary" onclick="App.convertSuspenseAdvance('${e.id}')">Convert to supplier advance</button> `:''}${!closed&&customerPayableId?`<button class="btn sm primary" onclick="App.correctPayable('${customerPayableId}')">Close linked payable</button>`:!closed&&customerPayableControl?`<button class="btn sm ghost" onclick="App.editEntry('${e.id}')">Open customer payable</button>`:!closed&&systemControlled?`<button class="btn sm ghost" onclick="App.explainSystemEntry('${e.id}')">SYSTEM CONTROLLED · how to correct</button>`:!closed&&!posLocked&&!e.reversalOf&&!e.reversedByMovementId?`<button class="btn sm ghost" onclick="App.editEntry('${e.id}')">Edit / correct</button> ${(!e.revision)&&manualEntry?`<button class="btn sm ghost" onclick="App.reverseEntry('${e.id}',false)">Reverse</button> <button class="btn sm ghost" onclick="App.reverseEntry('${e.id}',true)">Void</button>`:''}`:'<span class="tiny muted">'+(status||(posLocked?'POS locked':'Automatic posting'))+'</span>'}</td></tr>`;
    }).join("");
    return `<div class="page-head"><div><h2>Journal</h2><p>${periodLabel()} · ${display.length} displayed entr${display.length===1?'y':'ies'} · correction mechanics grouped into posting history</p></div>
      ...
      <div class="hint"><b>SYSTEM CONTROLLED</b> journals must be corrected through the named Admin or Finance workflow; use “how to correct” for the exact route. Manual journals can be corrected while their accounting month is open.</div>
