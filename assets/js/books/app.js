...
App.createSupplierInline=function(selectId){if(!window.__manageSupplier)return alert('Supplier service is not ready. Refresh Finance Books.');var name=prompt('Official supplier name');if(!name)return;window.__manageSupplier({action:'create',name:name}).then(function(x){var select=document.getElementById(selectId);if(select){select.innerHTML=booksSupplierOptions(x.supplierId);select.value=x.supplierId;}}).catch(function(e){alert('Could not create supplier: '+(e&&e.message||e));});};
