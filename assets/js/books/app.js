...
App.showSupplierPayableLedger=function(encodedKey){
  ...
  m.innerHTML='<div class="modal-head"><h3>'+esc(supplier)+' · subsidiary payable ledger</h3><button class="x" onclick="App.closeModal()">×</button></div><div class="modal-body"><div class="hint">Every supplier invoice is listed here. Select an invoice to trace its purchase, original journal, payment, correction, and reversal entries.</div><div class="kpis"><div class="kpi bad"><div class="lbl">Outstanding</div><div class="val">'+peso(open)+'</div></div><div class="kpi"><div class="lbl">Invoices recorded</div><div class="val">'+docs.length+'</div><div class="sub">Original value '+peso(original)+'</div></div></div><div class="tbl-wrap"><table><thead><tr><th>Invoice / source</th><th>Date</th><th>Due</th><th>Status</th><th class="num">Original</th><th class="num">Outstanding</th></tr></thead><tbody>'+rows+'<tr class="total-row"><td colspan="5">Supplier outstanding balance</td><td class="num">'+peso(open)+'</td></tr></tbody></table></div></div><div class="modal-foot"><button class="btn ghost" onclick="App.closeModal()">Close</button></div>';document.getElementById('modalBg').classList.add('show');
};
