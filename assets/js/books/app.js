...
App.showSupplierPayableLedger=function(encodedKey){
  ...
  var rows=docs.map(function(d){var remaining=(d.status||'open')==='open'?r2(Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0):0;return '<tr><td><button class="btn sm ghost" onclick="App.showPayableTrace(\''+esc(d.id)+'\')">'+esc(d.ref||d.id)+'</button>'+(d.openingBalance===true?'<div class="tiny muted">Opening balance</div>':'')+'</td><td class="tiny">'+esc(d.date||'')+'</td><td class="tiny">'+esc(d.due||'—')+'</td><td>'+esc(payableStatusLabel(d))+'</td><td class="num">'+peso(d.amount||0)+'</td><td class="num">'+peso(remaining)+'</td></tr>';}).join(''),m=document.getElementById('modal');
  m.innerHTML='<div class="modal-head"><h3>'+esc(supplier)+' · supplier subsidiary ledgers</h3><button class="x" onclick="App.closeModal()">×</button></div><div class="modal-body"><div class="hint">Payables and Supplier Advances are separate control accounts. Select an invoice to trace its purchase, original journal, payment, correction, and reversal entries.</div><div class="section-label">Accounts Payable · 2000</div><div class="kpis"><div class="kpi bad"><div class="lbl">Outstanding</div><div class="val">'+peso(open)+'</div></div><div class="kpi"><div class="lbl">Invoices recorded</div><div class="val">'+docs.length+'</div><div class="sub">Original value '+peso(original)+'</div></div></div><div class="tbl-wrap"><table><thead><tr><th>Invoice / source</th><th>Date</th><th>Due</th><th>Status</th><th class="num">Original</th><th class="num">Outstanding</th></tr></thead><tbody>'+rows+'<tr class="total-row"><td colspan="5">Supplier outstanding balance</td><td class="num">'+peso(open)+'</td></tr></tbody></table></div><div id="supplierAdvanceDetail" style="margin-top:1rem"><div class="section-label">Supplier Advances · 1115</div><div class="tiny muted">Loading original payments and allocations…</div></div></div><div class="modal-foot"><button class="btn ghost" onclick="App.closeModal()">Close</button></div>';document.getElementById('modalBg').classList.add('show');App.loadSupplierAdvances(docs[0]);
};
...
function subledgerPage(kind){
  ...
  var docs = openDocs(map).sort(function(a,b){return String(a.due||'9999-99-99').localeCompare(String(b.due||'9999-99-99'));}),openingButton=isAr?'':openingPayableHeaderButton()+'<button class="btn ghost" onclick="App.showSupplierAdvanceLedger()">Supplier Advances · 1115</button>';
...
function openDocOptions(map,selectedId,skipProvisional){ var d=openDocs(map); if(skipProvisional)d=d.filter(function(x){return x.provisional!==true;}); if(!d.length) return ''; return d.map(function(x){return '<option value="'+esc(x.id)+'"'+(selectedId&&x.id===selectedId?' selected':'')+'>'+esc(x.party||'—')+' · '+peso(x.amount)+(x.due?' · due '+esc(x.due):'')+(x.ref?' · '+esc(x.ref):'')+'</option>';}).join(''); }
...
App.reversePayablePayment=function(id){
  var d=(window.__apMap||{})[id];
  if(!d)return alert('That bill is no longer available. Refresh Finance Books.');
  if(String(d.status||'')!=='paid')return alert('Only a bill recorded as paid can have its payment reversed.');
  if(d.paymentReversalMovementId) return alert('This payment has already been reversed.');
  if(!d.settlementMovementId) return alert('This bill has no settlement record, so it cannot be reversed cleanly. Raise it for review.');
  App._txnModal('Reverse payment \u00b7 '+esc(d.party||id),'<div class="hint">Use only when the payment was recorded but never made. Finance debits the account it was paid from and credits Accounts Payable, and the bill returns to outstanding. The original posting stays in the audit trail.</div><div class="grid2"><div class="field"><label>Amount</label><input value="'+r2(Number(d.paidAmount!=null?d.paidAmount:d.amount)||0).toFixed(2)+'" disabled/></div><div class="field"><label>Correction date</label><input id="rp_date" type="date" value="'+todayStr()+'"/></div></div><div class="field"><label>Correction reference</label><input id="rp_ref" value="'+esc(d.settlementReference||'')+'" placeholder="Required"/></div><div class="field"><label>Reason for reversing</label><textarea id="rp_reason" maxlength="300" placeholder="Required"></textarea></div>','Reverse payment',function(btn){
