...
const PAGES = {
  ...
  cashflow(){
    ...
    return `<div class="page-head"><div><h2>Cash Flow</h2><p>Authoritative cash statement · moved from Admin</p></div><div class="btn-row">${csvButton('cashflow')}<button class="btn" onclick="App.go('transactions')">Record transaction</button><button class="btn primary" onclick="App.cashAccountEdit('')">+ Cash account</button></div></div>
  ...
  coa(){
    ...
    const accountRow = a=>{const workflow=systemAccountWorkflow(a.code);return `<tr><td><span class="acc-code">${a.code}</span></td><td><span class="${accountGroupFor(a)?'account-child':''}"><b>${esc(a.name)}</b></span>${workflow?' <span class="type-pill t-expense">SYSTEM CONTROLLED</span>':''}</td><td class="tiny muted">${esc(a.note)}${workflow?'<br><b>'+esc(workflow)+'</b>':''}</td><td class="num">${peso(normalBalanceFor(a.code,entries))}</td>
        <td style="white-space:nowrap">${workflow?'<span class="tiny muted">Source-managed</span>':`<button class="btn sm ghost" onclick="App.editAccount('${a.code}')">Edit</button>`}</td></tr>`;};
    const body = groups.map(g=>{
    ...
    return `<div class="page-head"><div><h2>Chart of Accounts</h2><p>${DB.accounts.length} posting accounts · ${ACCOUNT_GROUPS.length} protected main accounts · balances as of latest entry</p></div>
      ...
      <div class="hint"><b>Main accounts are read-only totals.</b> <b>SYSTEM CONTROLLED</b> posting accounts are updated through the named Admin or Finance workflow so the detailed record and General Ledger always agree. Unmarked accounts may be used for ordinary manual journals.</div>
