...
const App = {
  ...
  resetCurrentMonth(){try{window.AccazaReportPeriod.setMonth(todayStr().slice(0,7));if(window.AccazaReportPagination)window.AccazaReportPagination.reset('books-'+CURRENT);this.render();}catch(e){alert(e.message||e);}},
  ...
  reportFilter(){var selected=TABS.find(t=>t.id===CURRENT);if(!selected||selected.group==='controls')return'';var p=window.AccazaReportPeriod.get();if(CURRENT==='bs')return '<div class="report-filter as-of"><label>As of<input id="periodAsOf" type="date" value="'+esc(p.to)+'" max="'+todayStr()+'"/></label><button type="button" onclick="App.applyAsOf()">Apply</button><button type="button" onclick="App.resetCurrentMonth()">Today</button></div>';return '<div class="report-filter"><label>From<input id="periodFrom" type="date" value="'+esc(p.from)+'" max="'+todayStr()+'"/></label><label>To<input id="periodTo" type="date" value="'+esc(p.to)+'" max="'+todayStr()+'"/></label><label>Month<input id="periodMonth" type="month" value="'+esc(p.mode==='custom'?'':(p.endMonth||p.to.slice(0,7)))+'" max="'+todayStr().slice(0,7)+'" onchange="App.applyReportMonth()"/></label><button type="button" onclick="App.applyDateRange()">Apply</button><button type="button" onclick="App.resetCurrentMonth()">Current month</button></div>';},
  ...
  openEntryModal(pf,edit){
    ...
    const optHtml = sel => '<option value="">— account —</option>' +
      TYPES.map(t=>{ const rows=DB.accounts.filter(a=>a.type===t&&!isMainAccount(a.code)&&a.active!==false); if(!rows.length) return ""; return `<optgroup label="${t}">`+rows.map(a=>`<option value="${a.code}" ${a.code===sel?'selected':''}>${a.code} · ${esc(a.name)}${systemAccountWorkflow(a.code)?' · SYSTEM CONTROLLED':''}</option>`).join("")+`</optgroup>`; }).join("");
    ...
    m.innerHTML = `
      ...
      <div class="modal-body">
        <div class="hint"><b>System-controlled accounts are marked in the account list.</b> Their balances come from Admin or a dedicated Finance workflow. Correct the source record there so its supplier, customer, cash, inventory, or payable detail changes with the General Ledger.</div>
        ${edit?'<div class="hint">Eligible open-period cash/Undeposited transfers save in place with revision history and a synchronized pooled balance—no void/repost. Keep the same cash account and direction. A privileged Finance role and reason are required. Closed periods, bank reconciliation locks, or insufficient cash block the save. Standalone income/expense/capital reclassifications also save in place when the date and every operational balance stay unchanged. Inventory quantities and value are not adjusted.</div><div class="field"><label>Correction reason</label><input id="e_reason" value="'+esc(edit.reason||'')+'" placeholder="Required · explain what was wrong" oninput="App._edit.reason=this.value"/></div>':''}
        <div class="grid2">
  ...
  explainSystemEntry(id){const e=ENTRIES().find(x=>x.id===id)||{},advance=e.sourceType==='pettyVoucher'&&/purchase advance/i.test(e.memo||'')||(e.lines||[]).some(l=>l.code==='1115'),staffAdvance=e.sourceType==='pettyVoucher'&&/staff advance/i.test(e.memo||'')||(e.lines||[]).some(l=>l.code==='1120'||l.code==='6079');if(advance)return alert('SYSTEM CONTROLLED · Supplier Advances\n\nCorrect the original payment in Admin > Cash Payments. Allocate delivered items in Admin > Purchases. The system will update the supplier advance, inventory or expense, cash account, General Ledger, and audit trail together.\n\nDo not edit this journal directly.');if(staffAdvance)return alert('SYSTEM CONTROLLED · Staff Advances\n\nCorrect the original payment or its liquidation in Admin > Cash Payments. The system will update the staff advance, cash, payroll expense or write-off account, and General Ledger together.\n\nDo not edit this journal directly.');const routes=[...new Set((e.lines||[]).map(l=>systemAccountWorkflow(l.code)).filter(Boolean))];alert('SYSTEM CONTROLLED JOURNAL\n\n'+(routes.length?routes.join('\n'):'Use the source workflow shown in the Chart of Accounts.')+'\n\nCorrect the originating Admin or Finance record so its detailed record and General Ledger remain linked.');},
  ...
  cashJournalHistory(id){
    ...
    window.__financeCmd({action:'cash_journal_history',commandId:'history_'+Date.now(),originalMovementId:id}).then(result=>{
      ...
      document.getElementById('modal').innerHTML='<div class="modal-head"><h3>Journal revision history</h3><button class="x" onclick="App.closeModal()">×</button></div><div class="modal-body">'+revisions.map(r=>'<div class="card card-pad"><b>Revision '+r.revision+'</b><p>'+esc(r.reason)+' · '+esc(r.changedByRole)+' · '+esc(r.changedBy)+'</p><p class="tiny">'+esc(new Date(r.changedAt).toLocaleString('en-PH',{timeZone:'Asia/Manila'}))+' · Philippine time</p><div class="tiny">Before · '+detail(r.before)+'<hr>After · '+detail(r.after)+(r.kind==='classification_only'?'<p>Classification only — operational balances unchanged.</p>':'<p>Undeposited pool: '+peso(r.poolBefore)+' → '+peso(r.poolAfter)+'</p>')+'</div></div>').join('')+'</div>';
      document.getElementById('modalBg').classList.add('show');
...
// Opening cash comes from server day/month cash indexes plus the cash-account opening
// movements; only the selected period's movements are downloaded (AccazaCashFlowBasis).
function cfBasisInput(from,to){return {from:from,to:to,accounts:window.__cfAccounts||{},movements:window.__cashFlowPeriodReady?(window.__financialMovements||{}):null,monthly:window.__cashFlowMonthlyReady?(window.__cashFlowMonthly||{}):null,daily:window.__cashFlowDailyMonth?(window.__cashFlowDaily||{}):null,dailyMonth:window.__cashFlowDailyMonth||'',openings:window.__cashFlowOpeningsReady?(window.__cashFlowOpenings||{}):null,groups:window.__cashFlowGroups||{},meta:window.__cashFlowIndexMeta||null,categoryOf:cfCategory,isCorrection:cfIsCorrection};}
function cfStatement(){
  ...
  const accs=cfAccounts(),input=cfBasisInput(CF_FROM,CF_TO),ready=!!(window.AccazaCashFlowBasis&&window.AccazaCashFlowBasis.ready(input));
  const basis=ready?window.AccazaCashFlowBasis.statement(input):{begin:{},ending:{},add:{},ded:{},detail:[],corrections:0,correctionDetail:[],openingSources:[]};
  const begin=basis.begin,ending=basis.ending,add=basis.add,ded=basis.ded,openingSources=new Set(basis.openingSources);
  ...
  const keys=accs.map(a=>a.id).concat(['register','float','undeposited','petty']),sum=o=>r2(Object.values(o).reduce((s,v)=>s+(Number(v)||0),0));return {loading:!ready,accs,keys,begin,ending,add,ded,detail:basis.detail,corrections:r2(basis.corrections),correctionDetail:basis.correctionDetail,totBegin:sum(begin),totEnd:sum(ending),totAdd:sum(add),totDed:sum(ded)};
}
...
function cfAccountLedgerBalance(id){return window.AccazaCashFlowBasis?r2(window.AccazaCashFlowBasis.ledgerBalance(id,window.__cashFlowMonthly||{},window.__cashFlowOpenings||{})):0;}
App.cashAccountEdit=function(id){const a=(window.__cfAccounts||{})[id]||{},isNew=!id,m=document.getElementById('modal'),current=isNew?0:cfAccountLedgerBalance(id);App._cashOpening={id:id||'',oldOpening:Number(a.opening)||0,current};m.innerHTML=`<div class="modal-head"><h3>${isNew?'Add cash account':'Correct cash-account opening'}</h3><button class="x" onclick="App.closeModal()">×</button></div><div class="modal-body"><div class="field"><label>Account name</label><input id="cfa_name" value="${esc(a.name||'')}"/></div><div class="grid2"><div class="field"><label>Type</label><select id="cfa_type"><option value="bank" ${a.type==='bank'?'selected':''}>Bank</option><option value="ewallet" ${a.type==='ewallet'?'selected':''}>E-wallet</option></select></div><div class="field"><label>Opening balance date</label><input id="cfa_date" type="date" value="${a.openingDate||todayStr()}"/></div></div><div class="field"><label>Correct opening balance</label><input id="cfa_open" type="number" step=".01" value="${Number(a.opening)||0}" oninput="App.cashOpeningPreview()"/></div><div id="cfa_preview" class="balance-bar"></div><div class="grid2"><div class="field"><label>Supporting reference</label><input id="cfa_ref" placeholder="Cash count, bank statement or approval ID"/></div><div class="field"><label>Reason for correction</label><input id="cfa_reason" placeholder="Required when the opening changes"/></div></div><div class="field"><label>Payment methods routed here (comma-separated)</label><input id="cfa_feed" value="${esc((a.feedMethods||[]).join(', '))}" placeholder="GCash, Bank Transfer"/></div><div class="hint">The difference posts once to this cash account against Owner's Capital. If its date is the report's first day, it appears as opening funds introduced during the period—not as cash held before that day. Later receipts and payments are never rewritten.</div></div><div class="modal-foot"><button class="btn ghost" onclick="App.closeModal()">Cancel</button><button class="btn primary" onclick="App.cashAccountSave('${id||''}',this)">Save correction</button></div>`;document.getElementById('modalBg').classList.add('show');App.cashOpeningPreview();};
// One form creates a bank / e-wallet: its Finance Books ledger account and the cash account every payment screen lists (Xero-style).
App.nextBankCode=function(kind){const taken=new Set(DB.accounts.map(a=>String(a.code)).concat(Object.values(window.__cashAccountMap||{}).map(String))),ranges=kind==='ewallet'?[[1022,1029]]:[[1010,1019],[1031,1039]];for(const [a,b] of ranges)for(let n=a;n<=b;n++)if(!taken.has(String(n)))return String(n);return '';};
App.bankAccountForm=function(code,typedCode){if(!window.__booksUser)return alert('Sign in to Finance Books first.');const existing=code?acc(code):null,m=document.getElementById('modal'),kind='bank',suggested=code||typedCode||App.nextBankCode(kind);m.innerHTML=`<div class="modal-head"><h3>${existing?'Make '+esc(existing.code)+' a bank account':'Add bank account'}</h3><button class="x" onclick="App.closeModal()">×</button></div><div class="modal-body"><div class="grid2"><div class="field"><label>Name</label><input id="ba_name" value="${esc(existing?existing.name:'')}" placeholder="e.g. Maribank"/></div><div class="field"><label>Type</label><select id="ba_kind" ${code||typedCode?'':'onchange="App.bankKindChanged()"'}><option value="bank">Bank</option><option value="ewallet">E-wallet</option></select></div></div><div class="grid2"><div class="field"><label>Finance Books code</label><input id="ba_code" value="${esc(suggested)}" ${code?'readonly':''} placeholder="1010-1039"/></div><div class="field"><label>Opening balance date</label><input id="ba_date" type="date" value="${todayStr()}"/></div></div><div class="field"><label>Opening balance (money already in the account)</label><input id="ba_open" type="number" step=".01" value="0"/></div><div class="grid2"><div class="field"><label>Bank statement reference</label><input id="ba_ref" placeholder="Required when opening is not zero"/></div><div class="field"><label>Reason</label><input id="ba_reason" placeholder="e.g. Existing balance when the account was added"/></div></div><div class="hint">This creates the ledger account <b>and</b> the bank account in one step. It appears right away in Purchases, Cash Payments, Payables, deposits and POS payment routing. The opening balance posts once to this account against Owner's Capital.${existing?' A code that already has journals must have them voided first; post them again after converting so the money lands in the bank account.':''}</div></div><div class="modal-foot"><button class="btn ghost" onclick="App.closeModal()">Cancel</button><button class="btn primary" onclick="App.bankAccountCreate(this,${code?'true':'false'})">${existing?'Make bank account':'Add bank account'}</button></div>`;document.getElementById('modalBg').classList.add('show');};
App.bankKindChanged=function(){const k=document.getElementById('ba_kind').value,c=document.getElementById('ba_code');if(c)c.value=App.nextBankCode(k);};
App.bankAccountCreate=function(btn,converting){if(!window.__cashAccountSave)return alert('Live Finance service is not ready.');const name=document.getElementById('ba_name').value.trim(),kind=document.getElementById('ba_kind').value,code=document.getElementById('ba_code').value.trim(),opening=Number(document.getElementById('ba_open').value)||0,reference=document.getElementById('ba_ref').value.trim(),reason=document.getElementById('ba_reason').value.trim();if(!name)return alert('Enter the bank or e-wallet name.');if(code&&!isBankLedgerCode(code))return alert('Use a Finance Books code from 1010 to 1039 (1030 is retired).');if(!converting&&code&&acc(code))return alert('Finance Books '+code+' already exists. Use "Make bank account" on that row in Chart of Accounts.');if(Math.abs(opening)>=.005&&(!reference||!reason))return alert('Enter the bank statement reference and reason for the opening balance.');btn.disabled=true;window.__cashAccountSave({action:'upsert',commandId:'bankacct_'+Date.now(),accountId:'acc_'+Date.now().toString(36),booksCode:code,name,type:kind,opening,openingDate:document.getElementById('ba_date').value||todayStr(),reference,reason,feedMethods:[]}).then(r=>{App.closeModal();alert(name+' is ready as Finance Books '+((r&&r.booksCode)||code)+'. It can now be selected in every payment screen.');}).catch(e=>{alert('Could not add bank account: '+((e&&e.message)||e));btn.disabled=false;});};
...
const PAGES = {
  ...
  cashflow(){
    ...
    // The control audit reads the whole order archive and ledger on the server, so it runs
    // only from the "Run control audit" button, never on opening this tab.
    ...
    return `<div class="page-head"><div><h2>Cash Flow</h2><p>Authoritative cash statement · moved from Admin</p></div><div class="btn-row">${csvButton('cashflow')}<button class="btn" onclick="App.go('transactions')">Record transaction</button><button class="btn primary" onclick="App.bankAccountForm('')">+ Add bank account</button></div></div>
      ${s.loading?'<div class="hint">Loading cash balances for this period…</div>':''}<div class="hint">Opening cash is the amount held at the end of the day before this report starts. Funds first recorded on the From date appear as cash received during the period. Transfers between Register Cash, Undeposited Collection and a bank stay in the audit trail but are excluded here because they do not increase or reduce total cash.</div>
      ...
      <div class="card card-pad" style="margin-bottom:1rem"><div class="page-head" style="margin-bottom:.6rem"><div><div class="section-label">Cash flow statement</div></div><div class="tiny">${esc(periodLabel())}</div></div>
        <div class="tbl-wrap"><table><thead><tr><th>Cash movement</th><th class="num">Amount</th></tr></thead><tbody>
          ...
          <tr class="sub-row"><td>Cash paid outside the business</td><td class="num neg">(${peso(s.totDed)})</td></tr>${deductionRows}
          <tr class="sub-row"><td>Cash-balance corrections <span class="tiny muted">not receipts, payments or internal transfers</span></td><td class="num">${s.corrections<0?'−':''}${peso(Math.abs(s.corrections))}</td></tr>${correctionRows}
          <tr class="total-row"><td>Calculated closing cash</td><td class="num">${peso(expected)}</td></tr>
          <tr class="group-account"><td>Closing cash by location · ${CF_TO}</td><td class="num">${peso(s.totEnd)}</td></tr>${balRows(s.ending)}
        </tbody></table></div>
      ...
      <div class="card" style="margin-bottom:1rem"><div class="card-pad"><div class="section-label">Cash accounts · all open for viewing and editing</div></div><div class="tbl-wrap"><table><thead><tr><th>Account</th><th>Books account</th><th>Type</th><th>Opening date</th><th class="num">Opening</th><th></th></tr></thead><tbody>${s.accs.map(a=>`<tr><td>${esc(a.name)}</td><td>${esc((window.__cashAccountMap||{})[a.id]||'—')}</td><td>${esc(a.type)}</td><td>${a.openingDate||'—'}</td><td class="num">${peso(a.opening)}</td><td><button class="btn sm ghost" onclick="App.cashAccountEdit('${a.id}')">Edit</button></td></tr>`).join('')}</tbody></table></div></div>
  ...
  journal(){
    ...
    const rows = display.map(g=>{
      const e=g.primary,dr=r2((e.lines||[]).reduce((s,l)=>s+(Number(l.debit)||0),0)),cr=r2((e.lines||[]).reduce((s,l)=>s+(Number(l.credit)||0),0)),corrected=!!g.replacement,closed=!corrected&&!!g.reversal,status=corrected?'Corrected':closed?(g.reversal&&g.reversal.voided?'Voided':'Reversed'):'';
      ...
      const posLocked=e.sourceType==='order'||/^order_|^pos_/i.test(e.type||'')||/^POS-/i.test(e.ref||''),manualEntry=e.sourceType==='booksManualJournal'||e.sourceType==='manual'||e.sourceType==='transfer'||['manual_books_journal','manual_cash','cash_transfer'].includes(e.type),customerPayableId=linkedCustomerPayableId(e),customerPayableControl=(e.lines||[]).some(l=>l.code==='2030'),suspenseLines=(e.lines||[]).filter(l=>l.code==='1900'&&Number(l.debit)>0&&!Number(l.credit)),cashLines=(e.lines||[]).filter(l=>!Number(l.debit)&&Number(l.credit)>0&&(l.accountKey==='register_cash'||l.accountKey==='cash_awaiting_deposit'||l.accountKey==='petty_cash'||String(l.accountKey||'').startsWith('cash_account:'))),suspenseAdvanceEligible=manualEntry&&!e.supplierAdvanceConversionId&&(e.lines||[]).length===2&&suspenseLines.length===1&&cashLines.length===1&&Math.abs(Number(suspenseLines[0].debit)-Number(cashLines[0].credit))<.005,systemControlled=!manualEntry&&(e.source==='pos-bridge'||(e.lines||[]).some(l=>systemAccountWorkflow(l.code)));
      const actions=`${e.revision?`<button class="btn sm ghost" onclick="App.cashJournalHistory(\'${e.id}\')">History · r${e.revision}</button>`:""}${!closed&&suspenseAdvanceEligible?`<button class="btn sm primary" onclick="App.convertSuspenseAdvance('${e.id}')">Convert to supplier advance</button> `:''}${!closed&&customerPayableId?`<button class="btn sm primary" onclick="App.correctPayable('${customerPayableId}')">Close linked payable</button>`:!closed&&customerPayableControl?`<button class="btn sm ghost" onclick="App.editEntry('${e.id}')">Open customer payable</button>`:!closed&&systemControlled?`<button class="btn sm ghost" onclick="App.explainSystemEntry('${e.id}')">SYSTEM CONTROLLED · how to correct</button>`:!closed&&!posLocked&&!e.reversalOf&&!e.reversedByMovementId?`<button class="btn sm ghost" onclick="App.editEntry('${e.id}')">Edit / correct</button> ${(!e.revision)&&manualEntry?`<button class="btn sm ghost" onclick="App.reverseEntry('${e.id}',false)">Reverse</button> <button class="btn sm ghost" onclick="App.reverseEntry('${e.id}',true)">Void</button>`:''}`:'<span class="tiny muted">'+(status||(posLocked?'POS locked':'Automatic posting'))+'</span>'}`;
      return `<tr><td class="journal-date">${e.date}</td><td class="journal-reference">${esc(e.ref||e.reference||e.id)||'—'}<div class="tiny">${e.source==='pos'?'<span class="badge-rev" style="color:#28576b;background:#e9f2f6">POS</span>':''}${status?'<span class="badge-rev">'+status+'</span>':''}</div></td><td class="journal-entry"><b>${esc(e.memo)||'(no description)'}</b>${e.linkedDiscrepancyId?`<div class="tiny"><span class="badge-rev">Admin variance · ${esc(e.linkedDiscrepancyId)}</span></div>`:''}<div class="journal-lines">${entryLines(e)}</div>${history}<div class="journal-actions">${actions}</div></td><td class="num journal-amount">${closed?peso(0):peso(dr)}</td><td class="num journal-amount">${closed?peso(0):peso(cr)}</td><td class="num journal-balance">${peso(r2(dr-cr))}</td></tr>`;
    }).join("");
    return `<div class="page-head"><div><h2>Journal</h2><p>${periodLabel()} · ${display.length} displayed entr${display.length===1?'y':'ies'} · correction mechanics grouped into posting history</p></div>
      ...
      <div class="hint"><b>SYSTEM CONTROLLED</b> journals must be corrected through the named Admin or Finance workflow. The Balance column is the journal-entry difference and must remain zero; account running balances are available by opening an account in General Ledger.</div>
      <div class="card"><div class="tbl-wrap"><table class="journal-table">
        ...
        <tbody>${rows||'<tr><td colspan=6><div class="empty"><div class="big">📓</div>No entries in this period.<br><button class="btn primary" style="margin-top:.8rem" onclick="App.newEntry()">Post your first entry</button></div></td></tr>'}</tbody>
      </table></div></div>`;
  },
  ...
  tb(){
    ...
    return `<div class="page-head"><div><h2>Trial Balance</h2><p>${periodLabel()} activity · debits and credits posted inside the selected period</p></div>${csvButton('tb')}</div>
  ...
  bs(){
    ...
    return `<div class="page-head"><div><h2>Balance Sheet</h2><p>Cumulative balance as of ${periodBounds().end}</p></div>${csvButton('bs')}</div>
  ...
  coa(){
    ...
    const accountRow = a=>{const workflow=systemAccountWorkflow(a.code),banks=bankAccountsForCode(a.code),convertible=!banks.length&&!workflow&&a.type==='Asset'&&a.active!==false&&isBankLedgerCode(a.code)&&window.__booksUser;return `<tr><td><span class="acc-code">${a.code}</span></td><td><span class="${accountGroupFor(a)?'account-child':''}"><b>${esc(a.name)}</b></span>${workflow?' <span class="type-pill t-expense">SYSTEM CONTROLLED</span>':''}${banks.length?' <span class="type-pill t-asset">BANK</span>':''}${convertible?' <span class="type-pill t-expense">NOT A BANK ACCOUNT YET</span>':''}</td><td class="tiny muted">${esc(a.note)}${workflow?'<br><b>'+esc(workflow)+'</b>':''}${banks.length?'<br>Selectable in Purchases, Cash Payments and Payables as <b>'+banks.map(b=>esc(b.name)).join(', ')+'</b>':''}${convertible?'<br>Payment screens can\'t use this code until it is converted.':''}</td><td class="num">${peso(normalBalanceFor(a.code,entries))}</td>
        <td style="white-space:nowrap">${workflow?'<span class="tiny muted">Source-managed</span>':`${convertible?`<button class="btn sm" onclick="App.bankAccountForm('${a.code}')">Make bank account</button> `:''}<button class="btn sm ghost" onclick="App.editAccount('${a.code}')">Edit</button>`}</td></tr>`;};
    const body = groups.map(g=>{
    ...
    return `<div class="page-head"><div><h2>Chart of Accounts</h2><p>${DB.accounts.length} posting accounts · ${ACCOUNT_GROUPS.length} protected main accounts · balances as of latest entry</p></div>
      ${(!window.__booksUser||window.__booksChartManager)?`<div class="btn-row" style="gap:.5rem">${(window.__booksUser&&window.__booksChartManager)?'<button class="btn" onclick="App.importLocalChart()">⬆ Import local accounts</button>':''}<button class="btn" onclick="App.bankAccountForm('')">+ Add bank account</button><button class="btn primary" onclick="App.editAccount()">+ Add account</button></div>`:'<span class="tiny muted">Chart is managed by the finance owners</span>'}</div>
      <div class="hint"><b>Main accounts are read-only totals.</b> <b>SYSTEM CONTROLLED</b> posting accounts are updated through the named Admin or Finance workflow so the detailed record and General Ledger always agree. Unmarked accounts may be used for ordinary manual journals. <b>Bank and e-wallet accounts</b> (codes 1010-1039) are added with <b>+ Add bank account</b>: one step creates the ledger account and the bank that every payment screen can select.</div>
...
function openDocs(map){ return Object.keys(map||{}).map(function(k){return Object.assign({id:k}, map[k]||{});}).filter(function(d){return (d.status||'open')==='open' && (Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0)>0;}); }
...
function arApTotals(){
  var ar=r2(adminSalesAr().total+openDocs(window.__arMap).reduce(function(s,d){return s+(Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0);},0));
  var ap=openDocs(window.__apMap).reduce(function(s,d){return s+(Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0);},0);
...
function settledSection(map){
  ...
  return '<div class="card">'+head+'<div class="tbl-wrap"><table><thead><tr><th>Party</th><th>Bill date</th><th>Payment ref</th><th class="num">Amount</th><th></th></tr></thead><tbody>'+docs.map(function(d){
    ...
    return '<tr><td><button class="btn sm ghost" onclick="App.showSupplierPayableLedger(\''+encodeURIComponent(payableSupplierKey(d)).replace(/'/g,'%27')+'\')">'+esc(d.party||'\u2014')+'</button>'+(d.ref?'<div class="tiny muted">'+esc(d.ref)+'</div>':'')+'</td><td class="tiny">'+esc(d.date||'')+'</td><td class="tiny">'+esc(d.settlementReference||'\u2014')+'</td><td class="num">'+peso(d.amount)+'</td><td>'+(reversed?'<span class="tiny muted">Payment reversed</span>':d.settlementMovementId?'<button class="btn sm ghost" onclick="App.reversePayablePayment(\''+esc(d.id)+'\')">Reverse payment</button>':'<span class="tiny muted">No settlement record</span>')+'</td></tr>';}).join('')+'</tbody></table></div></div>';
}
function payableSupplierKey(d){return String(d&&d.supplierId||'').trim()||'name:'+String(d&&d.party||'').trim().toLowerCase();}
function payableSupplierDocs(key){return Object.keys(window.__apMap||{}).map(function(id){return Object.assign({id:id},window.__apMap[id]||{});}).filter(function(d){return d.type!=='customer_change_refund'&&payableSupplierKey(d)===key;}).sort(function(a,b){return String(a.date||'').localeCompare(String(b.date||''))||String(a.ref||a.id).localeCompare(String(b.ref||b.id));});}
function payableRelatedEntries(d){
  var invoice=d&&d.purchaseInvoiceId&&(window.__piMap||{})[d.purchaseInvoiceId],seeds=[d&&d.movementId,d&&d.settlementMovementId,d&&d.reversalMovementId,d&&d.paymentReversalMovementId,d&&d.reversedPaymentMovementId].concat(invoice&&invoice.movementIds||[]).filter(Boolean),wanted={};seeds.forEach(function(id){wanted[id]=true;});
  var all=ENTRIES(),changed=true;while(changed){changed=false;all.forEach(function(e){if(wanted[e.id])return;var linked=e.linkedPayableId===d.id||e.sourceId===d.id||(d.purchaseInvoiceId&&e.sourceId===d.purchaseInvoiceId)||wanted[e.reversalOf]||wanted[e.correctsMovementId]||wanted[e.correctionReversalMovementId];if(linked){wanted[e.id]=true;changed=true;}});}
  return all.filter(function(e){return wanted[e.id];}).sort(function(a,b){return String(a.date||'').localeCompare(String(b.date||''))||String(a.id||'').localeCompare(String(b.id||''));});
}
function payableStatusLabel(d){var remaining=r2(Number(d.remainingAmount!=null?d.remainingAmount:(d.status==='open'?d.amount:0))||0);return d.status==='reversed'?'Reversed':d.status==='paid'?'Paid':remaining>0&&remaining<r2(Number(d.amount)||0)?'Part-paid':d.provisional===true?'Pending invoice':'Unpaid';}
function payableEntryTable(entries){return entries.map(function(e){var lines=(e.lines||[]).map(function(l){return '<div class="tiny"><span class="acc-code">'+esc(l.code||'')+'</span> '+esc(l.name||l.label||'')+' · Dr '+peso(l.debit||0)+' · Cr '+peso(l.credit||0)+'</div>';}).join('');return '<tr><td class="tiny">'+esc(e.date||'')+'</td><td><b>'+esc(e.memo||e.type||'Journal entry')+'</b><div class="tiny muted">'+esc(e.ref||e.reference||e.id)+'</div>'+lines+'</td><td class="tiny">'+esc(e.reversalOf?'Reversal of '+e.reversalOf:e.correctsMovementId?'Corrects '+e.correctsMovementId:e.sourceType||'Original entry')+'</td></tr>';}).join('');}
App.showSupplierPayableLedger=function(encodedKey){
  var key=decodeURIComponent(encodedKey),docs=payableSupplierDocs(key);if(!docs.length)return alert('No supplier payable records were found. Refresh Finance Books.');var supplier=docs[0].party||'Supplier',open=r2(docs.reduce(function(s,d){return s+((d.status||'open')==='open'?(Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0):0);},0)),original=r2(docs.reduce(function(s,d){return s+(Number(d.amount)||0);},0));
  var rows=docs.map(function(d){var remaining=(d.status||'open')==='open'?r2(Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0):0;return '<tr><td><button class="btn sm ghost" onclick="App.showPayableTrace(\''+esc(d.id)+'\')">'+esc(d.ref||d.id)+'</button>'+(d.openingBalance===true?'<div class="tiny muted">Opening balance</div>':'')+'</td><td class="tiny">'+esc(d.date||'')+'</td><td class="tiny">'+esc(d.due||'—')+'</td><td>'+esc(payableStatusLabel(d))+'</td><td class="num">'+peso(d.amount||0)+'</td><td class="num">'+peso(d.paidAmount||0)+'</td><td class="num"><b>'+peso(remaining)+'</b></td><td>'+(remaining>0&&d.provisional!==true?'<button class="btn sm primary" onclick="App.txnPay(\''+esc(d.id)+'\')">Pay</button>':'')+'</td></tr>';}).join(''),m=document.getElementById('modal');
  m.innerHTML='<div class="modal-head"><h3>'+esc(supplier)+' · supplier subsidiary ledger</h3><button class="x" onclick="App.closeModal()">×</button></div><div class="modal-body"><div class="hint">Select an invoice for its source and payment trail, or Pay to record a full or partial settlement. Original invoices are never rewritten.</div><div class="section-label">Accounts Payable · 2000</div><div class="kpis"><div class="kpi bad"><div class="lbl">Outstanding</div><div class="val">'+peso(open)+'</div></div><div class="kpi"><div class="lbl">Invoices recorded</div><div class="val">'+docs.length+'</div><div class="sub">Original value '+peso(original)+'</div></div></div><div class="tbl-wrap"><table><thead><tr><th>Invoice / source</th><th>Date</th><th>Due</th><th>Status</th><th class="num">Original</th><th class="num">Paid</th><th class="num">Outstanding</th><th></th></tr></thead><tbody>'+rows+'<tr class="total-row"><td colspan="6">Supplier outstanding balance</td><td class="num">'+peso(open)+'</td><td></td></tr></tbody></table></div><div id="supplierAdvanceDetail" style="margin-top:1rem"><div class="section-label">Supplier Advances · 1115</div><div class="tiny muted">Loading original payments and allocations…</div></div></div><div class="modal-foot"><button class="btn ghost" onclick="App.closeModal()">Close</button></div>';document.getElementById('modalBg').classList.add('show');App.loadSupplierAdvances(docs[0]);
};
App.loadSupplierAdvances=function(supplier){var root=document.getElementById('supplierAdvanceDetail');if(!root)return;if(!window.__getSupplierAdvanceDetails){root.innerHTML='<div class="section-label">Supplier Advances · 1115</div><div class="tiny muted">Advance details are unavailable. Refresh and sign in again.</div>';return;}window.__getSupplierAdvanceDetails({supplierId:supplier.supplierId||'',supplierName:supplier.party||''}).then(function(data){if(!document.body.contains(root))return;var rows=(data.rows||[]),remaining=r2(rows.reduce(function(s,x){return s+(Number(x.remainingAmount)||0);},0)),html=rows.map(function(x){var allocations=Object.keys(x.allocations||{}).map(function(id){var a=x.allocations[id]||{},p=(window.__piMap||{})[id]||{};return '<div class="tiny"><button class="btn sm ghost" onclick="App.piDetail(\''+esc(id)+'\')">'+esc(a.ref||p.ref||id)+'</button> · '+peso(a.amount||p.total||0)+'</div>';}).join('')||'<span class="tiny muted">No purchase allocated yet</span>';return '<tr><td><b>'+esc(x.reference||x.id)+'</b><div class="tiny muted">'+esc(x.source==='register_shift'?'Register shift '+x.shiftId:'Revolving fund')+' · '+esc(x.purpose||'')+'</div></td><td class="num">'+peso(x.amount)+'</td><td>'+allocations+'</td><td class="num">'+peso(x.allocatedAmount)+'</td><td class="num"><b>'+peso(x.remainingAmount)+'</b></td></tr>';}).join('');root.innerHTML='<div class="section-label">Supplier Advances · account 1115</div><div class="hint">Original supplier payments remain assets until allocated to a purchase or returned. Allocations link to the purchase records below.</div><div class="tbl-wrap"><table><thead><tr><th>Original payment / purpose</th><th class="num">Paid</th><th>Purchase allocations</th><th class="num">Allocated</th><th class="num">Unallocated</th></tr></thead><tbody>'+(html||'<tr><td colspan="5" class="empty">No supplier advances for this supplier.</td></tr>')+'<tr class="total-row"><td colspan="4">Supplier advance balance in account 1115</td><td class="num">'+peso(remaining)+'</td></tr></tbody></table></div>'+(data.truncated?'<div class="tiny muted">Showing the newest 200 advances.</div>':'');}).catch(function(e){root.innerHTML='<div class="section-label">Supplier Advances · 1115</div><div class="balance-bar off">Could not load advance details: '+esc(e&&e.message||e)+'</div>';});};
App.showPayableTrace=function(id){
  var d=(window.__apMap||{})[id];if(!d)return alert('This payable is no longer available. Refresh Finance Books.');var invoice=d.purchaseInvoiceId&&(window.__piMap||{})[d.purchaseInvoiceId],entries=payableRelatedEntries(Object.assign({id:id},d)),remaining=(d.status||'open')==='open'?r2(Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0):0,items=invoice&&(invoice.lines||[]).map(function(l){return '<tr><td>'+esc(l.itemName||l.itemId||'Item')+'</td><td class="num">'+esc(l.qty||'')+' '+esc(l.unit||'')+'</td><td class="num">'+peso(l.total||0)+'</td></tr>';}).join(''),payments=Object.keys(d.settlements||{}).map(function(paymentId){var p=d.settlements[paymentId]||{};return '<tr><td>'+esc(p.date||'')+'</td><td>'+esc(p.reference||paymentId)+'</td><td>'+esc(p.accountId||'')+'</td><td class="num">'+peso(p.amount||0)+'</td><td>'+(p.status==='reversed'?'<span class="tiny muted">Reversed</span>':'<button class="btn sm ghost" onclick="App.reversePayablePayment(\''+esc(id)+'\',\''+esc(paymentId)+'\')">Reverse</button>')+'</td></tr>';}).join(''),m=document.getElementById('modal');
  m.innerHTML='<div class="modal-head"><h3>'+esc(d.party||'Supplier')+' · '+esc(d.ref||id)+'</h3><button class="x" onclick="App.closeModal()">×</button></div><div class="modal-body"><div class="hint"><b>Original payable → purchase invoice → journal → payment / reversal.</b> Posted history is never rewritten.</div><div class="grid2"><div class="card card-pad"><div class="section-label">Payable document</div><div class="tiny">ID: '+esc(id)+'</div><div class="tiny">Invoice date: '+esc(d.date||'—')+'</div><div class="tiny">Due: '+esc(d.due||'—')+'</div><div class="tiny">Status: '+esc(payableStatusLabel(d))+'</div><div><b>'+peso(d.amount||0)+'</b> original · <b>'+peso(d.paidAmount||0)+'</b> paid · <b>'+peso(remaining)+'</b> outstanding</div></div><div class="card card-pad"><div class="section-label">Original source</div>'+(invoice?'<div><b>Purchase invoice '+esc(invoice.ref||d.purchaseInvoiceId)+'</b></div><div class="tiny">Purchase ID: '+esc(d.purchaseInvoiceId)+'</div><div class="tiny">Received '+esc(invoice.date||'—')+' by '+esc(invoice.by||'—')+'</div><div class="tiny">'+esc(invoice.description||'Inventory / expense purchase')+'</div>':'<div><b>'+(d.openingBalance===true?'Supplier opening balance':'Finance payable entry')+'</b></div><div class="tiny">Source movement: '+esc(d.movementId||'—')+'</div>')+'</div></div><div class="card" style="margin-top:1rem"><div class="card-pad"><div class="section-label">Payment history</div></div><div class="tbl-wrap"><table><thead><tr><th>Date</th><th>Reference</th><th>Paid from</th><th class="num">Amount</th><th></th></tr></thead><tbody>'+(payments||'<tr><td colspan="5" class="empty">No payments recorded.</td></tr>')+'</tbody></table></div></div>'+(invoice?'<div class="card" style="margin-top:1rem"><div class="card-pad"><div class="section-label">Original invoice lines</div></div><div class="tbl-wrap"><table><thead><tr><th>Item / treatment</th><th class="num">Quantity</th><th class="num">Amount</th></tr></thead><tbody>'+(items||'<tr><td colspan="3" class="empty">No item lines retained.</td></tr>')+'<tr class="total-row"><td colspan="2">Invoice total</td><td class="num">'+peso(invoice.total||d.amount||0)+'</td></tr></tbody></table></div></div>':'')+'<div class="card" style="margin-top:1rem"><div class="card-pad"><div class="section-label">Related Finance Books transactions · '+entries.length+'</div><div class="tiny muted">Includes the original posting and every linked payment, correction, or reversal.</div></div><div class="tbl-wrap"><table><thead><tr><th>Date</th><th>Journal entry and debit / credit lines</th><th>Relationship</th></tr></thead><tbody>'+(payableEntryTable(entries)||'<tr><td colspan="3" class="empty">No journal entry is currently linked. This requires reconciliation review.</td></tr>')+'</tbody></table></div></div></div><div class="modal-foot"><button class="btn ghost" onclick="App.showSupplierPayableLedger(\''+encodeURIComponent(payableSupplierKey(d)).replace(/'/g,'%27')+'\')">Back to supplier ledger</button><button class="btn" onclick="App.closeModal();App.go(\'journal\')">Open full Journal</button><button class="btn primary" onclick="App.closeModal()">Close</button></div>';document.getElementById('modalBg').classList.add('show');
};
App.showSupplierAdvanceLedger=function(){
  var m=document.getElementById('modal');
  m.innerHTML='<div class="modal-head"><h3>Supplier Advances · account 1115</h3><button class="x" onclick="App.closeModal()">×</button></div><div class="modal-body"><div class="hint">Supplier advances are asset payments made before the final purchase amount is known. The original payment stays in account 1115 until it is allocated to an inventory purchase, returned, or otherwise corrected through an auditable Finance Books transaction.</div><div id="allSupplierAdvanceDetail"><div class="tiny muted">Loading original payments and allocations…</div></div></div><div class="modal-foot"><button class="btn primary" onclick="App.closeModal()">Close</button></div>';
  document.getElementById('modalBg').classList.add('show');
  var root=document.getElementById('allSupplierAdvanceDetail');
  if(!window.__getSupplierAdvanceDetails){root.innerHTML='<div class="balance-bar off">Advance details are unavailable. Refresh and sign in again.</div>';return;}
  window.__getSupplierAdvanceDetails({supplierId:'',supplierName:''}).then(function(data){
    ...
    root.innerHTML='<div class="section-label">Original payments and allocations · '+rows.length+(data.truncated?' of more than 200':'')+'</div><div class="tbl-wrap"><table><thead><tr><th>Supplier / reference</th><th>Funding and purpose</th><th class="num">Paid</th><th>Purchase allocations</th><th class="num">Allocated</th><th class="num">Unallocated</th></tr></thead><tbody>'+(html||'<tr><td colspan="6" class="empty">No supplier advances were found.</td></tr>')+'<tr class="total-row"><td colspan="5">Unallocated Supplier Advances · account 1115</td><td class="num">'+peso(balance)+'</td></tr></tbody></table></div>'+(data.truncated?'<div class="tiny muted">Showing the newest 200 advances. Narrow the supplier ledger when reviewing older history.</div>':'');
  }).catch(function(e){root.innerHTML='<div class="balance-bar off">Could not load advance details: '+esc(e&&e.message||e)+'</div>';});
};
function subledgerPage(kind){
  ...
  var docs = openDocs(map).filter(function(d){return isAr||d.type!=='customer_change_refund';}).sort(function(a,b){return String(a.due||'9999-99-99').localeCompare(String(b.due||'9999-99-99'));}),openingButton=isAr?'':openingPayableHeaderButton()+'<button class="btn ghost" onclick="App.showSupplierAdvanceLedger()">Supplier Advances · 1115</button>';
  ...
  var total=0; docs.forEach(function(d){var amt=Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0; total+=amt; buckets[agingBucket(d.due)]+=amt;});
  ...
  var rows = docs.map(function(d){
    ...
    var party=isAr?'<b>'+esc(d.party||'—')+'</b>':'<button class="btn sm ghost" onclick="App.showSupplierPayableLedger(\''+encodeURIComponent(payableSupplierKey(d)).replace(/'/g,'%27')+'\')">'+esc(d.party||'—')+'</button>';
    return '<tr><td>'+party+(d.ref?'<div class="tiny muted">'+esc(d.ref)+'</div>':'')+(d.openingBalance===true?openingPayableInfo(d):'')+'</td><td class="tiny">'+esc(d.type||'')+'</td><td class="tiny">'+esc(d.date||'')+'</td><td class="tiny">'+esc(d.due||'—')+'</td><td><span class="type-pill '+(overdue?'t-expense':'t-income')+'">'+bkt+'</span></td><td class="num">'+peso(d.remainingAmount!=null?d.remainingAmount:d.amount)+'</td>'+(isAr?'':'<td>'+(d.provisional===true?'<span class="tiny muted">Finalize invoice in Purchases first</span>':'<button class="btn sm ghost" onclick="App.correctPayable(\''+esc(d.id)+'\')">'+(d.type==='customer_change_refund'?'Close to capital':'Pay / correct')+'</button>'+(d.openingBalance===true?openingPayableRowButton(d):''))+'</td>')+'</tr>';}).join('');
  if(!isAr){var groups={};docs.filter(function(d){return d.type!=='customer_change_refund';}).forEach(function(d){var key=payableSupplierKey(d),g=groups[key]||(groups[key]={key:key,name:d.party||'Supplier',count:0,outstanding:0,overdue:0,oldestDue:''}),value=Number(d.remainingAmount!=null?d.remainingAmount:d.amount)||0;g.count++;g.outstanding=r2(g.outstanding+value);if(agingBucket(d.due)!=='Current'&&agingBucket(d.due)!=='No due date')g.overdue=r2(g.overdue+value);if(d.due&&(!g.oldestDue||d.due<g.oldestDue))g.oldestDue=d.due;});var supplierRows=Object.keys(groups).map(function(key){return groups[key];}).sort(function(a,b){return b.outstanding-a.outstanding||a.name.localeCompare(b.name);}).map(function(g){return '<tr><td><button class="btn sm ghost" onclick="App.showSupplierPayableLedger(\''+encodeURIComponent(g.key).replace(/'/g,'%27')+'\')"><b>'+esc(g.name)+'</b></button></td><td class="num">'+g.count+'</td><td class="tiny">'+esc(g.oldestDue||'—')+'</td><td class="num">'+peso(g.overdue)+'</td><td class="num"><b>'+peso(g.outstanding)+'</b></td></tr>';}).join('');return '<div class="page-head"><div><h2>Payables</h2><p>Supplier balances · '+docs.length+' outstanding invoice(s)</p></div><div class="btn-row">'+openingButton+'</div></div><div class="kpis"><div class="kpi bad"><div class="lbl">Total supplier payables</div><div class="val">'+pesoNoDec(total)+'</div><div class="sub">'+Object.keys(groups).length+' supplier(s)</div></div>'+agingCards+'</div><div class="hint">Select a supplier to review every invoice, original amount, payment history and outstanding balance. Older unpaid invoices remain visible regardless of the report-date filter.</div><div class="card"><div class="tbl-wrap"><table><thead><tr><th>Supplier</th><th class="num">Open invoices</th><th>Oldest due</th><th class="num">Overdue</th><th class="num">Outstanding balance</th></tr></thead><tbody>'+supplierRows+'<tr class="total-row"><td colspan="4">Total supplier outstanding</td><td class="num">'+peso(total)+'</td></tr></tbody></table></div></div>'+settledSection(map);}
  var t = arApTotals();
...
function booksSupplierOptions(selected){return '<option value="">— select supplier —</option>'+booksSuppliers().map(function(x){return '<option value="'+esc(x.id)+'"'+(selected===x.id?' selected':'')+'>'+esc(x.name)+'</option>';}).join('');}
App.createSupplierInline=function(selectId){if(!window.__manageSupplier)return alert('Supplier service is not ready. Refresh Finance Books.');var name=prompt('Official supplier name');if(!name)return;window.__manageSupplier({action:'create',name:name}).then(function(x){window.__supplierMap=window.__supplierMap||{};window.__supplierMap[x.supplierId]=Object.assign({},window.__supplierMap[x.supplierId]||{},{name:x.name,active:true});var select=document.getElementById(selectId);if(select){select.innerHTML=booksSupplierOptions(x.supplierId);select.value=x.supplierId;}}).catch(function(e){alert('Could not create supplier: '+(e&&e.message||e));});};
App.convertSuspenseAdvance=function(id){var e=ENTRIES().find(function(x){return x.id===id;}),line=e&&(e.lines||[]).find(function(x){return x.code==='1900'&&Number(x.debit)>0&&!Number(x.credit);});if(!e||!line)return alert('This journal no longer has an eligible Suspense debit. Refresh Finance Books.');if(!window.__booksConvertSuspense)return alert('The supplier-advance conversion service is not ready. Refresh Finance Books.');App._txnModal('Convert Suspense to supplier advance','<div class="hint">This does not pay cash again. It clears Suspense into one supplier-linked advance, keeps the original cash reduction, and makes the amount available for later allocation in Purchases.</div><div class="grid2"><div class="field"><label>Amount</label><input value="'+esc(Number(line.debit).toFixed(2))+'" disabled/></div><div class="field"><label>Original date</label><input value="'+esc(e.date||'')+'" disabled/></div></div><div class="field"><label>Supplier</label><div style="display:flex;gap:.35rem"><select id="sa_supplier">'+booksSupplierOptions('')+'</select><button class="btn sm ghost" type="button" onclick="App.createSupplierInline(\'sa_supplier\')">＋ New</button></div></div><div class="field"><label>Payment / receipt reference</label><input id="sa_ref" value="'+esc(e.ref||'')+'" maxlength="120"/></div><div class="field"><label>What will this advance purchase?</label><textarea id="sa_purpose" maxlength="300"></textarea></div><div class="field"><label>Conversion reason</label><textarea id="sa_reason" maxlength="300">Correct advance payment posted to Suspense</textarea></div>','Request approval & convert',function(btn){var supplierId=fval('sa_supplier'),supplier=(window.__supplierMap||{})[supplierId],reference=fval('sa_ref'),purpose=fval('sa_purpose'),reason=fval('sa_reason');if(!supplier)return alert('Select or create the supplier.');if(!reference)return alert('Enter the payment or receipt reference.');if(!purpose)return alert('Enter what the supplier advance will purchase.');if(!reason)return alert('Enter the conversion reason.');btn.disabled=true;var old=btn.textContent;btn.textContent='Converting…';window.__booksConvertSuspense({originalMovementId:id,supplierId:supplierId,amount:Number(line.debit),reference:reference,purpose:purpose,reason:reason}).then(function(){App.closeModal();alert('Suspense cleared into a supplier advance. It is now available in Admin Purchases after selecting '+supplier.name+'.');App.render();}).catch(function(err){alert('Could not convert Suspense: '+((err&&err.message)||err));btn.disabled=false;btn.textContent=old;});});};
...
PAGES.transactions = function(){
  ...
  var transactions=financeTransactions(),transactionRows=transactions.map(function(e){var amount=(e.lines||[]).reduce(function(s,l){return s+(Number(l.debit)||0);},0);return '<tr><td>'+esc(e.date||'')+'</td><td><b>'+esc(e.memo||'Finance transaction')+'</b><div class="tiny muted">'+esc(e.ref||e.id||'')+'</div></td><td>'+esc(e.sourceType||e.source||'Manual')+'</td><td>'+esc(csvEntryStatus(e))+'</td><td class="num">'+peso(amount)+'</td></tr>';}).join('');
  return '<div class="page-head"><div><h2>Transactions</h2><p>Post finance transactions to the shared ledger through your server functions</p></div><div class="btn-row">'+csvButton('transactions')+'</div></div>'+
    ...
    '</div>'+ownerFundingHistory()+'<div class="card" style="margin-top:1rem"><div class="card-pad"><div class="section-label">Transaction list · '+esc(periodLabel())+'</div><div class="tiny muted">One row per Finance entry. Download CSV for source IDs and debit/credit totals.</div></div><div class="tbl-wrap"><table><thead><tr><th>Date</th><th>Transaction</th><th>Source</th><th>Status</th><th class="num">Amount</th></tr></thead><tbody>'+(transactionRows||'<tr><td colspan="5" class="empty">No Finance transactions in this period.</td></tr>')+'</tbody></table></div></div>';
};
...
App.txnBill=function(){ App._txnModal('New bill','<div class="grid2"><div class="field"><label>Supplier / vendor</label><div style="display:flex;gap:.35rem"><select id="tb_supplier">'+booksSupplierOptions('')+'</select><button class="btn sm ghost" type="button" onclick="App.createSupplierInline(\'tb_supplier\')">＋ New</button></div></div><div class="field"><label>Category</label><select id="tb_type">'+catOptions('rent')+'</select></div></div><div class="grid2"><div class="field"><label>Amount</label><input id="tb_amount" type="number" step="0.01" min="0"/></div><div class="field"><label>Bill date</label><input id="tb_date" type="date" value="'+todayStr()+'"/></div></div><div class="grid2"><div class="field"><label>Due date</label><input id="tb_due" type="date"/></div><div class="field"><label>Bill / invoice reference</label><input id="tb_ref" required placeholder="Required"/></div></div>','Post bill',function(btn){var supplierId=fval('tb_supplier'),supplier=(window.__supplierMap||{})[supplierId],amount=Number(fval('tb_amount'))||0;if(!supplier)return alert('Select or create a supplier.');if(!(amount>0))return alert('Amount must be greater than zero.');if(!fval('tb_ref'))return alert('Bill or invoice reference is required.');App._txnRun(btn,{action:'create_payable',commandId:uid(),documentId:uid(),supplierId:supplierId,party:supplier.name,type:fval('tb_type'),amount:amount,date:fval('tb_date'),due:fval('tb_due'),ref:fval('tb_ref')});});};
...
App.reversePayablePayment=function(id,paymentId){
  var d=(window.__apMap||{})[id];
  if(!d)return alert('That bill is no longer available. Refresh Finance Books.');
  paymentId=paymentId||d.settlementMovementId;var payment=(d.settlements||{})[paymentId]||{};
  if(!paymentId) return alert('This bill has no settlement record, so it cannot be reversed cleanly. Raise it for review.');
  if(payment.status==='reversed'||payment.reversalMovementId)return alert('This payment has already been reversed.');
  App._txnModal('Reverse payment \u00b7 '+esc(d.party||id),'<div class="hint">This reverses only the selected payment. Other partial payments remain posted and the original invoice stays unchanged.</div><div class="grid2"><div class="field"><label>Amount</label><input value="'+r2(Number(payment.amount||d.paidAmount||d.amount)||0).toFixed(2)+'" disabled/></div><div class="field"><label>Correction date</label><input id="rp_date" type="date" value="'+todayStr()+'"/></div></div><div class="field"><label>Correction reference</label><input id="rp_ref" value="'+esc(payment.reference||d.settlementReference||'')+'" placeholder="Required"/></div><div class="field"><label>Reason for reversing</label><textarea id="rp_reason" maxlength="300" placeholder="Required"></textarea></div>','Reverse payment',function(btn){
    var reference=fval('rp_ref'),reason=fval('rp_reason'),date=fval('rp_date');
...
App.txnPay=function(preselectId){ var target=preselectId&&(window.__apMap||{})[preselectId]; if(preselectId&&!target) return alert('That payable is no longer available. Refresh Finance Books.'); if(target&&target.provisional===true) return alert('Finalize the supplier invoice in Purchases before paying this provisional obligation.'); var opts=openDocOptions(window.__apMap,preselectId,true),initial=target?r2(Number(target.remainingAmount!=null?target.remainingAmount:target.amount)||0):0; if(!opts) return alert('No open payables to pay. A purchase still waiting for its supplier invoice must be finalized in Purchases first.'); App._txnModal('Pay a bill','<div class="field"><label>Open payable</label><select id="tp_doc" onchange="App._payableAmount()">'+opts+'</select></div><div class="grid2"><div class="field"><label>Payment amount</label><input id="tp_amount" type="number" min="0.01" step="0.01" value="'+(initial?initial.toFixed(2):'')+'"/><div class="tiny muted">Enter any amount up to the outstanding balance.</div></div><div class="field"><label>Date</label><input id="tp_date" type="date" value="'+todayStr()+'"/></div></div><div class="field"><label>Pay from</label><select id="tp_source" onchange="App._paySourceToggle()">'+billPaymentSourceOptions()+'</select></div><div class="field" id="tp_owner_wrap" style="display:none"><label>Owner / partner who paid</label><input id="tp_owner" placeholder="Required for Owner\'s Capital"/></div><div class="hint">A partial payment reduces only this invoice\'s outstanding balance. Register Cash Float is protected and cannot pay bills.</div><div class="field"><label>Payment reference</label><input id="tp_ref" required placeholder="Required · receipt, transfer, cheque or voucher ID"/></div>','Pay',function(btn){var source=fval('tp_source'),owner=fval('tp_owner'),docId=fval('tp_doc'),d=(window.__apMap||{})[docId],remaining=r2(Number(d&&d.remainingAmount!=null?d.remainingAmount:d&&d.amount)||0),value=r2(Number(fval('tp_amount'))||0);if(!(value>0))return alert('Payment amount must be greater than zero.');if(value>remaining)return alert('Payment cannot exceed the outstanding balance of '+peso(remaining)+'. Use Supplier Advances for an overpayment.');if(!source)return alert('Choose a payment source.');if(source==='cash_float')return alert('Register Cash Float is protected and cannot be used to pay bills.');if(source==='owner_capital'&&!owner)return alert('Enter the owner or partner who paid.');if(!fval('tp_ref'))return alert('Payment reference is required.');App._txnRun(btn,{action:'pay_payable',commandId:uid(),documentId:docId,amount:value,paymentSource:source,accountId:source,date:fval('tp_date'),ref:fval('tp_ref'),ownerName:owner}); });if(!initial)setTimeout(App._payableAmount,0); };
...
PAGES.purchases = function(){
  ...
  var period=window.AccazaReportPeriod&&window.AccazaReportPeriod.get?window.AccazaReportPeriod.get():null;
  var head='<div class="page-head"><div><h2>Purchases</h2><p><span class="linkish" onclick="App.go(&#39;transactions&#39;)">← Transactions</span> · '+invs.length+' invoices · '+pesoNoDec(total)+' total'+(period?' · '+esc(period.label||(period.from+' to '+period.to)):'')+'</p></div></div>';
