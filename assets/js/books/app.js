...
function cfStatement(){
  ...
  moves.forEach(m=>{if(fullyReversed[m.id])return;const d=cfDay(m.occurredAt),ds=cfDeltas(m),net=r2(Object.values(ds).reduce((s,v)=>s+v,0));if(d<CF_FROM)Object.keys(ds).forEach(k=>bump(begin,k,ds[k]));if(d<=CF_TO)Object.keys(ds).forEach(k=>bump(ending,k,ds[k]));if(d>=CF_FROM&&d<=CF_TO&&Math.abs(net)>=.005){if(cfIsCorrection(m)){corrections=r2(corrections+net);correctionDetail.push({date:d,id:m.id,type:'Manual Books correction',net});}else{const cat=cfCategory(m,net);bump(net>0?add:ded,cat,Math.abs(net));detail.push({date:d,id:m.id,type:cat,net});}}});
...
const PAGES = {
  ...
  cashflow(){
    ...
    return `<div class="page-head"><div><h2>Cash Flow</h2><p>Authoritative cash statement · moved from Admin</p></div><div class="btn-row">${csvButton('cashflow')}<button class="btn" onclick="App.go('transactions')">Record transaction</button><button class="btn primary" onclick="App.cashAccountEdit('')">+ Cash account</button></div></div>
      ...
      <div class="card" style="margin-bottom:1rem"><div class="card-pad"><div class="section-label">Banking actions · excluded from cash flow</div><div class="tiny muted">Use this operational queue only to document where cash is held. Moving the remaining Undeposited Collection to a bank is an internal transfer—not income, a receipt, or an expense—and never appears in the statement above.</div>${custody.length?'<div style="margin-top:.65rem"><button class="btn primary" onclick="App.cfRecordCustodyDeposit()">Record selected bank transfer</button></div>':''}</div><div class="tbl-wrap"><table><thead><tr><th>Cash source</th><th>Date</th><th class="num">Available to transfer</th><th></th></tr></thead><tbody>${payoutDepositRows}${custodyDepositRows||(!payoutDepositRows?'<tr><td colspan="4" class="empty">No banking actions waiting.</td></tr>':'')}</tbody></table></div></div>
  ...
  journal(){
    ...
    const rows = display.map(g=>{
      ...
      return `<tr>
        ...
        <td class="journal-actions">${e.revision?`<button class="btn sm ghost" onclick="App.cashJournalHistory(\'${e.id}\')">History · r${e.revision}</button>`:""}${!closed&&customerPayableId?`<button class="btn sm primary" onclick="App.correctPayable('${customerPayableId}')">Close linked payable</button>`:!closed&&customerPayableControl?`<button class="btn sm ghost" onclick="App.editEntry('${e.id}')">Open customer payable</button>`:!closed&&!posLocked&&!e.reversalOf&&!e.reversedByMovementId?`<button class="btn sm ghost" onclick="App.editEntry('${e.id}')">Edit / correct</button> ${(!e.revision)&&manualEntry?`<button class="btn sm ghost" onclick="App.reverseEntry('${e.id}',false)">Reverse</button> <button class="btn sm ghost" onclick="App.reverseEntry('${e.id}',true)">Void</button>`:''}`:'<span class="tiny muted">'+(status||(posLocked?'POS locked':'Automatic posting'))+'</span>'}</td></tr>`;
    }).join("");
