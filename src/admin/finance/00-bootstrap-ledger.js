...
function refreshCashBalances(forceAfterCurrent){var a=A();if(cashBalanceRefresh)return forceAfterCurrent?cashBalanceRefresh.then(function(){return refreshCashBalances(false);}):cashBalanceRefresh;if(!a||!a.getCurrentCashBalances)return Promise.resolve(false);cashBalanceRefresh=a.getCurrentCashBalances().then(function(result){var data=(result&&result.data)||result||{},balances=data.balances||{};if(data.source!=='cashBalanceSummary')throw new Error('Cash balance summary is unavailable.');cashBalancesMap=balances;cashRegisterGross=Math.round((Number(data.registerGross)||0)*100)/100;cashBalancesReady=true;registerFloatAmount=Math.round((Number(data.registerFloatAmount)||4000)*100)/100;window.dispatchEvent(new CustomEvent('accaza:cash-balances-updated'));return true;}).catch(function(e){cashBalancesMap={};cashRegisterGross=0;cashBalancesReady=false;console.error('Could not refresh current Finance Books cash balances',e);window.dispatchEvent(new CustomEvent('accaza:cash-balances-updated'));return false;}).finally(function(){cashBalanceRefresh=null;});return cashBalanceRefresh;}
...
function currentCashBalance(id){if(id==='cash_float')return registerFloatAmount;if(!cashBalancesReady)return 0;var b=id==='cash_on_hand'?cashRegisterGross:Number(cashBalancesMap[id])||0;if(id==='cash_on_hand')b=Math.max(0,b-registerFloatAmount);return Math.round(b*100)/100;}
